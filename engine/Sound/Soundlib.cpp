...
CachedSound *SoundManager::OpenSound(uint32 wave)
{
	if (!sound_initialised)
	{
		return (NULL);
	...
	uint32	numbytes;	//=file.GetLength()-file.GetPosition();
...
int SoundManager::GetWaveSize(uint32 wave)
{
...
CachedSound *	SoundManager::EnsureInCache(uint32 wave)			//  Places wave in cache and returns
{
...
SOUNDERROR SoundManager::PreLoadSound(uint32 wave)	//  Ensures a sound is loaded into the
																	//  cache
{
	if (mixer_suspended || !sound_initialised)
	{
...
SOUNDERROR SoundManager::PlaySoundEffect(uint32 wave, int ticks, long volume, long pan)		//  Loads in sound or locates within
...
SOUNDERROR SoundManager::StartControlledSound(uint32 wave, SOUNDHANDLE &handle, long volume, long pan, bool looped)
{
	if (mixer_suspended || !sound_initialised)
	{
