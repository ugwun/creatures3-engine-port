...
//					Play, Pause, BeginTrack, InteruptTrack,
// Fade all wait 					until the next update
...
// Helper: find the first path separator ('\' or '/') in a string.
// CAOS scripts use backslash (e.g. "DS_music.mng\TrackName") while
// POSIX paths use forward slash.  We accept either.
static std::string::size_type findPathSep(const std::string &s) {
  std::string::size_type fwd = s.find('/');
  std::string::size_type bck = s.find('\\');
...
bool MusicManager::LoadScrambled() {
  ...
  try {
    ...
    std::string dir(buf);
    std::string path = dir + currentMNG;

    // On case-sensitive filesystems (macOS APFS), the config-supplied MNG
    // name may have different casing from the actual file on disk.
    // Try the exact path first; if it fails, scan the directory for a
    // case-insensitive match and use the real filename.
    if (access(path.c_str(), F_OK) != 0) {
      std::string lowerTarget = currentMNG;
...
// ----------------------------------------------------------------------
MusicTrack *MusicManager::GetTrack(const char* trackName) {
  ...
  if (oldTrack) {
    ...
    if (oldTrack->IsFinished()) {
      ...
      if (!desiredTrack.empty()) {
        ...
        std::string track;
        currentMNG = desiredTrack.substr(0, findPathSep(desiredTrack));
        if (findPathSep(desiredTrack) == std::string::npos)
          currentMNG = "Music.mng";
        DestroyContents();
        theMusicSoundManager->SetMNGFile(currentMNG);
        LoadScrambled();
        status = Playing;
        track = desiredTrack.substr(findPathSep(desiredTrack) + 1);
...
// ----------------------------------------------------------------------
bool MusicManager::SetVariable(const char* name, MusicValue value) {
...
//				manager
// ----------------------------------------------------------------------
bool MusicManager::ReadVariableContents(const char* name, MusicValue &value) {
...
// ----------------------------------------------------------------------
MusicValue *MusicManager::GetVariable(const char* varName, bool lvalue) {
...
// ----------------------------------------------------------------------
void MusicManager::ActivateBeginTrack(const char* name) {
  ...
  if (findPathSep(incomingMNG) != std::string::npos)
    incomingMNG = incomingMNG.substr(0, findPathSep(incomingMNG));
  else
    incomingMNG = "Music.mng";
  std::string realName = name;
  realName = realName.substr(findPathSep(realName) + 1);
  ...
  if (incomingMNG != currentMNG) {
    ...
    if (oldTrack) {
      ...
      parsedTrack = parsedTrack.substr(findPathSep(parsedTrack) + 1);
...
//				playing.
// ----------------------------------------------------------------------
void MusicManager::ActivateInteruptTrack(const char* name) {
  // Special case - check if we're being asked for silence
  if (strcmp(name, "Silence") == 0) {
    // Just fade out the current track
    Fade();
...
// ----------------------------------------------------------------------
const char* MusicManager::GetCurrentTrackName() {
  if (currentTrack) {
