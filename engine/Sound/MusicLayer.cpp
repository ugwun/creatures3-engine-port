...
void MusicLayer::Update(MusicValue uberVolume,
	...
	for (queueit = queue.begin(); queueit != queue.end();queueit++)
		...
		uint32 nameToken = 0xff000000 + (uint32) sound->GetWave();
...
// ----------------------------------------------------------------------
MusicValue *MusicLayer::GetVariable(const char* varName, bool lvalue)
	{
...
// ----------------------------------------------------------------------
MusicWaveID MusicLayer::GetWaveID(const char* name)
	{
...
// ----------------------------------------------------------------------
MusicEffectID MusicLayer::GetEffectID(const char* name)
	{
...
// ----------------------------------------------------------------------
uint32 MusicLayer::ConvertName(const char* name)
	{
	uint32 token=0;
...
void MusicLayer::PlaySound(MusicValue uberVolume,
	...
	// Convert the wave ID into a token by simply adding 0xff000000
	uint32 nameToken = 0xff000000 + (uint32) wave;
...
SoundHandle MusicLayer::LoopSound(MusicValue uberVolume,
	...
	uint32 nameToken = ConvertName( playWave -> GetName() );*/
...
//			etc.
// ----------------------------------------------------------------------
void MusicLayer::Munge(const char* name, const char* script)
	{
