...
public:
  ...
  SOUNDERROR PreLoadSound(uint32 wave); //  Ensures a sound is loaded into the
                                       //  cache

  SOUNDERROR PlaySoundEffect(uint32 wave, int ticks = 0, long volume = 0,
                             long pan = 0);
  //  Load and Play sound immediately or
  //  preload sound and queue it to be played
  //  after 'ticks' has elapsed

  SOUNDERROR StartControlledSound(uint32 wave, SOUNDHANDLE &handle,
...
private:
  ...
  std::map<uint32, void *> sound_cache;
