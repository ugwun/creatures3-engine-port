...
SOUNDERROR SoundManager::FlushCache() {
  ...
  for (std::map<uint32, void *>::iterator it = sound_cache.begin();
...
void *SoundManager::LoadWavForID(uint32 wave) {
  // Check cache first
  std::map<uint32, void *>::iterator it = sound_cache.find(wave);
...
SOUNDERROR SoundManager::PreLoadSound(uint32 wave) {
  if (!sound_initialised)
...
SOUNDERROR SoundManager::PlaySoundEffect(uint32 wave, int ticks, long volume,
                                         long pan) {
...
SOUNDERROR SoundManager::StartControlledSound(uint32 wave, SOUNDHANDLE &handle,
