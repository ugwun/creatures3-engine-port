...
// ----------------------------------------------------------------------
CreaturesArchive::CreaturesArchive(std::iostream &stream, Mode mode,
                                   bool bNoVersion /* = false */)
    : myStream(stream), myMode(mode), myAgentArchiveStyle(PERSISTENT_OBJECTS),
  ...
  std::string baseHint = "Creatures Evolution Engine - Archived information "
                         "file. zLib 1.13 compressed.";
  ...
  if (mode == Load) {
    ...
    if (!bNoVersion) {
      if (myVersion != GetCurrentVersion()) {
        std::string str = ErrorMessageHandler::Format(
            "archive_error", 6, "CreaturesArchive::CreaturesArchive");
    ...
    int err = deflateInit(
        &myStreamBuffer,
        theApp.GetZLibCompressionLevel()); // compression level 0 - 9
    if (err != Z_OK) {
  ...
  if (myMode == Load)
    ...
    while (myStreamBuffer.avail_in > 0 || zret == Z_OK) {
      ...
      if (myStreamBuffer.avail_out < BUFFER_SIZE) {
        myStream.write((char *)myCompressedDataBuffer,
                       BUFFER_SIZE - myStreamBuffer.avail_out);
        myStreamBuffer.next_out = myCompressedDataBuffer;
        myStreamBuffer.avail_out = BUFFER_SIZE;
      }
    }
...
void CreaturesArchive::Write(const char* string) {
  ASSERT(IsSaving());
...
void CreaturesArchive::Write(const void *data, size_t count) {
  ...
  while (written < count) {
    ...
    if (myStreamBuffer.avail_in < BUFFER_SIZE) {
      ...
      memcpy(myStreamBuffer.next_in + myStreamBuffer.avail_in,
             (char *)data + written, toAdd);
      written += toAdd;
      myStreamBuffer.avail_in += toAdd;
    }
    if (myStreamBuffer.avail_in < BUFFER_SIZE)
      return; // We have not filled the buffer, so there wasn't enough data to
              // think about

    // Right then, we have a full input buffer, let's compress it & dump the
    // output to disk...
    ...
    // Right then, there is no more input in the input buffer, so return to the
    // written < count
    myStreamBuffer.avail_in = 0;
    myStreamBuffer.next_in = myUncompressedDataBuffer;
  }
...
void CreaturesArchive::Write(const PersistentObject *object) {
  ...
  std::map<PersistentObject const *, int32>::const_iterator itor =
      myArchiveMap.find(object);
  ...
  if (itor == myArchiveMap.end()) {
    ...
    std::map<std::string, int32>::const_iterator classItor =
        myClassMap.find(object->GetClassNameX());
...
void CreaturesArchive::WriteFloatRefTarget(const float &v) {
  ...
  if (myFloatMap.find(&v) != myFloatMap.end()) {
    std::string str = ErrorMessageHandler::Format(
        "archive_error", 0, "CreaturesArchive::WriteFloatRef");
    throw Exception(str.c_str());
  }
...
void CreaturesArchive::WriteFloatRef(const float *v) {
  ...
  std::map<ArchiveFloat const *, int32>::const_iterator it = myFloatMap.find(v);
  if (it == myFloatMap.end()) {
    std::string str = ErrorMessageHandler::Format(
        "archive_error", 5, "CreaturesArchive::WriteFloatRef");
    throw Exception(str.c_str());
  }
  Write(it->second);
}
...
void CreaturesArchive::ReadFloatRef(float *&v) {
  ...
  if (id < 0 || id >= myFloatVector.size()) {
    std::string str = ErrorMessageHandler::Format(
        "archive_error", 1, "CreaturesArchive::ReadFloatRef");
    throw Exception(str.c_str());
  }
  v = myFloatVector[id];
}
...
//				object schemas.
// ---------------------------------------------------------------------
int32 CreaturesArchive::GetFileVersion() { return myVersion; }
...
// ---------------------------------------------------------------------
int32 CreaturesArchive::GetCurrentVersion() { return 12; }
