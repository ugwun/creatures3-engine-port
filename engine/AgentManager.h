...
// -------------------------------------------------------------------------
// Class: AgentManager
// *Probably* a singleton that manages the lifecycle of all Agents.
// It handles creation (CreateSimpleAgent, CreateCreature), destruction,
// updating all agents every tick, and lookup by ID or Classifier.
// -------------------------------------------------------------------------
...
public:
  ...
  //
  // ----------------------------------------------------------------------
  AgentHandle CreateCreature(int family, AgentHandle &gene, int gene_index,
                             int8 sex, int8 variant = 0);
  ...
  //				firstimage - base pointer for the objects images
  // in gallery 				imagecount - number of images in
  // gallery 				plane - z - order
  ...
  //				 Add it to the agent library mapped by a unique
  // id
  //
  // ----------------------------------------------------------------------
  AgentHandle CreateSimpleAgent(int family, int genus, int species,
                                FilePath const &gallery, int32 imagecount,
                                int32 firstimage, int32 plane);

  // ----------------------------------------------------------------------
  // Method:      CreateCompoundAgent
  // Arguments:   gallery - name of gallery
  //				baseimage - base pointer for the objects images
  // in gallery 				numimages - number of images in
  // gallery 				uniqueid - to map the agent
  //
  //
  // Returns:     pointer to the newly created agent
  //
  ...
  //
  // ----------------------------------------------------------------------
  AgentHandle CreateCompoundAgent(int family, int genus, int species,
                                  FilePath const &gallery, int32 numimages,
                                  int32 baseimage, int32 plane);

  AgentHandle CreateVehicle(int family, int genus, int species,
                            FilePath const &gallery, int32 numimages,
                            int32 baseimage, int32 plane);
  ...
  void FindByAreaAndFGS(AgentList &agents, const Classifier &c, const Box &r);

  void FindBySightAndFGS(AgentHandle const &viewer, AgentList &agents,
                         const Classifier &c);
  ...
  //
  // ----------------------------------------------------------------------
  AgentHandle WhatAmITouching(Vector2D &ptThis, AgentHandle &me,
  ...
  AgentHandle WhatVehicleCouldIDropInto(const AgentHandle &me,
  ...
  // encountered 				fMood		- sum of
  // creature's mood 				fThreat   - sum of creature's
  // threat
  //
  // Returns:     the first creature you can hear
  //
  ...
  //
  // ----------------------------------------------------------------------
  void CalculateMoodAndThreat(uint32 &iSelectableCount, uint32 &iLowestHealth,
                              float &fMood, float &fThreat);

  static void KillAgent(const AgentHandle &agent);
  void UpdateAllAgents();
  void KillAllAgents();
  void ExecuteScriptOnAllAgents(int event, AgentHandle &from, const CAOSVar &p1,
                                const CAOSVar &p2);
  void ExecuteScriptOnAllAgentsDeferred(int event, AgentHandle &from,
  ...
  // Returns:     None
  //
  // Description: Updates a list for what smells are linked to which
  // classifiers.
  // ----------------------------------------------------------------------
  ...
  //
  // ----------------------------------------------------------------------
  static AgentMap &GetAgentIDMap() { return ourAgentMap; }
  ...
  int GetCreatureCount() { return ourCreatureCollection.size(); }
  ...
  //	This *should* be private, but for an annoying 'feature' of friend
  // declarations
