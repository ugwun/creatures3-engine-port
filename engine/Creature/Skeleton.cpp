...
inline IntegerPair PairUp(int a, int b) { return std::make_pair(a, b); }
...
bool Skeleton::MoveToSafePlaceGivenCurrentLocation() {
  ...
  bool ok = theApp.GetWorld().GetMap().FindSafeCreatureLocation(
      ...
      myCurrentDownFoot, myPermiability, downFootPosition);
  if (ok) {
...
bool Skeleton::MoveToSafePlace(const Vector2D &positionPreferred) {
  ...
  bool ok = theApp.GetWorld().GetMap().FindSafeCreatureLocation(
      ...
      myExtremes[BODY_LIMB_RIGHT_LEG] + delta, myMinY + delta.y,
      myCurrentDownFoot, myPermiability, downFootPosition);
...
bool Skeleton::TestMoveBy(float x, float y, bool forcePositionCheck) {
  ...
  if ((myMovementStatus == FLOATING) || (myMovementStatus == CARRIED))
    ...
    return theApp.GetWorld().GetMap().IsCreatureLocationValidInRectangle(
        footLeft, footRight, yMin, cabin.left, cabin.right, cabin.top,
        cabin.bottom);
  } else {
    return theApp.GetWorld().GetMap().IsCreatureLocationValidInRoomSystem(
...
float Skeleton::GetDistanceToObstacle(int direction) {
  ...
  if (myMovementStatus == AUTONOMOUS) {
    theApp.GetWorld().GetMap().TestForCreatureCollisionInRoomSystem(
        footLeft, footRight, myMinY, direction, myGeneralRange, myPermiability,
        collision, distanceCollision);
  } else if (myMovementStatus == INVEHICLE) {
    Box cabin;
    myCarrierAgent.GetVehicleReference().GetCabinExtent(cabin);
    theApp.GetWorld().GetMap().TestForCreatureCollisionInRectangle(
        footLeft, footRight, myMinY, direction, myGeneralRange, cabin.left,
        cabin.right, cabin.top, cabin.bottom, collision, distanceCollision);
  } else
    return FLT_MAX;
...
void Skeleton::HandleMovementWhenAutonomous() {
  ...
  theApp.GetWorld().GetMap().MoveCreatureInsideRoomSystem(
      myExtremes[BODY_LIMB_LEFT_LEG], myExtremes[BODY_LIMB_RIGHT_LEG], myMinY,
      applyPhysics, permiability, myCollisionFactor, myAeroDynamicFactor,
      myGravitationalAcceleration, myDownFootPosition, myVelocityVector,
      collision, wall, stopped, foot, velocityCollision);
...
void Skeleton::HandleMovementWhenInVehicle() {
  ...
  theApp.GetWorld().GetMap().MoveCreatureInsideRectangle(
      myExtremes[BODY_LIMB_LEFT_LEG], myExtremes[BODY_LIMB_RIGHT_LEG], myMinY,
      applyPhysics, myCollisionFactor, myAeroDynamicFactor,
      myGravitationalAcceleration, r.left, r.right, r.top, r.bottom,
      myDownFootPosition, myVelocityVector, collision, wall, stopped, foot,
      velocityCollision);
...
Skeleton::Skeleton()
    : myTintin(NULL), myCompositeBodyPartGallery(NULL),
...
Skeleton::Skeleton(AgentHandle &gene, int gene_index)
  ...
  try {
    Init(); // set defaults
    GetGenomeStore().MoveSlotFrom(0, gene.GetAgentReference().GetGenomeStore(),
                                  gene_index);
    ...
    myFailedConstructionException =
        "NLE0014: Unknown exception caught in skeleton constructor";
    return;
  }
  myAgentType = AgentHandle::agentNormal | AgentHandle::agentSkeleton;
}
...
void Skeleton::Init() {
  myReach = Vector2D((float)atof(theCatalogue.Get("Creature Reach", 0)),
                     (float)atof(theCatalogue.Get("Creature Reach", 1)));
...
void Skeleton::UpdatePositionsWithRespectToDownFoot() {
  ...
  myBody->SetPosition(
      Map::FastFloatToInteger(
          myDownFootPosition.x - OffsetX -
          myBody->GetBodyData()
              ->JoinX[Leg][myBody->GetView()]), // calc abs xy of
      Map::FastFloatToInteger(
          myDownFootPosition.y - OffsetY -
          myBody->GetBodyData()
              ->JoinY[Leg][myBody->GetView()])); // topleft of body
  ...
  for (i = 0; i < MAX_BODY_LIMBS; i++) { // for each limb chain
    OffsetX = myBody->GetWorldX() +
              myBody->GetBodyData()
                  ->JoinX[i][myBody->GetView()]; // start point of 1st
    OffsetY =
        myBody->GetWorldY() +
        myBody->GetBodyData()->JoinY[i][myBody->GetView()]; // limb in chain

    for (currentLimb = myLimbs[i]; // descend limb chain
         currentLimb != NULL; currentLimb = currentLimb->GetNextLimb()) {

      currentLimbData = currentLimb->GetLimbData(); // ptr to data fr limb type
      Lpv = currentLimb->GetView();                 // temp store fr View

      // Move limb to correct position.
      currentLimb->SetPosition(OffsetX - currentLimbData->StartX[Lpv],
  ...
  CreatureHead *head = (CreatureHead *)myLimbs[BODY_LIMB_HEAD];
  if (head)
    head->UpdatePosn(myCurrentDirection, myFacialExpression, myEyeState,
                     myEarSet);
  ...
  float ymax = -FLT_MAX;
  for (i = 0; i < 6; i++) {
    if (myExtremes[i].x < xmin)
      xmin = myExtremes[i].x;
    if (myExtremes[i].x > xmax)
      xmax = myExtremes[i].x;
    if (myExtremes[i].y < ymin)
      ymin = myExtremes[i].y;
    if (myExtremes[i].y > ymax)
      ymax = myExtremes[i].y;
  }
...
void Skeleton::UpdateFeet() {
  ...
  if (myMovementStatus == INVEHICLE) {
    ...
    theApp.GetWorld().GetMap().TestNewUpFootInRectangle(
        myDownFootPosition, myUpFootPosition, myMinX, myMaxX, r.left, r.right,
        r.top, r.bottom, myDownFootPosition, collision, wall, footChange);
  ...
  theApp.GetWorld().GetMap().TestNewUpFootInRoomSystem(
      myDownFootPosition, myPreviousUpFootPosition, myUpFootPosition, myMinX,
      myMaxX, myPreviousMinY, myMinY, myPermiability, directionRight,
      myDownFootPosition, gradientDownhill, gradientUphill, collision, wall,
      footChange, fall);
...
void Skeleton::GetExtremesForPose(char *pose,
                                  Vector2D extremes[MAX_BODY_LIMBS]) {
  int i;
...
void Skeleton::GeneratePlotOrder() {
  ...
  for (int bodyDirection = 0; bodyDirection < 4; bodyDirection++)
    for (int headDirection = 0; headDirection < 4; headDirection++) {
      ...
      myOrder.push_back(
          PairUp(PlaneOffset[bodyDirection][PART_HEAD], PART_HEAD));
      int currentBodyPart;

      for (currentBodyPart = PART_BODY; currentBodyPart < PART_LEFT_EAR;
           currentBodyPart++) {
        int currentPlane = PlaneOffset[bodyDirection][currentBodyPart];
        IntegerPairListIterator it;
        for (it = myOrder.begin(); it != myOrder.end(); it++) {
      ...
      for (currentBodyPart = PART_LEFT_EAR; currentBodyPart < NUMPARTS;
           currentBodyPart++) {
      ...
      for (currentBodyPart = PART_HEAD; currentBodyPart < NUMPARTS;
           currentBodyPart++) {
...
void Skeleton::UpdatePlotOrder() {
  ...
  // If facing right (east) the creature needs to plot the carried agent _just_
  // before the right humerus Otherwise _just_ after the right radius will do
  // nicely :)
  ...
  for (i = PART_HEAD; i < NUMPARTS; i++) {
    if (myCarriedAgent.IsValid())
      if (myCurrentDirection == EAST)
        if (myPlotOrders[myCurrentDirection][headDirection][i] ==
            myLimbs[4]) // right humerus
          myCarriedAgent.GetAgentReference().ChangePhysicalPlane(
              myBody->GetPlane());

    myPlotOrders[myCurrentDirection][headDirection][i]->SetPlane(
        myBody->GetPlane());
    myPlotOrders[myCurrentDirection][headDirection][i]->YouAreCameraShy(
        myEntitiesAreCameraShy);
    theMainView.UpdatePlane(myPlotOrders[myCurrentDirection][headDirection][i]);

    if (myCarriedAgent.IsValid())
      if (myCurrentDirection != EAST)
        if (myPlotOrders[myCurrentDirection][headDirection][i] ==
            myLimbs[4]->GetNextLimb()) // right radius
          myCarriedAgent.GetAgentReference().ChangePhysicalPlane(
              myBody->GetPlane());
  }
}
...
void Skeleton::SetCurrentPoseString(char *pose) {
  ...
  static int Index[MAX_BODY_LIMBS] = {_HEAD,     _LTHIGH,   _RTHIGH,
                                      _LHUMERUS, _RHUMERUS, _TAILROOT};
  int i, j;
  Limb *currentLimb;
  int p;
  ...
  CreatureHead *head = (CreatureHead *)myLimbs[BODY_LIMB_HEAD];
  head->SetCurrentPoseString(myCurrentDirection, myFacialExpression, myEyeState,
                             myEarSet);
...
inline char Skeleton::HeadAngle() {
  ...
  x = Map::FastFloatToInteger(
      fabsf(myItPosition.x - myDownFootPosition.x)); // dist to target
  ...
  y = Map::FastFloatToInteger((myItPosition.y - myBody->GetWorldY()) /
                                  TILTGRIDY +
                              (TILTY / 2)); // y posn in table (eye level=3)
  if (y < 0)
    y = 0;
  if (y >= TILTY)
    y = TILTY - 1;
...
// Move one more step towards myTargetPoseString (if necessary) & redraw whole
// obj If a head angle of '?' is defined, Skeleton will automatically look up or
// down as required to face IT. If a Direction of '?' is defined, Skeleton will
// incrmentally turn head then body towards IT. A dirn of '!' means turn to face
// AWAY from IT.
void Skeleton::MoveTowardsTargetPoseString() {
  ...
  if ((Pose[_DIRN] != 'X') && // if already facing that way
    ...
    NextDirn = Next[Status][Pose[_DIRN] - '0']; // get new dirn & head angle
    ...
    if ((ToIt) && (Pose[_DIRN] == '1')) { // if table said south
      if ((myIt.IsValid()) && myIt.GetAgentReference().GetPlane() <
                                  GetPlane()) // & trying to face an IT
  ...
  // run through pose[] string, comparing it with myCurrentPoseString[], and
  // build a 'next step' string to use for calling SetPose(). For every limb
  // that moves, add one to the myMusclesLocus emitter locus. The biochemistry
  // can use this value to determine how much energy is being expended
...
// This function may be called repeatedly for macro commands like POSE and REAC,
// where the command must start the Skeleton shifting towards a given pose, and
// repeat the action until that pose has been reached.
bool Skeleton::SetTargetPoseString(char *pose) {
  _ASSERT(!myGarbaged);

  // compare given pose to current myTargetPoseString. If they are the same,
  // then either the target has already been reached, or the shift is in
  // progress. If they are different, then the new target should be submitted
...
void Skeleton::Update() {
  ...
  */
  ...
  if (!myStoppedFlag || (myMovementStatus == CARRIED) ||
    ...
    // Update the creature's expression
    CreatureHead *head = (CreatureHead *)myLimbs[BODY_LIMB_HEAD];
    head->SetCurrentPoseString(myCurrentDirection, myFacialExpression,
                               myEyeState, myEarSet);
    // Don't walk
    return;
  }
  ...
  // move another step towards current target pose. If already there, then this
  // will merely update gaze and blinking
  MoveTowardsTargetPoseString();

  // update any animation if the target has been reached
  if ((*myAnimationPointer) &&
      (HasTargetPoseStringBeenReached())) { // if reached target and an anim is
                                            // active

    if (!myStandStill)
      myAnimationPointer += 3; // move on to next target pose

    ASSERT(myAnimationPointer - myAnimationString <=
           MAX_SKELETON_ANIMATION_STRING_LENGTH);
    ...
    if (*myAnimationPointer) // if not reached end
    {
      ...
      int poseID = ((myAnimationPointer[0] - '0') * 100) +
                   ((myAnimationPointer[1] - '0') * 10) +
                   (myAnimationPointer[2] - '0');
...
void Skeleton::SetAnimationString(char *animation) {
  ...
  strcpy(myTargetPoseString,
         myPoseStringTable[poseID]); // define a new target pose
}

// Helper function for ANIM macro
bool Skeleton::SetAnim(const uint8 *anim, int length,
                       int part) // helper fn for ANIM macro
{
...
// helper function for POSE macro. Return TRUE when desired pose has been
// reached
bool Skeleton::ShowPose(int pose, int part) {
  ...
  if (pose >= 0 && pose < MAX_POSES) {
    ...
    SetTargetPoseString(
        myPoseStringTable[pose]); // start new pose shift if necc
    return true;
  } else
    return false;
}
...
// 0 if still moving, or -1 if failed (no IT obj defined, or IT is below floor
// level) Incremental function - call every clock-tick 27/4/96: Parameter added
// to enable creature to "point out" an object, instead of "reach out and touch
// it", using the same pose tables. The pose is the same except the creature
// faces the camera not the object. This can be used so a creature can ask the
// user what an object is called, for example.
int Skeleton::ReachOut(
    BOOL Point /*=FALSE*/) // TRUE if should point to obj, not touch it
{
  _ASSERT(!myGarbaged);
  ...
  if (myIt.GetAgentReference().GetCarrier() ==
      mySelf) // If I'm already carrying it don't start a
  ...
  // then move a bit more towards that dirn & pose. Return 1 if pose has been
  // reached
  ResetAnimationString(); // stop any existing animation

  char *pose =
      (char *)&(myPoseStringTable[POSE_TOUCH + GridX * REACHGRIDYPOSES + GridY]
                                 [0]); // pose

  if (Point) { // if I'm to point it out not touch it
    char pointpose[MAX_POSE_STRING_LENGTH +
                   1];       // then make a copy of the touch pose
    strcpy(pointpose, pose); // and change its head angle to
    pointpose[_HEAD] = '4';  // face camera instead of object
    return (SetTargetPoseString(pointpose)) ? 1
                                            : 0; // and move towards that pose
  }

  return (SetTargetPoseString(pose))
             ? 1
             : 0; // else, just use pose straight from table
}
...
Vector2D Skeleton::GetMapCoordOfWhereIPickUpAgentsFrom(int pose,
                                                       bool bRelative) {
  _ASSERT(!myGarbaged);
...
Each APPEARANCE gene determines the visual variant of a single body part (or
pair of parts in the case of legs etc.). To ensure that these genes do not get
inherited from a single parent, it is important to spread them evenly throughout
the genome. Doing so will vastly increase the likelihood that crossing- over
will extract some of mum's appearance genes, and some of dad's. Incidentally,
where possible I should place APPEARANCE genes near to any INSTINCT or other
trait genes that might reasonably be expected to be linked - eg. if facial
APPEARANCE and mood-related genes are close to each other, then I can associate
grumpy face types with grumpy personality, with a reasonable likelihood of them
remaining linked.
...
where	P = part# ("A" to "Z"),
                G = genus+sex*4, i.e. 0=male norn, 1=male grendel, 4=female
norn... A = age ("0" to "9"), V = variant ("0" to "9"). Eg. A012.ATT is the
attachment table for variant 2 of the head ("A") of a male norn ("0") aged '1'.
If the explicit file cannot be found, then the system defaults by searching
for:- First: a younger age Second: an earlier variant of the same or younger age
...
// Helper for skeleton constructor. Read an attachment table file and return a
// pointer to a copyable BodyData struct containing the attachment data that
// needs to be copied to a Body constructor. The file is an exact binary list of
// items in the order that would be used for initialisation of a BodyData
// struct.
...
//	 containing all the correct sprites for your body's appearance, species
//& sex
...
bool Skeleton::CreateSkeleton(Genome &genome, int Age) {
  // Now that I know this, I can set up creature's Classifier in the following
  // format:-
  // - Family  = CREATURE,
  // - Genus   = 1 to 4 (norn, grendel, ettin, side),
  // - Species = used to represent sex - 1=male or 2=female (allows sex-specific
  // scripts)
  ...
  if (!myCompositeBodyPartGallery) {
    CAOSVar &sprite =
        theApp.GetWorld().GetGameVar("engine_creature_template_size_in_mb");
    ...
    myCompositeBodyPartGallery =
        new CreatureGallery(sprite.GetInteger(), galleryName);
...
bool Skeleton::ReloadSkeleton(Genome &genome, int Age) {
  _ASSERT(!myGarbaged);
  ...
  // work out how many actual parts are needed for this creature, ie. don't
  // count optional tail if it has not been genetically specified
  ...
  if (myCompositeBodyPartGallery->IsComplete()) {
    ...
    if (SharedGallery::theSharedGallery().CreateGallery(
            galleryName, GetUniqueID(), myGenera, myVariants, ValidParts, Sex,
            Age, myBodySprites, myCompositeBodyPartGallery,
            myIndividualBodyPartGalleries, myNumSpritesInFile,
  ...
  if (ok) {
    ...
    myBody->Reload(GetBodyData(PART_BODY, myGenera[PART_BODY], Sex, Age,
                               myVariants[PART_BODY]),
                   name, myBodySprites[PART_BODY], Age);
    ...
    ((CreatureHead *)(myLimbs[BODY_LIMB_HEAD]))
        ->Reload(Age, name, GetLimbDataPtr, myBodySprites);
    ///////////////////////////////////////////////////////////////////////////////
    // right leg
    ///////////////////////////////////////////////////////////////////////////////
    ...
    for (int32 i = BODY_LIMB_LEFT_LEG; i < MAX_BODY_LIMBS; i++) {
      ...
      myLimbs[i]->Reload(partNumber, GetLimbDataPtr, name, myBodySprites, 0,
                         Age);
    }
...
// Kill body & limb entities (during destruction or prior to redefining
// appearance).
...
int Skeleton::GetOverlayIndex(int bodypart) {
  ...
  case PART_LFOOT: {
    _ASSERT(myLimbs[BODY_LIMB_LEFT_LEG]);
    index = myLimbs[BODY_LIMB_LEFT_LEG]
                ->GetNextLimb()
                ->GetNextLimb()
                ->GetCurrentIndex();
    break;
  ...
  case PART_RFOOT: {
    _ASSERT(myLimbs[BODY_LIMB_RIGHT_LEG]);
    index = myLimbs[BODY_LIMB_RIGHT_LEG]
                ->GetNextLimb()
                ->GetNextLimb()
                ->GetCurrentIndex();
    break;
  ...
  case PART_LEFT_EAR: {
    return ((CreatureHead *)(myLimbs[BODY_LIMB_HEAD]))
        ->GetOverlayIndex(HEAD_LIMB_LEFT_EAR);
  }
  case PART_RIGHT_EAR: {
    return ((CreatureHead *)(myLimbs[BODY_LIMB_HEAD]))
        ->GetOverlayIndex(HEAD_LIMB_RIGHT_EAR);
  }
  case PART_HAIR: {
    return ((CreatureHead *)(myLimbs[BODY_LIMB_HEAD]))
        ->GetOverlayIndex(HEAD_LIMB_HAIR);
  }
  default:
    return index;
  }
...
int Skeleton::GetClothingSet(int bodypart, int layer) {
  ...
  case PART_LFOOT: {
    _ASSERT(myLimbs[BODY_LIMB_LEFT_LEG]);
    index =
        myLimbs[BODY_LIMB_LEFT_LEG]->GetNextLimb()->GetNextLimb()->GetOverlay(
            layer);
    break;
  }
  case PART_RTHIGH: {
  ...
  case PART_RFOOT: {
    _ASSERT(myLimbs[BODY_LIMB_RIGHT_LEG]);
    index =
        myLimbs[BODY_LIMB_RIGHT_LEG]->GetNextLimb()->GetNextLimb()->GetOverlay(
            layer);
    break;
  }
  case PART_LHUMERUS: {
  ...
  case PART_LEFT_EAR: {
    return ((CreatureHead *)(myLimbs[BODY_LIMB_HEAD]))
        ->GetClothingSet(HEAD_LIMB_LEFT_EAR, layer);
  }
  case PART_RIGHT_EAR: {
    return ((CreatureHead *)(myLimbs[BODY_LIMB_HEAD]))
        ->GetClothingSet(HEAD_LIMB_RIGHT_EAR, layer);
    ;
  }
  case PART_HAIR: {
    return ((CreatureHead *)(myLimbs[BODY_LIMB_HEAD]))
        ->GetClothingSet(HEAD_LIMB_HAIR, layer);
    ;
  }
  default:
    return index;
...
bool Skeleton::WearOutfit(int set, int layer) {
  ...
  _ASSERT(myLimbs[BODY_LIMB_LEFT_LEG]);
  myLimbs[BODY_LIMB_LEFT_LEG]->GetNextLimb()->GetNextLimb()->SetClothing(set,
                                                                         layer);
  ...
  _ASSERT(myLimbs[BODY_LIMB_RIGHT_LEG]);
  myLimbs[BODY_LIMB_RIGHT_LEG]->GetNextLimb()->GetNextLimb()->SetClothing(
      set, layer);
  ...
  ((CreatureHead *)(myLimbs[BODY_LIMB_HEAD]))
      ->WearItem(HEAD_LIMB_LEFT_EAR, set, layer);

  ((CreatureHead *)(myLimbs[BODY_LIMB_HEAD]))
      ->WearItem(HEAD_LIMB_RIGHT_EAR, set, layer);

  ((CreatureHead *)(myLimbs[BODY_LIMB_HEAD]))
      ->WearItem(HEAD_LIMB_HAIR, set, layer);
  return true;
}
...
bool Skeleton::WearItem(int bodypart, int set, int layer) {
  ...
  case PART_LFOOT: {
    _ASSERT(myLimbs[BODY_LIMB_LEFT_LEG]);
    myLimbs[BODY_LIMB_LEFT_LEG]->GetNextLimb()->GetNextLimb()->SetClothing(
        set, layer);
    break;
  }
  case PART_RTHIGH: {
...
bool Skeleton::HitTest(const Vector2D &point) {
  _ASSERT(!myGarbaged);
  Box rect;
  GetAgentExtent(rect);
...
void Skeleton::GetBodyPartFileName(int bodypart, std::string &path) {
  ...
  if (bodypart < MAX_BODY_LIMBS) {
    ...
    ((CreatureHead *)(myLimbs[BODY_LIMB_HEAD]))
        ->GetHeadPartFileName(bodypart, path);
  }
}
...
// virtual
void Skeleton::DrawLine(int32 x1, int32 y1, int32 x2, int32 y2,
  ...
  if (myBody)
    myBody->DrawLine(x1, y1, x2, y2, lineColourRed, lineColourGreen,
                     lineColourBlue, stippleon, stippleoff, stipplestartAt);
}
...
// Helper function for APPR & WALK macros. Set up the best animation for
// walking, by looking at the myGaitLoci[] receptors. If none active, use normal
// gait
void Skeleton::Walk() {
  _ASSERT(!myGarbaged);
  ...
  SetAnimationString(
      myGaitTable[Choice]); // use either strongest receptor's gait, or default
}
...
bool Skeleton::ExpressGenes(const Genome &g, int age) {
  ...
  try {
    ...
    g2.Reset();
    while ((g2.GetGeneType(CREATUREGENE, G_POSE, NUMCREATURESUBTYPES)) !=
           false) {
    ...
    while ((g2.GetGeneType(CREATUREGENE, G_GAIT, NUMCREATURESUBTYPES)) !=
      ...
      for (i = 0; i < 8; i++) {
        c = g2.GetCodon(0,
                        MAX_POSES - 1); // get up to eight pose numbers (0=end)
      ...
      // note: pose# 0 can signify 'end' cos pose0 is a reach-out pose not a
      // walking one
    }
...
bool Skeleton::CannotBeMuchCloser() {
  ...
  return
      // can reach equally either side but we stop slightly closer than we can
      // reach just to be sure.
      fabsf(myDownFootPosition.x - myItPosition.x) <= (GetReachX() * 0.7f);
}
...
bool Skeleton::Write(CreaturesArchive &archive) const {
  ...
  archive.Write(myTargetPoseString,
                MAX_POSE_STRING_LENGTH +
                    1); // Pose that UpdatePose() is currently striving towards

  archive.Write(
      myAnimationString,
      MAX_SKELETON_ANIMATION_STRING_LENGTH); // current animation string
  ...
  for (i = 0; i < MAX_GAITS; ++i)
    archive.WriteFloatRefTarget(
        myGaitLoci[i]); // trigger different walking gaits ([0]=default)

  // to do with IT (attended-to object)...

  archive << myIt;                // object of this creature's attention
                                  // NULL if none
  archive << myIntrospectiveFlag; // for extraspective actions like GO EAST and
                                  // GO WEST
...
bool Skeleton::Read(CreaturesArchive &archive) {
  ...
  if (version >= 3) {
    ...
    archive.Read(myTargetPoseString,
                 MAX_POSE_STRING_LENGTH +
                     1); // Pose that UpdatePose() is currently striving towards
    ...
    archive >> myIt;                // object of this creature's attention
                                    // NULL if none
    archive >> myIntrospectiveFlag; // for extraspective actions like GO EAST
                                    // and GO WEST
    archive >> myItPosition;        // Last known xy of target to touch
                                    // when trying to Activate It etc.
                                    // Updated by SlowUpdate()
...
bool Skeleton::Visibility(int scope) {
  ...
  for (int i = 0; i < MAX_BODY_LIMBS; i++) { // for each limb chain
...
void Skeleton::ReadAppearanceGenes(Genome &genome, TintManager &tinty) {
  ...
  // Which body parts have their variant data set by which Appearance gene (eg.
  // REGION_LEGS must determine the variant#s of both thighs, shins & feet. Up
  // to 6 parts may be set for each region - entries are part#s or -1 if unused.
  static int RegionParts[NUMREGIONS][6] = {
      ...
      {PART_LFOOT, PART_LSHIN, PART_LTHIGH, PART_RFOOT, PART_RSHIN,
       PART_RTHIGH}, // REGION_LEGS
      {PART_LRADIUS, PART_LHUMERUS, PART_RRADIUS, PART_RHUMERUS, -1,
       -1},                                          // REGION_ARMS
  ...
  genome.Reset();
  while ((genome.GetGeneType(CREATUREGENE, G_GENUS, NUMCREATURESUBTYPES,
                             SWITCH_AGE)) != FALSE) {
    ...
    // (re-)read the APPEARANCE genes to determine which visual variant of any
    // body part to use and build an array showing which to use for each
    // individual part
    genome.Reset();
    while ((genome.GetGeneType(CREATUREGENE, G_APPEARANCE, NUMCREATURESUBTYPES,
                               SWITCH_AGE)) != FALSE) {
      ...
      vgenus = genome.GetCodon(
          0, NUMCREATUREGENI); // genus of creature the variant applies to.

      for (i = 0; (i < 6) && (RegionParts[region][i] != -1);
           i++) // for each valid entry in LUT
      {
        myVariants[RegionParts[region][i]] =
            variant; // set that part to correct variant
        myGenera[RegionParts[region][i]] = vgenus;
      }

      if (region == REGION_TAIL)
        HasTail++;
      if (region == REGION_HAIR)
        HasHair++; //
    };
  ...
  // Set up colour table for re-colouring sprites in response to any pigment
  // genes

  int pig_total[3], pig_count[3];
  int Pigment[3];
  pig_total[0] = pig_total[1] = pig_total[2] =
      0; // default pigmentation is mid-strength
  ...
  genome.Reset();
  while ((genome.GetGeneType(CREATUREGENE, G_PIGMENT, NUMCREATURESUBTYPES,
                             SWITCH_AGE)) != FALSE) {
    int pig = genome.GetCodon(0, 2); // pigment being defined (r,g,b)
    pig_total[pig] +=
        genome.GetCodon(0, 255); // intensity (0=faint 128=normal 255=strong)
    pig_count[pig]++;
  }

  genome.Reset();
  while ((genome.GetGeneType(CREATUREGENE, G_PIGMENTBLEED, NUMCREATURESUBTYPES,
                             SWITCH_AGE)) != FALSE) {
  ...
  tinty.BuildTintTable(Pigment[0], Pigment[1], Pigment[2], ColourRotation,
                       ColourSwap);
...
bool Skeleton::CreateBodyPart(int sex, int age, TintManager &tinty) {
  if (myBodyPartToBeCreated < NUMPARTS) {
    ...
    if (!myCompositeBodyPartGallery->IsComplete()) {
      ...
      gallery = SharedGallery::theSharedGallery().CreateGallery(
          galleryName, GetUniqueID(), myGenera, myVariants, NUMPARTS, sex, age,
          myBodySprites, myCompositeBodyPartGallery,
          myIndividualBodyPartGalleries, myNumSpritesInFile);
    }
    ...
    if (gallery) {
      ...
      CAOSVar &sprite =
          theApp.GetWorld().GetGameVar("engine_mirror_creature_body_parts");
      _ASSERT(sprite.GetType() == CAOSVar::typeInteger);
      bool mirror = false;
      ...
      myBody = new Body(
          ...
          GetBodyData(PART_BODY, myGenera[PART_BODY], sex, age,
                      myVariants[PART_BODY]),
          name, myBodySprites[PART_BODY],
          myNumSpritesInFile, // base sprite
          myNormalPlane, PART_BODY, myGenera[PART_BODY], sex, age,
          myVariants[PART_BODY], PART_BODY, mirror);
      ...
      head = new CreatureHead(
          ...
          NULL, name, myBodySprites, myNumSpritesInFile, PART_HEAD,
          myGenera[PART_HEAD], sex, age, myVariants[PART_HEAD], GetLimbDataPtr,
          myBodySprites[PART_HEAD + 1], PART_HEAD, mirror);
      ...
      Tip = new Limb(NULL,
                     GetLimbData(PART_RFOOT, myGenera[PART_RFOOT], sex, age,
                                 myVariants[PART_RFOOT]),
                     name, myBodySprites[PART_RFOOT], myNumSpritesInFile,
                     PART_RFOOT, myGenera[PART_RFOOT], sex, age,
                     myVariants[PART_RFOOT], PART_RFOOT, mirror);

      Mid = new Limb(Tip,
                     GetLimbData(PART_RSHIN, myGenera[PART_RSHIN], sex, age,
                                 myVariants[PART_RSHIN]),
                     name, myBodySprites[PART_RSHIN], myNumSpritesInFile,
                     PART_RSHIN, myGenera[PART_RSHIN], sex, age,
                     myVariants[PART_RSHIN], PART_RSHIN, mirror);

      Root = new Limb(Mid,
                      GetLimbData(PART_RTHIGH, myGenera[PART_RTHIGH], sex, age,
                                  myVariants[PART_RTHIGH]),
                      name, myBodySprites[PART_RTHIGH], myNumSpritesInFile,
                      PART_RTHIGH, myGenera[PART_RTHIGH], sex, age,
                      myVariants[PART_RTHIGH], PART_RTHIGH, mirror);
      ...
      Tip = new Limb(NULL,
                     GetLimbData(PART_LFOOT, myGenera[PART_LFOOT], sex, age,
                                 myVariants[PART_LFOOT]),
                     name, myBodySprites[PART_LFOOT], myNumSpritesInFile,
                     PART_LFOOT, myGenera[PART_LFOOT], sex, age,
                     myVariants[PART_LFOOT], PART_LFOOT, mirror);

      Mid = new Limb(Tip,
                     GetLimbData(PART_LSHIN, myGenera[PART_LSHIN], sex, age,
                                 myVariants[PART_LSHIN]),
                     name, myBodySprites[PART_LSHIN], myNumSpritesInFile,
                     PART_LSHIN, myGenera[PART_LSHIN], sex, age,
                     myVariants[PART_LSHIN], PART_LSHIN, mirror);

      Root = new Limb(Mid,
                      GetLimbData(PART_LTHIGH, myGenera[PART_LTHIGH], sex, age,
                                  myVariants[PART_LTHIGH]),
                      name, myBodySprites[PART_LTHIGH], myNumSpritesInFile,
                      PART_LTHIGH, myGenera[PART_LTHIGH], sex, age,
                      myVariants[PART_LTHIGH], PART_LTHIGH, mirror);
      ...
      //////////////////////////////////////////////////////////////////////////////
      Tip = new Limb(NULL,
                     GetLimbData(PART_LRADIUS, myGenera[PART_LRADIUS], sex, age,
                                 myVariants[PART_LRADIUS]),
                     name, myBodySprites[PART_LRADIUS], myNumSpritesInFile,
                     PART_LRADIUS, myGenera[PART_LRADIUS], sex, age,
                     myVariants[PART_LRADIUS], PART_LRADIUS, mirror);

      Root = new Limb(Tip,
                      GetLimbData(PART_LHUMERUS, myGenera[PART_LHUMERUS], sex,
                                  age, myVariants[PART_LHUMERUS]),
                      name, myBodySprites[PART_LHUMERUS], myNumSpritesInFile,
                      PART_LHUMERUS, myGenera[PART_LHUMERUS], sex, age,
                      myVariants[PART_LHUMERUS], PART_LHUMERUS, mirror);
      ...
      Tip = new Limb(NULL,
                     GetLimbData(PART_RRADIUS, myGenera[PART_RRADIUS], sex, age,
                                 myVariants[PART_RRADIUS]),
                     name, myBodySprites[PART_RRADIUS], myNumSpritesInFile,
                     PART_RRADIUS, myGenera[PART_RRADIUS], sex, age,
                     myVariants[PART_RRADIUS], PART_RRADIUS, mirror);

      Root = new Limb(Tip,
                      GetLimbData(PART_RHUMERUS, myGenera[PART_RHUMERUS], sex,
                                  age, myVariants[PART_RHUMERUS]),
                      name, myBodySprites[PART_RHUMERUS], myNumSpritesInFile,
                      PART_RHUMERUS, myGenera[PART_RHUMERUS], sex, age,
                      myVariants[PART_RHUMERUS], PART_RHUMERUS, mirror);

      AttachLimbChain(BODY_LIMB_RIGHT_ARM, Root);

      Tip = new Limb(NULL,
                     GetLimbData(PART_TAILTIP, myGenera[PART_TAILTIP], sex, age,
                                 myVariants[PART_TAILTIP]),
                     name, myBodySprites[PART_TAILTIP], myNumSpritesInFile,
                     PART_TAILTIP, myGenera[PART_TAILTIP], sex, age,
                     myVariants[PART_TAILTIP], PART_TAILTIP, mirror);

      Root = new Limb(Tip,
                      GetLimbData(PART_TAILROOT, myGenera[PART_TAILROOT], sex,
                                  age, myVariants[PART_TAILROOT]),
                      name, myBodySprites[PART_TAILROOT], myNumSpritesInFile,
                      PART_TAILROOT, myGenera[PART_TAILROOT], sex, age,
                      myVariants[PART_TAILROOT], PART_TAILROOT, mirror);
...
bool Skeleton::BuildGalleries(int sex, int age, uint32 &currentPart,
                              TintManager &tintTable) {
  uint32 fsp = 0;
  // bypass the shared gallery for this
  // because we will delete it ourselves in a moment

  fsp = ValidFsp(currentPart, myGenera[currentPart], sex, age,
                 myVariants[currentPart], "C16", IMAGES_DIR);
  if (fsp) {
    ...
    try {
      if (galleryName != myIndividualBodyPartGalleries[currentPart].GetName()) {
        ...
        myIndividualBodyPartGalleries[currentPart].Recolour(
            tintTable.GetMyTintTable());
      }
      myNumSpritesInFile +=
          myIndividualBodyPartGalleries[currentPart].GetCount();
    ...
    // version
    fsp = ValidFsp(currentPart, 0, sex, age, 0, "C16", IMAGES_DIR);
...
bool Skeleton::PreloadBodyPartsForNextAgeStage(Genome &genome, int sex) {
  ...
  if (myBodyPartToBeCreatedForNextAgeStage < NUMPARTS) {
    return BuildGalleries(sex, age, myBodyPartToBeCreatedForNextAgeStage,
                          *myTintin);
  } else {
    Limb *Root = NULL;
    ...
    if (!myCompositeBodyPartGalleryForNextAgeStage->IsComplete()) {
      ...
      SharedGallery::theSharedGallery().CreateGallery(
          galleryName, GetUniqueID(), myGenera, myVariants, NUMPARTS, sex, age,
          myBodySprites, myCompositeBodyPartGalleryForNextAgeStage,
          myIndividualBodyPartGalleries, myNumSpritesInFile);
    }
...
bool Skeleton::PrepareForAgeing(int age) {
  if (!myCompositeBodyPartGalleryForNextAgeStage) {
    ...
    OutputFormattedDebugString(
        "PrepareForAgeing: age=%d template_size_in_mb=%d\n", age,
        sprite.GetInteger());
