...
{
	uint32 set=mySets[0][a] & mySets[1][b] & mySets[2][c];
...
void Voice::GetSound(char a, char b, char c, uint32 &sound, int &time)
{
	char group[3];
...
bool Voice::ReadData(std::string& thisVoice)
{
	...
	for(i=0;i<32;i++)
	{
		mySounds[i] = *((uint32*)theCatalogue.Get( thisVoice, 1 + (i*2) ));
...
/*
void Voice::ReadData(const char* name)
{
	File file(name);
	
	for (int i=0;i<32;i++) {
		file.Read(&mySounds[i],sizeof(uint32));
	...
	for (i=0; i<3; i++) {
		for (int j=0; j<27; j++) {
			file.Read(&mySets[i][j],sizeof(uint32));
		}
	}
}
*/
