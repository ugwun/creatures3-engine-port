...
int GetCreatureCount() { return theCreatureCount; }
...
void Creature::HandlePickup(Message *Msg) {
  ...
  if (Msg->GetFrom().IsPointerAgent()) {
    ...
    if ((theApp.GetCreaturePickupStatus() == 1) ||
        ((theApp.GetCreaturePickupStatus() == 2) &&
         (!theApp.GetInputManager().IsKeyDown(VK_SHIFT))) ||
        ((theApp.GetCreaturePickupStatus() == 3) &&
         ((!theApp.GetInputManager().IsKeyDown(VK_SHIFT)) ||
          !Music()->SelectableByUser()))) {
...
bool Creature::CanSee(const AgentHandle &other) {
  ...
  // The loop is to get the creature and the agent's base vehicle, e.g. the root
  // vehicle of a creature on a hoverboard (an open air cabin) is NULLHANDLE,
  // i.e. the world, whereas the root vehicle of a creature in a lift is just
  // the lift.

  // Get the creature's real vehicle (ignoring open air cabins) - using
  // NULLHANDLE to mean the world:
  AgentHandle creaturesEnclosingVehicle = AgentHandle(this);
  do {
    creaturesEnclosingVehicle =
        myMovementStatus == INVEHICLE
            ? creaturesEnclosingVehicle.GetAgentReference().GetCarrier()
            : NULLHANDLE;
  } while (creaturesEnclosingVehicle.IsValid() &&
           creaturesEnclosingVehicle.GetAgentReference().TestAttributes(
               attrOpenAirCabin));

  // Get the agent's real vehicle (ignoring open air cabins) - using NULLHANDLE
  // to mean the world:
  AgentHandle agentsEnclosingVehicle = other;
  do {
    agentsEnclosingVehicle =
        myMovementStatus == INVEHICLE
            ? agentsEnclosingVehicle.GetAgentReference().GetCarrier()
            : NULLHANDLE;
  } while (agentsEnclosingVehicle.IsValid() &&
           agentsEnclosingVehicle.GetAgentReference().TestAttributes(
               attrOpenAirCabin));
...
int Creature::ClickAction(int x, int y) {
  ...
  // stop right here
  if ((myDefaultClickAction == START_HOLD_HANDS) ||
...
// -------------------------------------------------------------------------
// Creature::Update()
// *Probably* the main tick for a living creature.
// It handles life-cycle growth loops (aging, loading body parts), updates
// all internal Faculties (Brain, Sensory, Biochemistry), and checks for
// environmental conditions (like being underwater or in crowded rooms).
// -------------------------------------------------------------------------
void Creature::Update() {
  ...
  if (myStoppedFlag) {
    ...
    if (ContinueLoadingDataForNextAgeStage()) {
      if (Life()->GetAge() <= NUMAGES) {
        Genome g(GetGenomeStore(), 0, Life()->GetSex(), myAgeAlreadyLoaded,
                 Life()->GetVariant());
  ...
  if (!Life()->GetWhetherDead()) {
    ...
    if (theApp.GetWorld().GetMap().GetRoomIDForPoint(
            myLimbs[BODY_LIMB_HEAD]->CentrePoint(), roomId)) {
      int roomType;
    ...
    myCrowdedLocus = 0.0f;
    float value;
    if (theApp.GetWorld().GetMap().GetRoomPropertyMinusMyContribution(
            AgentHandle(*this), value))
...
void Creature::RemakeSkeletonAfterSerialisation() {
  Genome g(GetGenomeStore(), 0, Life()->GetSex(), Life()->GetAge(),
           Life()->GetVariant());
...
Creature::Creature() : Skeleton() {
  ...
  myAgentType = AgentHandle::agentNormal | AgentHandle::agentSkeleton |
                AgentHandle::agentCreature;
}

// Destructor.
Creature::~Creature() { --theCreatureCount; }

// 'Birth' constructor.
// Called during "new: crea <moniker> <sex>" macro to create a new creature.
Creature::Creature(
    int family, uint32 id, AgentHandle gene, int gene_index, // your genome
    ...
    : Skeleton(gene, gene_index) // construct skeleton
{
  ...
  try {
    ...
    myFaculties[5] =
        (Faculty *)(myReproductiveFaculty = new ReproductiveFaculty());
    ...
    if (!ExpressGenes())
      myFailedConstructionException = ErrorMessageHandler::Format(
          "creature_error", 0, "Creature::Creature", GetMoniker().c_str());
    ...
    myFailedConstructionException =
        "NLE0012: Unknown exception caught in creature constructor";
  }
}
...
void Creature::HoldHandsWithThePointer() {
  ...
  theApp.GetWorld().GetMap().ShiftCreatureAlongFloor(
      ...
      myPermiability, delta, downFootNew, collision, wall, footChange, fall);
  ...
  if (collision) {
    myLastWallHit = wall;
    ExecuteScriptForEvent(SCRIPTBUMP, mySelf, INTEGERZERO, INTEGERZERO);
  }
}

void Creature::StartHoldingHandsWithThePointer(const AgentHandle &from,
                                               const CAOSVar &p1,
                                               const CAOSVar &p2) {
  if (myIsHoldingHandsWithThePointer)
    return;

  if (!Life()->GetWhetherAlert() || Life()->GetWhetherZombie() ||
      (myMovementStatus == INVEHICLE))
    return;
...
void Creature::StopHoldingHandsWithThePointer(const AgentHandle &from,
                                              const CAOSVar &p1,
                                              const CAOSVar &p2) {
  ...
  _ASSERT(mySelf ==
          thePointer.GetPointerAgentReference().GetHandHeldCreature());
...
// respond to an enquiry by a biochemical Receptor/Emitter object for its locus
// address
float *
Creature::GetLocusAddress(int type, // ID relates to RECEPTOR or EMITTER locus?
                          int organ,
  ...
  for (int i = 0; i < noOfFaculties; i++) {
    float *locusPointer =
        myFaculties[i]->GetLocusAddress(type, organ, tissue, locus);
    if (locusPointer)
      return locusPointer;
  }
  ...
  if (organ == ORGAN_CREATURE) {
    if (type == RECEPTOR) {
      ...
      switch (tissue) { // according to tissue...
      case TISSUE_SOMATIC:
...
bool Creature::FormBodyParts() {
  if (!Skeleton::ExpressGenes(Genome(GetGenomeStore(), 0, Life()->GetSex(),
                                     Life()->GetAge(), Life()->GetVariant()),
                              Life()->GetAge()))
    return false;
  return true;
}
