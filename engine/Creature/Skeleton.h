...
// Skeleton class. Derived from Agent. Virtual parent for Creature class, which
// adds brain, genetics, biochemistry and senses to the Skeleton
...
const int MAX_GAITS =
    16; // number of possible walking gaits (including default gait)
const int MAX_SKELETON_ANIMATION_STRING_LENGTH =
    ((30 * 3) + 2);                    // 30 Poses + 'R' + '\0';
...
protected:
  ...
  // which sprite# in final sprite file refers to the first pose of each body
  // part Filled in while copying sprites from library files. Used to construct
  // Body & Limbs
  ...
  char myCurrentPoseString[MAX_POSE_STRING_LENGTH +
                           1]; // Pose$ representing current pose, used by
                               // UpdatePose()
  char myTargetPoseString[MAX_POSE_STRING_LENGTH +
                          1]; // Pose that UpdatePose() is currently striving
                              // towards

  char myAnimationString
  ...
  char myPoseStringTable[MAX_POSES]
                        [MAX_POSE_STRING_LENGTH +
                         1]; // array of poses fr actions & reaching out
  char
      myGaitTable[MAX_GAITS] // array of walking animations ("010203R"), one for
                 [MAX_SKELETON_ANIMATION_STRING_LENGTH]; // each gait receptor
                                                         // (room fr 8 poses +
                                                         // 'R',\0)

  // Chemo-receptor and emitter loci that are controlled/monitored by skeleton
  // rather than creature
  float myMusclesLocus;   // emitter: amount of energy expended by movement this
                          // tick
  ...
  AgentHandle myIt; // object of this creature's attention
                    // NULL if none
  bool
      myIntrospectiveFlag; // for extraspective actions like GO EAST and GO WEST
  ...
  bool BuildGalleries(int sex, int age, uint32 &currentPart,
                      TintManager &tinty);

public:
  void GetExtremesForPose(char *pose, Vector2D extremes[MAX_BODY_LIMBS]);
  ...
  bool ExpressGenes(const Genome &g,
                    int age); // Express any genes relevant to current age

  bool PrepareForAgeing(int age);

  virtual int ReachOut(BOOL Point = FALSE); // macro helper: touch IT or point
                                            // to it. Made virtual - dsb
  ...
  virtual void DrawLine(int32 x1, int32 y1, int32 x2, int32 y2,
                        ...
                        uint8 stippleoff = 0, uint32 stipplestartAt = 0);
  ...
  Skeleton(AgentHandle &gene,
           int gene_index); // Constructor called by Creature constr
  ...
  void AttachLimbChain(int position,
                       Limb *limbChain); // attach a chain of Limbs to
  ...
  bool SetTargetPoseString(char *pose);  // define new TargetPose if necc. Retn
                                         // TRUE if reached already
  bool HasTargetPoseStringBeenReached(); // retn TRUE if TargetPose has now been
                                         // reached

  void SetAnimationString(
      char *animation); // start a new animation, given a c++ string ("0123R")
  void
  ResetAnimationString(); // cancel any animation prior to special pose change
  ...
  inline void SetItAgent(AgentHandle &a) { // define object of your attention
  ...
  inline AgentHandle GetItAgent() { // return curr IT object
    _ASSERT(!myGarbaged);
    return myIt;
  }
  ...
  virtual bool
  Visibility(int scope); // retn 1 if any part of agent is on screen
...
// stop any animation - MUST call this before trying to change pose via any
// other means than an animation!!!!
