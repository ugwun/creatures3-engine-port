...
const int SIMPLEIDS =
    1; // ID of first SimpleObject ID (1-25) - 25 simpleobj genuses
const int COMPOUNDIDS =
    26; // ID of first CompObj ID (26-35) - 10 compobj genuses
const int CREATUREIDS =
    36; // ID of first Creature ID (36-39) - 4 creature genuses
...
void SensoryFaculty::PostInit() {
  ...
  // -1 cos you need 1 set of spare dendrites for migration to work (see genome
  // notes)
  int forfSize =
      myCreature.GetCreatureReference().GetBrain()->GetLobeSize("forf") - 1;
  if (forfSize >= 1) {
...
// Description:
// ------------------------------------------------------------------------
SensoryFaculty::~SensoryFaculty() { delete myDriveChemicals; }

// ------------------------------------------------------------------------
// Function:    Update
// Class:       SensoryFaculty
// Description: Sets most of the inputs to brain lobes so that the creature's
//				brain knows its situation ("situ"), any details
//of the IT object
//				("detl"), its drive levels ("driv"), the smell
//levels of the 40 				categories ("smel"), the displacement of any objects in
//visual 				range ("visn").
//
//				This function is also responsible for updating
//the list of all 				the agents the creature knows about and choosing which agent
//				"represents" each category.  There are several
//"category representative 				algorithms" (nearest object, random one and so on)
//and which algorithm 				is used for which category is decided based on an array in
//the catalogue 				files.
// ------------------------------------------------------------------------
void SensoryFaculty::Update() {
  ...
  brain->SetInput("situ", IP_IN_VEHICLE,
                  creature.GetCarrier().IsValid() ? 1.0f : 0.0f);

  brain->SetInput("situ", IP_AGE_LEVEL,
                  ((float)creature.Life()->GetAge()) / NUMAGES);
  brain->SetInput("situ", IP_CARRYING_SOMETHING,
                  creature.GetCarried() != NULLHANDLE ? 1.0f : 0.0f);
  brain->SetInput("situ", IP_BEING_CARRIED,
                  creature.GetMovementStatus() == Agent::CARRIED ? 1.0f : 0.0f);
  brain->SetInput(
      "situ", IP_FALLING,
      !creature.IsStopped() ? 1.0f : 0.0f); // if not stopped you're falling:
  ...
  brain->SetInput("situ", IP_SELECTED_CREATURE,
                  myCreature == theApp.GetWorld().GetSelectedCreature() ? 1.0f
                                                                        : 0.0f);
  ...
  int roomId;
  if (theApp.GetWorld().GetMap().GetRoomIDForPoint(
          creature.GetDownFootPosition(), roomId)) {
    for (i = 0; i < CA_PROPERTY_COUNT; i++) {
      ...
      creature.GetBiochemistry()->SetChemical(FIRST_SMELL_CHEMICAL + i,
                                              smellValue);
      ...
      if (neuronId == GetCategoryIdOfAgent(myCreature))
        smellValue =
            theApp.GetWorld().GetMap().GetRoomPropertyMinusMyContribution(
                myCreature, smellValue);
  ...
  for (genusId = 0; genusId < ourNumCategories; genusId++) {
    ...
    // if you can still see last known agent and have been talking about it -
    // keep it
    if (myKnownAgents[genusId].IsValid() &&
        creature.CanSee(myKnownAgents[genusId]) &&
        creature.GetBrain()->GetNeuronState("noun", genusId, STATE_VAR) >
            0.20f) {
    ...
    theAgentManager.FindBySightAndFGS(myCreature, visibleAgentsInThisCategory,
                                      ourCategoryClassifiers[genusId]);

    int whichAlgorithm =
        atoi(theCatalogue.Get("Category Representative Algorithms", genusId));
    if (whichAlgorithm < 0 ||
        whichAlgorithm >= NUM_CATEGORY_REPRESENTATIVE_ALGORITHMS)
    ...
    if (whichAlgorithm == PICK_A_RANDOM_ONE &&
        !creature.GetVirtualMachine().IsRunning()) {
    ...
    for (AgentListIterator l = visibleAgentsInThisCategory.begin();
         l != visibleAgentsInThisCategory.end(); l++, agentNo++) {
      ...
      if (winningAgent.IsInvalid()) {
        ...
        if (whichAlgorithm == PICK_NEAREST_IN_X_DIRECTION ||
            whichAlgorithm == PICK_NEAREST_IN_CURRENT_ROOM ||
            whichAlgorithm == PICK_RANDOM_NEAREST_IN_X_DIRECTION) {
          // closeness in X direction to creature's centre:
          float thisAgentXDistance =
              fabsf(creature.GetCentre().x -
          ...
          if (whichAlgorithm == PICK_RANDOM_NEAREST_IN_X_DIRECTION) {
            ...
            if (i < NO_RANDOM_NEAR_AGENTS) {
              if (i == agentsNearest.size()) {
                ...
                agentsDistance.insert(agentsDistance.begin() + i,
                                      thisAgentXDistance);
            ...
            float winningAgentXDistance = fabsf(
                creature.GetCentre().x - winningAgent.GetAgentReference()
                                             .WhereShouldCreaturesPressMe()
                                             .x);
          ...
          float thisAgentYDistance =
              fabsf(creature.GetPosition().y + creature.GetHeight() -
    ...
    if (whichAlgorithm == PICK_RANDOM_NEAREST_IN_X_DIRECTION &&
      ...
      // if have myKnownAgent[] already you are using the old one (cos still
      // running VM) and you have some to select from then select from nearest
      // vector
      int whichAgentToChoose = Rnd(0, agentsNearest.size() - 1);
      while (whichAgentToChoose != 0 &&
             agentsDistance[whichAgentToChoose] >
                 (visualRange / NEAR_RAND_VISUAL_RANGE)) {
  ...
  // Ensure that a carried object is always the representative for that
  // category:
  if (creature.GetCarried().IsValid()) {
    int categoryOfCarriedObject = GetCategoryIdOfAgent(creature.GetCarried());
    if (categoryOfCarriedObject >= 0 &&
        categoryOfCarriedObject < ourNumCategories &&
        !creature.GetCarried().GetAgentReference().TestAttributes(
            Agent::attrInvisible))
...
void SensoryFaculty::SetVisualInput(int i) {
  ...
  float xDisplacement = BoundIntoMinusOnePlusOne(
      (myKnownAgents[i].GetAgentReference().GetCentre().x -
       creature.GetCentre().x) /
      visualRange);
  float yDisplacement = BoundIntoMinusOnePlusOne(
      (myKnownAgents[i].GetAgentReference().GetCentre().y -
       creature.GetCentre().y) /
      visualRange);
...
// Use this agent's Family and Genus to produce a single number in range 0-39,
// which represents the 'type' of an object. Two objects of the same
// family+genus have the same ID#, and are INDISTINGUISHABLE from each other as
// far as creatures are concerned (they have the same name, 'look' the same,
// have the same significance, etc.)
// ------------------------------------------------------------------------
...
int SensoryFaculty::GetCategoryIdOfClassifier(const Classifier *c) {
  for (int i = 0; i < ourNumCategories; ++i) {
    if (c->GenericMatchForWildCard(ourCategoryClassifiers[i], TRUE, TRUE, TRUE,
                                   FALSE))
      return i;
  }
...
// Function:    Stimulate
// Class:       SensoryFaculty
// Description: Directly stimulate creature by using a built-in stimulus from
// creature's library
...
//				float strengthMultiplier = increase strength by
//this multiple 				bool forceNoLearning = allow learning to be disabled
// ------------------------------------------------------------------------
void SensoryFaculty::Stimulate(AgentHandle &from, int stim,
                               int fromScriptEventNo, float strengthMultiplier,
                               bool forceNoLearning) {
  Stimulus *s =
      &myStimulusLib[stim]; // get MY personal version of this stimulus
  s->fromAgent = from;
...
void SensoryFaculty::Stimulate(Stimulus s) // COPY of stimulus data
{
  ...
  if (!c.Life()->GetWhetherAlert() && !c.Life()->GetWhetherZombie()) {
    if (!(s.bitFlags & IFASLEEP)) // if stim not perceptible by
    {
      ...
      if (c.Life()->GetWhetherAsleep() &&
          s.incomingSentence ==
              c.Linguistic()->GetPlatonicWord(LinguisticFaculty::PERSONAL,
                                              LinguisticFaculty::ME)) {
  ...
  c.Linguistic()->HearSentence(s.fromAgent, s.incomingSentence, s.verbIdToStim,
                               s.nounIdToStim);
  ...
  if (s.verbStim > 1.0f) {
    ...
    if (s.verbStim != 0.0f)
      c.GetBrain()->SetInput(
          "verb", GetNeuronIdFromScriptOffset(s.verbIdToStim), s.verbStim);
  }
    ...
    if (chemicalId != 0) {
      ...
      if ((s.bitFlags & stimTrainingOffFlags[i]) || s.forceNoLearning) {
        ...
        AdjustChemicalLevel( // training is off
            s.chemicalsToAdjust[i], adjustment);
      } else {
        AdjustChemicalLevelWithTraining( // training is on
            s.chemicalsToAdjust[i], adjustment, s.fromScriptEventNo,
            s.fromAgent);
      }
    }
  }
...
void SensoryFaculty::ReadFromGenome(Genome &g) {
  ...
  g.Reset();
  while ((g.GetGeneType(CREATUREGENE, G_STIMULUS, NUMCREATURESUBTYPES)) !=
         false) {
...
void SensoryFaculty::AdjustChemicalLevelWithTraining(
    int whichChemical, float adjustment, int fromScriptEventNo,
    AgentHandle const &fromAgent) {
  ...
  if (drive != -1) {
    ...
    if (myCreature.GetCreatureReference().Life()->GetWhetherAlert()) {
      ...
      bool learn = true;
      if (theApp.GetWorld()
              .GetGameVar("engine_synchronous_learning")
              .GetInteger() == 1) {
        if (fromAgent != myCreature && fromAgent != thePointer) {
          // check decision creature has made corresponds to script agent is
          // running
          int decisionOffset =
              myCreature.GetCreatureReference().Motor()->GetCurrentDecisionId();
          int expectedAgentScript =
              GetExpectedAgentScriptFromDecisionOffset(decisionOffset);
#ifdef STIM_TEST_TRACE
          OutputFormattedDebugString(
              "Agent script: %d Decision script crossref:%d\n",
          ...
          const AgentHandle &itAgent =
              myCreature.GetCreatureReference().GetItAgent();
...
#endif
        myCreature.GetCreatureReference().GetBrain()->SetInput("resp", drive,
                                                               adjustment);
      }
#ifdef STIM_TEST_TRACE
      else
...
#endif
      myCreature.GetCreatureReference().GetBrain()->SetInput("prox", drive,
                                                             adjustment);
    }
  }
#ifdef STIM_TEST_TRACE
...
bool SensoryFaculty::SetupStaticVariablesFromCatalogue() {
  ...
  if (ourNumCategories != check) {
    ErrorMessageHandler::Show(
        "creature_error", 1,
        "SensoryFaculty::SetupStaticVariablesFromCatalogue");
    return false;
  }
...
float SensoryFaculty::DistanceToNearestCreature(int sex, int genus) {
  CreatureCollection &creatureCollection =
      theAgentManager.GetCreatureCollection();
  ...
  float nearest =
      meCreature.GetVisualRange() + 1.0f; // detect @ <= visual range
  ...
  for (int c = 0; c != noCreatures; c++) {
    ...
    if (meCreature.CanSee(AgentHandle(nearCreature))) {
      if ((sex == 0 || nearCreature.Life()->GetSex() == sex) &&
          (genus == 0 ||
           creatureCollection[c].GetAgentReference().GetClassifier().Genus() ==
               genus)) {
        float f =
            fabsf(meCreature.GetPosition().x - nearCreature.GetPosition().x);
...
//				your friend or foe list, initialising its
//friendliness towards 				you based on your blood relation etc.
...
int SensoryFaculty::AddFriendOrFoe(AgentHandle &creatureOrPointer) {
  ...
  FriendOrFoeIterator ffi =
      std::find(myFriendsAndFoeHandles.begin(), myFriendsAndFoeHandles.end(),
                creatureOrPointer);
  ...
  for (int i = 0; i != myFriendsAndFoeHandles.size(); ++i) {
    if (myFriendsAndFoeHandles[i].IsInvalid() &&
        myFriendsAndFoeMonikers[i].empty()) {
      // never used slot
      oldestSlot = i;
      break;
    } else if (myFriendsAndFoeHandles[i].IsInvalid() &&
               (myFriendsAndFoeLastEncounters[i] < oldestEncounter ||
                oldestSlot == -1))
      oldestSlot = i;
  }
  ...
  if (creatureOrPointer.IsCreature()) {
    ...
    myFriendsAndFoeMonikers[oldestSlot] =
        creatureOrPointer.GetAgentReference().GetGenomeStore().MonikerAsString(
            0);
    ...
    if (c.GetMoniker() == creature.GetMotherMoniker() ||
      ...
      myCreature.GetCreatureReference().GetBrain()->SetNeuronState(
          "forf", oldestSlot, STATE_VAR, 0.8f);
    } else if (c.GetMotherMoniker() == creature.GetMotherMoniker() ||
               c.GetFatherMoniker() == creature.GetFatherMoniker()) {
      // bothers and sisters
      myCreature.GetCreatureReference().GetBrain()->SetNeuronState(
          "forf", oldestSlot, STATE_VAR, 0.225f);

    } else if ((c.GetFamily() == creature.GetFamily()) &&
               (c.GetGenus() == creature.GetGenus()) && (c.GetGenus() != 2)) {
      // creatures of smae genus (but not grendels)
      myCreature.GetCreatureReference().GetBrain()->SetNeuronState(
          "forf", oldestSlot, STATE_VAR, 0.175f);
      ...
      myCreature.GetCreatureReference().GetBrain()->SetNeuronState(
          "forf", oldestSlot, FOURTH_VAR, -1.0f);
    }

  } else {
    myFriendsAndFoeMonikers[oldestSlot] =
        theApp.GetWorld().GetUniqueIdentifier();
  ...
  myCreature.GetCreatureReference().GetBrain()->SetNeuronState(
      "forf", oldestSlot, NGF_VAR, 1.0f);

  // flag bad action concept neurons for this agent catagory to have dendrites
  // migrate to them
  for (int b = 0; b != theCatalogue.GetArrayCountForTag("Bad Action Script");
       b++) {
    int badDecisionNeuron = GetNeuronIdFromScriptOffset(
        atoi(theCatalogue.Get("Bad Action Script", b)));
    int badConceptNeuron = (GetNumCategories() * badDecisionNeuron) +
                           GetCategoryIdOfAgent(creatureOrPointer);
    myCreature.GetCreatureReference().GetBrain()->SetNeuronState(
        "comb", badConceptNeuron, NGF_VAR, 1.0f);
  }

  // flag good action concept neurons for this agent catagory to have dendrites
  // migrate to them
  for (int g = 0; g != theCatalogue.GetArrayCountForTag("Good Action Script");
       g++) {
    int goodDecisionNeuron = GetNeuronIdFromScriptOffset(
        atoi(theCatalogue.Get("Good Action Script", g)));
    int goodConceptNeuron = (GetNumCategories() * goodDecisionNeuron) +
                            GetCategoryIdOfAgent(creatureOrPointer);
    myCreature.GetCreatureReference().GetBrain()->SetNeuronState(
        "comb", goodConceptNeuron, NGF_VAR, 1.0f);
  }
...
// Class:       SensoryFaculty
// Description: Used by the LinguisticFaculty for this norn say how he much he
// likes/
...
// Returns:     int =
// ------------------------------------------------------------------------
int SensoryFaculty::GetOpinionOfCreature(AgentHandle &creatureOrPointer,
                                         float &opinion, float &moodOpinion) {
  opinion = 0.0f;
  moodOpinion = 0.0f;
  ...
  if (creatureOrPointer.IsValid()) {
    ...
    for (int i = 0; i != myFriendsAndFoeHandles.size(); i++) {
      if (myFriendsAndFoeHandles[i] == creatureOrPointer) {
        opinion = myCreature.GetCreatureReference().GetBrain()->GetNeuronState(
            "forf", i, STATE_VAR);
        moodOpinion =
            myCreature.GetCreatureReference().GetBrain()->GetNeuronState(
                "forf", i, OUTPUT_VAR);
        return i;
      }
    }
...
AgentHandle SensoryFaculty::GetNearestCreatureOrPointer() {
  ...
  float f = fabsf(meCreature.GetPosition().x -
                  thePointer.GetAgentReference().GetPosition().x);
  if (f < nearest)
...
void SensoryFaculty::ClearSeenFriendsOrFoes() {
  ...
  for (int i = 0; i != myFriendsAndFoeHandles.size(); i++) {
    myCreature.GetCreatureReference().GetBrain()->SetNeuronState(
        "forf", i, FIFTH_VAR, 0.0f);
    myCreature.GetCreatureReference().GetBrain()->SetNeuronState("forf", i,
                                                                 NGF_VAR, 0.0f);
  }
  ...
  for (int i = 0; i != myFriendsAndFoeHandles.size(); i++) {
    if (myFriendsAndFoeHandles[i].IsValid()) {
      ...
      if (myFriendsAndFoeMonikers[i] ==
        ...
        creature.Linguistic()->ClearWordStrength(LinguisticFaculty::NOUN,
                                                 pointerCat);
        creature.Linguistic()->SetWord(
            LinguisticFaculty::NOUN, pointerCat,
            thePointer.GetPointerAgentReference().GetName(), true);
      } else {
        // try all creatures against this moniker
        for (int c = 0; c != noCreatures; c++) {
          if (myFriendsAndFoeMonikers[i] == creatureCollection[c]
                                                .GetAgentReference()
                                                .GetGenomeStore()
                                                .MonikerAsString(0)) {
  ...
  if (thisCreatureIsImported) {
    ...
    for (int c = 0; c != noCreatures; c++) {
      ...
      if (myCreature != creatureCollection[c]) {
        creatureCollection[c]
            .GetCreatureReference()
            .Sensory()
            ->AssignFriendOrFoeByMoniker(
                creature.GetGenomeStore().MonikerAsString(0), myCreature);
      }
    }
  }
}
...
// Returns:     bool =
// ------------------------------------------------------------------------
bool SensoryFaculty::AssignFriendOrFoeByMoniker(
    std::string moniker, AgentHandle &creatureOrPointer) {
  for (int i = 0; i != myFriendsAndFoeHandles.size(); i++) {
...
void SensoryFaculty::RemoveFromAllFriendAndFoe(AgentHandle &creatureOrPointer) {
  ...
  // try all creatures against this moniker
  for (int c = 0; c != noCreatures; c++)
    creatureCollection[c].GetCreatureReference().Sensory()->RemoveFriendAndFoe(
        creatureOrPointer);
}
