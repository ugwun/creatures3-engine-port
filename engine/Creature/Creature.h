...
// -------------------------------------------------------------------------
// Class: Creature
// *Probably* represents the main living entities in the game (Norns, Grendels,
// Ettins). Derives from Skeleton for physical presence and contains numerous
// Faculties (Sensory, Life, Reproductive, Brain, etc.) to handle its complex
// biological and neurological simulation.
// -------------------------------------------------------------------------
...
public:
  ...
  Creature(int family, uint32 id, AgentHandle gene,
           int gene_index, // your genome
           int sex,        // your sex 0=randomly choose 1=male 2=female
           int variant); // only the agent manager can create Creatures (except
                         // serialization)
  ...
  void StartHoldingHandsWithThePointer(const AgentHandle &from,
                                       const CAOSVar &p1, const CAOSVar &p2);
  void HoldHandsWithThePointer();
  void StopHoldingHandsWithThePointer(const AgentHandle &from,
                                      const CAOSVar &p1, const CAOSVar &p2);
