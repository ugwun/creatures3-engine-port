...
void Organ::Init(float ClockRate, float RateOfRepair, float LifeForce,
  ...
  loc_LifeForce = std::max(1.0f, LifeForce);
  myInitialLifeForce = loc_LifeForce * myBaseLifeForce;
  myShortTermLifeForce = myInitialLifeForce;
  myLongTermLifeForce = myInitialLifeForce;

  myLongTermRateOfRepair = RateOfRepair;
  if (DamageDueToZeroEnergy > 1.0f)
    DamageDueToZeroEnergy = 1.0f;
  myDamageDueToZeroEnergy =
      (myInitialLifeForce * DamageDueToZeroEnergy) / 255.0f;
}
...
bool Organ::Write(CreaturesArchive &archive) const {
  ...
  // got to serialize reactions before receptors as receptors can bind to the
  // rate locus now on reactions:
  ...
  for (i = 0; i < myNoOfReceptorGroups; i++) {
    // ReceptorPtrArray is not directly serialisable in this format, so we'll
  ...
  for (i = 0, e = myEmitters; i < myNoOfEmitters; i++, e++) {
    ...
    archive << e->Chem << e->Threshold << e->bioTickRate << e->Gain
            << e->Effect;
...
bool Organ::Read(CreaturesArchive &archive) {
  ...
  if (version >= 3) {
    ...
    archive >> myClock;
    archive >> myInitialLifeForce >> myShortTermLifeForce >>
        myLongTermLifeForce;
    archive >> myLongTermRateOfRepair >> myEnergyCost;
    archive >> myDamageDueToZeroEnergy >> myEnergyAvailableFlag;
    archive >> myNoOfReceptors >> myNoOfReceptorGroups;
...
void Organ::InitFromGenome(Genome &genome) {
  ...
  while (true) {
    ...
    while ((genome.GetGeneType(BIOCHEMISTRYGENE, G_RECEPTOR, NUMBIOCHEMSUBTYPES,
                               SWITCH_AGE, BIOCHEMISTRYGENE, G_REACTION,
                               ORGANGENE)) != false) {
      ...
      r->IDOrgan = genome.GetCodon(0, NUM_RECEPTOR_ORGANS -
                                          1); // organ that receptor binds to
      r->IDTissue =
          genome.GetCodon(0, 255); // tissue in organ that receptor binds to

      if (r->IDOrgan == ORGAN_REACTION) // for current reaction replace tissue
                                        // no by reaction no
        r->IDTissue = thisReactionNo;

      r->IDLocus = genome.GetCodon(
          0, 255); // locus in that tissue that receptor binds to
    ...
    if ((genome.GetGeneType(BIOCHEMISTRYGENE, G_REACTION, NUMBIOCHEMSUBTYPES,
                            SWITCH_AGE, ORGANGENE)) == false) {
      // no more reactions so break:
      break;
    }
    if (myNoOfReactions ==
        MAXREACTIONS - 1) // ignore if too many genes! (save space for ATP->ADP)
    {
      break;
    }
    ...
    rn->propR1 = genome.GetCodon(
        1, MAXREACTANTS);      // proportion of reactant1 (1-MAXREACTANTS)
    ...
    rn->propP1 = genome.GetCodon(
        1, MAXREACTANTS);      // proportion of product1 (1-MAXREACTANTS)
    rn->P1 = genome.GetByte(); // product1 (0=none)
    rn->propP2 = genome.GetCodon(
        1, MAXREACTANTS);      // proportion of product2 (1-MAXREACTANTS)
  ...
  // same locus AND emit the same chemical. Otherwise, they are added to the
  // list.
  genome.Restore();
  while ((genome.GetGeneType(BIOCHEMISTRYGENE, G_EMITTER, NUMBIOCHEMSUBTYPES,
                             SWITCH_AGE, ORGANGENE)) != false) {
    ...
    o = genome.GetCodon(0,
                        NUM_EMITTER_ORGANS - 1); // organ that receptor binds to
    ...
    // Search for any pre-existing emitter at this locus and replace this only
    // if it also emits the same chemical
    for (i = 0; i < myNoOfEmitters; i++) {
      if (myEmitters[i].IDOrgan == o && myEmitters[i].IDTissue == t &&
          myEmitters[i].IDLocus == l && myEmitters[i].Chem == c) {
        e = &myEmitters[i];
        break;
      }
    }
  ...
  // Restore position. Necessary to prevent organs begin infinitely read from
  // the genome.
  genome.Restore();
...
bool Organ::Update() {
  ...
  if (!Failed()) {
    ...
    if (myClock >= 1.0f) {
      ...
      // If full injury applied, organ can fail in a second (clock-rate
      // depending). 10 = activations necessary to remove all life-force.
      ...
      workWasDone |=
          ProcessReceptors(true); // process just clock-rate receptors
    }
...
float Organ::LocToLf(float l) { return LOC_TO_LF(l); }
...
float *Organ::GetLocusAddress(int type, int organ, int tissue, int locus) {
  ...
  default:
    ...
    return myBiochemistryOwner->GetCreatureLocusAddress(type, organ, tissue,
                                                        locus);
  }
}
  ...
  for (int g = 0; g < myNoOfReceptorGroups; g++) {
    ...
    for (int rIndex = 0; rIndex < rGroup.size(); rIndex++) {
      ...
      if (r->Chem) { // if receptor is used...
        ...
        float inputSignal = myBiochemistryOwner->GetChemical(
                                r->Chem) - // get current chemical concentration
                            r->Threshold;  // get excess of conc over threshold
        if (inputSignal < 0.0f)
          inputSignal = 0.0f; // ignore changes below threshold
    ...
    if (r->IDOrgan == ORGAN_CREATURE && r->IDTissue == TISSUE_IMMUNE &&
        r->IDLocus == LOC_DIE) {
      result = (totalOfAllNominals + termToAddSoFar - termToSubSoFar) > 0.0f
                   ? 1.0f
                   : 0.0f;
    }
    *r->Dest = result;
...
bool Organ::ProcessReaction(int iIndex) {
  ...
  if (avail) {
    // rn->Rate is 0 for slow, 1 for fast (reverse from when loaded in from the
    // genome!)
    ...
    myBiochemistryOwner->SubChemical(
        rn->R1, avail * rn->propR1); // reduce any reactants by this much
    myBiochemistryOwner->SubChemical(
        rn->R2, avail * rn->propR2); // modulated by proportions [19/2/96]

    myBiochemistryOwner->AddChemical(
        rn->P1, avail * rn->propP1); // increase any products by this much
    myBiochemistryOwner->AddChemical(
        rn->P2, avail * rn->propP2); // (in proportion as specified)
...
void Organ::InitEnergyCost() {
  ...
  myEnergyCost =
      myBaseOrganADPCost +
      ((myNoOfReceptors + myNoOfEmitters + myNoOfReactions) / 2550.0);
