...
//
//				I have made the constructor private so that
// there can be 				only one agent manager
...
//					Moved the main view from the world class
// to here. 					agents add themselves to the
// display engine when they 					construct and
// remove themselves during destruction.
...
////////////////////////////////////////////////////////////////////////////
AgentManager &AgentManager::GetAgentManager() { return ourAgentManager; }

////////////////////////////////////////////////////////////////////////////
// Agent Creation methods
////////////////////////////////////////////////////////////////////////////

AgentHandle AgentManager::CreateCreature(int family, AgentHandle &gene,
                                         int gene_index, int8 sex,
                                         int8 variant) {
  std::string monikerForDebugMessage =
      gene.GetAgentReference().GetGenomeStore().MonikerAsString(gene_index);
  ...
  try {
    ...
    AgentHandle creature(
        new Creature(family, id, gene, gene_index, sex, variant));
    if (creature.IsCreature() &&
        creature.GetAgentReference().myFailedConstructionException == "") {
    } else {
#ifdef C2E_OLD_CPP_LIB
      char hackbuf[256];
      ...
      ost << "Creature:" << std::endl
          << "Family: " << family << " Moniker: " << monikerForDebugMessage;
      ost << " Gender: " << (int)sex << " Variant: " << (int)variant
          << std::endl;
      ost << creature.GetAgentReference().myFailedConstructionException;
      ErrorMessageHandler::NonLocalisable(ost.str(),
...
AgentHandle AgentManager::CreateSimpleAgent(int family, int genus, int species,
                                            FilePath const &gallery,
                                            int32 imagecount, int32 firstimage,
                                            int32 plane) {
  uint32 id = CreateUniqueAgentID();
  try {
    AgentHandle agent(new SimpleAgent(family, genus, species, id, gallery,
                                      imagecount, firstimage, plane, false));
    ...
    if (agent.GetAgentReference().myFailedConstructionException.size() == 0) {
      ...
      ourFGSHelperMap[id] = ourFGSMap.insert(
          std::make_pair(agent.GetAgentReference().GetClassifier(), agent));
    } else {
#ifdef C2E_OLD_CPP_LIB
      char hackbuf[256];
      std::ostrstream ost(hackbuf, sizeof(hackbuf));
      ...
      std::ostringstream ost;
#endif
      ost << "Simple Agent:" << std::endl
          << "Family: " << family << " Genus: " << genus
          << " Species: " << species << std::endl;
      ost << "Gallery: " << gallery.GetFileName()
          << " First Image: " << firstimage << " Image Count: " << imagecount
          << std::endl;
    ...
    ErrorMessageHandler::Show(e,
                              std::string("AgentManager::CreateSimpleAgent"));
    return NULLHANDLE;
  }
}

AgentHandle AgentManager::CreateCompoundAgent(int family, int genus,
                                              int species,
                                              FilePath const &gallery,
  ...
  uint32 id = CreateUniqueAgentID();
  try {
    AgentHandle agent(new CompoundAgent(family, genus, species, id, gallery,
                                        numimages, baseimage, plane));
    ...
    if (agent.GetAgentReference().myFailedConstructionException.size() == 0) {
      ...
      std::ostringstream ost;
#endif
      ost << "Compound Agent:" << std::endl
          << "Family: " << family << " Genus: " << genus
          << " Species: " << species << std::endl;
      ost << "Gallery: " << gallery.GetFileName()
          << " First Image: " << baseimage << " Image Count: " << numimages
          << std::endl;
      ost << "Created in plane: " << plane << std::endl;
      ost << agent.GetAgentReference().myFailedConstructionException;
      agent.GetAgentReference().Trash();
      ErrorMessageHandler::NonLocalisable(ost.str(),
                                          "AgentManager::CreateCompoundAgent");
    }
    return agent;
  } catch (EntityImage::EntityImageException &e) {
    ErrorMessageHandler::Show(e,
                              std::string("AgentManager::CreateCompoundAgent"));
    return NULLHANDLE;
  }
}

AgentHandle AgentManager::CreateVehicle(int family, int genus, int species,
                                        FilePath const &gallery,
                                        int32 numimages, int32 baseimage,
                                        int32 plane) {
  uint32 id = CreateUniqueAgentID();
...
AgentHandle AgentManager::CreatePointer() {
  ...
  try {
    AgentHandle agent(
        new PointerAgent(family, genus, species, id, gallery, 0, 0));

    if (agent.GetPointerAgentReference().myFailedConstructionException.size() ==
        0) {
...
void AgentManager::ExecuteScriptOnAllAgents(int event, AgentHandle &from,
                                            const CAOSVar &p1,
                                            const CAOSVar &p2) {
  AgentListIterator it;
  AgentMapIteratorListIterator kit;
  ...
  for (it = ourAgentList.begin(); it != ourAgentList.end(); ++it) {
    ...
    if (agent.IsValid()) {
      ...
      if (agentref.IsRunning() && !agentref.AreYouDoomed())
        agentref.ExecuteScriptForEvent(event, from, p1, p2);
      if (agentref.AreYouDoomed())
        KillAgent(agent);
    }
  }
...
void AgentManager::ExecuteScriptOnAllAgentsDeferred(int event,
                                                    AgentHandle &from,
                                                    const CAOSVar &p1,
                                                    const CAOSVar &p2) {
...
CreaturesArchive &operator<<(CreaturesArchive &ar,
                             AgentManager::DeferredScript const &script) {
  ar << script.event << script.from << script.p1 << script.p2;
  return ar;
}

CreaturesArchive &operator>>(CreaturesArchive &ar,
                             AgentManager::DeferredScript &script) {
  ar >> script.event >> script.from >> script.p1 >> script.p2;
  return ar;
}
...
//				Scan all existing creatures and pick a plot
// plane 				that doesn't clash with any of them.
  ...
  do {
    ...
    for (i = 0; i < ourCreatureCollection.size(); i++)
      ...
      if ((ourCreatureCollection[i] != me) &&
          (ourCreatureCollection[i].GetCreatureReference().GetPlane(0) >=
           plane) &&
          (ourCreatureCollection[i].GetCreatureReference().GetPlane(0) <=
           plane + 8)) {
        unique = 0;
...
AgentHandle AgentManager::WhoAmITouching(AgentHandle &me) {
  ...
  for (i = 0; i < ourCreatureCollection.size(); i++) { // scan creature library
    ...
    // & carrying creature
    // vehicle cast
    if (me.IsVehicle() && (o.GetAgentReference().GetCarrier() == me)) {
      return o;
    }
...
AgentHandle AgentManager::WhatVehicleCouldIDropInto(const AgentHandle &me,
                                                    const AgentHandle &ignore) {
  AgentListIterator it;
  AgentHandle v, a;
  ...
  for (it = ourAgentList.begin(); it != ourAgentList.end(); ++it) {
    ...
    if (vehicleref.TestAttributes(Agent::attrGreedyCabin) !=
        Agent::attrGreedyCabin)
      continue;
    ...
    if ((agentCentre.x <= cabin.left) || (agentCentre.x >= cabin.right) ||
        (agentCentre.y <= cabin.top) || (agentCentre.y >= cabin.bottom))
...
//				score - total for how much noise neighbouring
...
//
// ----------------------------------------------------------------------
void AgentManager::HowManyCreaturesCanIHear(AgentHandle &me, int32 &score) {
  ...
  for (creature = ourCreatureCollection.begin();
    ...
    if (me.GetCreatureReference().CanHear((*creature))) {
      int32 tmp =
          (int32)me.GetCreatureReference().GetAudibleRange() -
          abs((int32)(me.GetCreatureReference().GetPosition().x -
                      (*creature).GetCreatureReference().GetPosition().x));
      score += (tmp * tmp);
    }
  }
}
...
//
// ----------------------------------------------------------------------
AgentHandle AgentManager::WhatAmITouching(Vector2D &ptThis, AgentHandle &me,
                                          uint32 AttrFields, uint32 AttrState) {
  int32 BestPlane = -1;
  AgentHandle BestObj;
  AgentHandle Obj;
  AgentListIterator it;
...
AgentHandle AgentManager::WhatCreatureAmIOver(const AgentHandle &me,
                                              const Vector2D &xy) {
  Box r;
  int i;
  AgentHandle agent;
...
AgentHandle AgentManager::WhatAmIOver(const AgentHandle &me,
                                      const Vector2D &xy) {
  Box r;
  AgentListIterator it;
  AgentHandle agent;
...
// Arguments:   iSelectableCount - how many creatures were selectable and on
// screen
//				iLowestHealth - the lowest health value
// encountered 				fMood		- average creature's
// mood 				fThreat   - average creature's threat
//
// Returns:     the first creature you can hear
//
...
void AgentManager::CalculateMoodAndThreat(uint32 &iSelectableCount,
  ...
  for (subject = ourCreatureCollection.begin();
       subject != ourCreatureCollection.end(); subject++) {
    ...
    if (c.Visibility(0) && c.Music()->SelectableByUser() &&
        !c.Music()->Hatching()) {
      fMood += c.Music()->Mood();
...
void AgentManager::KillAgent(const AgentHandle &agent) {
  ...
  if (agent2.IsCreature()) {
    CreatureCollectionIterator creature;
    creature = std::find(ourCreatureCollection.begin(),
                         ourCreatureCollection.end(), agent);
  ...
  TaxonomicalHelperIterator thit =
      ourFGSHelperMap.find(agent.GetAgentReference().GetUniqueID());
...
bool AgentManager::UpdateSmellClassifierList(int caIndex,
                                             Classifier *classifier) {
  if (caIndex < 0 || (classifier->Family() == 0 && classifier->Genus() == 0 &&
...
void AgentManager::FindByFGS(AgentList &agents, const Classifier &c) {
  if (((c.myFamily == 0) && ((c.myGenus != 0) || (c.mySpecies != 0))) ||
      ((c.myGenus == 0) && (c.mySpecies != 0))) {
    AgentListIterator it;
    for (it = ourAgentList.begin(); it != ourAgentList.end(); ++it) {
      if ((*it).IsValid() &&
          (*it).GetAgentReference().GetClassifier().GenericMatchForWildCard(
...
void AgentManager::FindByArea(AgentList &input, AgentList &agents,
                              const Box &r) {
  AgentListIterator it;
...
void AgentManager::FindByAreaAndFGS(AgentList &agents, const Classifier &c,
                                    const Box &r) {
  AgentListIterator it;
  ...
  while(  it != agents.end() )
  {
          ...
          if(
  !(*it).GetAgentReference().GetClassifier().GenericMatchForWildCard( c, TRUE,
  TRUE, TRUE, FALSE ) )
          {
                  agents.erase( it );
          }
...
void AgentManager::FindBySightAndFGS(AgentHandle const &viewer,
                                     AgentList &agents, const Classifier &c) {
  AgentListIterator it;
  AgentListIterator itnext;
...
AgentHandle AgentManager::FindPreviousAgent(AgentHandle &was,
                                            const Classifier &c) {
  AgentList agents;
  FindByFGS(agents, c);
...
bool AgentManager::Read(CreaturesArchive &ar) {
  ...
  // Right, all the blasted agents are loaded in and stabilised - let's have
  // some fun :)
  AgentListIterator it;
  for (it = ourAgentList.begin(); it != ourAgentList.end(); it++)
...
void AgentManager::RegisterClone(AgentHandle &clone) {
  ...
  ourFGSHelperMap[id] = ourFGSMap.insert(
      std::make_pair(clone.GetAgentReference().GetClassifier(), clone));
  if (clone.IsCreature())
...
void AgentManager::AddCreatureToUpdateList(AgentHandle const &creature) {
  ...
  ourAgentMap[id] = ourAgentList.begin();
  ourFGSHelperMap[id] = ourFGSMap.insert(
      std::make_pair(creature.GetAgentReference().GetClassifier(), creature));
}
