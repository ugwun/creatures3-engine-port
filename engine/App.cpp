...
// -------------------------------------------------------------------------
// App::Init()
// *Probably* the main initialization routine for the entire engine.
// It sets up directories, random number generators, the main camera,
// sound systems, and loads the initial 'Startup' world.
// -------------------------------------------------------------------------
#ifdef _WIN32
...
#endif
{
  ...
  theFlightRecorder.Log(4, "App::Init() starting");
...
#else
  // On POSIX dev builds always enable all categories. machine.cfg has
  // FlightRecorderMask=33 (ERR+SND only) which would silently drop
  // WRLD/CAOS/SHUT. The browser monitor's category checkboxes provide
  // client-side filtering instead.
  theFlightRecorder.SetCategories(0xFFFFFFFF);
#endif
...
// -------------------------------------------------------------------------
// App::UpdateApp()
// *Probably* the core application tick / loop body.
// Handles system-level updates like window moving/resizing,
// saving/quitting triggers, advancing the World simulation (TaskSwitcher),
...
// -------------------------------------------------------------------------
// App::GetDirectories()
// Maps the logical directories (e.g. Sounds, Images, Worlds)
// to physical paths on disk. *Probably* pulls from user config or
// Windows Registry to locate all game assets.
// -------------------------------------------------------------------------
...
#else
  // POSIX version: use MachineSettings() (loaded from machine.cfg in
  // InitConfigFiles) machine.cfg has entries like: "Backgrounds Directory"
  // "Backgrounds" relative to the game's main directory.

  // The game directory is set via chdir() in main() (--game-dir flag).
  // All relative paths in machine.cfg (e.g. "Images", "Bootstrap") are
  // therefore relative to the current working directory, which is the game
  // root.  We use "./" as the base so they resolve correctly at runtime.
  const std::string machineBase = "./";

  for (dir = 0; dir < NUM_DIRS; dir++) {
    std::string key((char *)(directories[dir]));
    std::string relPath;
    MachineSettings().Get(key, relPath);
    ...
    struct stat inf;
    bool groovy = (stat(buffer.c_str(), &inf) == 0 && S_ISDIR(inf.st_mode));

    if (!groovy) {
      // Non-fatal: some optional directories may not exist.
      theFlightRecorder.Log(1, "[WARN] Directory not found: %s (for key: %s)",
                            buffer.c_str(), key.c_str());
      // Still set the path so code that checks doesn't crash on null.
    }
...
#ifndef _WIN32
// Load auxiliary directory paths (e.g. "Auxiliary 1 Images Directory")
// from machine.cfg.  These are used as fallback search paths for C3 assets.
{
  const char *dirSuffixes[] = {"Main Directory",
                               "Sounds Directory",
                               "Images Directory",
                               "Genetics Directory",
                               "Body Data Directory",
  ...
  for (int n = 1; n <= 9; ++n) {
    ...
    for (int d = 0; d < NUM_DIRS; ++d) {
      char key[64];
      snprintf(key, sizeof(key), "Auxiliary %d %s", n, dirSuffixes[d]);
      std::string relPath;
      ...
      if (stat(auxBuf.c_str(), &inf2) == 0 && S_ISDIR(inf2.st_mode)) {
        strcpy(m_auxDirs[d], auxBuf.data());
        m_hasAuxDirs[d] = true;
      }
    }
    if (!foundAny)
      break; // No more auxiliary sets.
  }
}
#endif
...
#else
  int inthack = 0; // default to windowed (SDL fullscreen not yet implemented)
...
void App::SetUpSound(void) {
  ...
  if (IOnlyPlayMidiMusic == false && !myCommandLineNoMusic && !myCommandLineNoSound) {
    ...
    // Set the default MNG from game config before loading.
    // On macOS the config may supply 'DS_music.mng' (lowercase m) while the
    // actual file is 'DS_Music.mng' — LoadScrambled handles the case fix.
    {
      std::string defaultMunge;
#ifdef _WIN32
      theRegistry.GetValue(theRegistry.DefaultKey(),
                           std::string("Default Munge"), defaultMunge,
                           HKEY_CURRENT_USER);
#else
      UserSettings().Get(std::string("Default Munge"), defaultMunge);
#endif
      if (!defaultMunge.empty())
        theMusicManager->SetCurrentMNG(defaultMunge);
    }
...
// size of 				window too. Note that the display modes
// are enumrated at 				start up of the display engine
//
// ----------------------------------------------------------------------
  ...
  try {
...
#ifndef _WIN32
    // Load Patch.catalogue from auxiliary game directories so that CAOS
    // "REAQ" can detect installed auxiliary games.  The DS world switcher
    // calls `reaq "patch_level"` to check if C3 is installed — this tag
    // is defined in C3's Patch.catalogue.
    //
    // We intentionally load ONLY Patch.catalogue rather than the entire
    // auxiliary catalogue directory.  Loading all ~80 C3 catalogue files
    // causes duplicate-tag rewrites in theCatalogue's string table, which
    ...
    //   "Auxiliary 1 Catalogue Directory" "../Creatures 3/Catalogue"
    {
      ...
      for (int n = 1; n <= 9; ++n) {
        ...
        // internal ID rewrites that corrupt SensoryFaculty mappings
        // → AHE0009 crash.
        ...
        for (size_t af = 0; af < sizeof(allowedFiles) / sizeof(allowedFiles[0]);
             ++af) {
          std::string catFile = auxCatPath + allowedFiles[af];
          struct stat st;
          if (stat(catFile.c_str(), &st) == 0 && S_ISREG(st.st_mode)) {
            try {
              theCatalogue.AddFile(catFile, langid);
              theFlightRecorder.Log(
                  4, "InitLocalisation: loaded auxiliary catalogue: %s",
                  catFile.c_str());
            } catch (Catalogue::Err &e) {
              ErrorMessageHandler::Show(e, "App::InitLocalisation (auxiliary)");
            }
          }
        }

        // Selectively import individual tags needed by C3 bootstrap
        // scripts.  We can't load full C3 catalogue files (causes
        // AHE0009), but AddTagFromFile safely extracts only the
        // requested tag without disturbing existing ID mappings.
        struct {
          const char *file;
          const char *tag;
        } neededTags[] = {
            {"Materia Medica.catalogue", "medicine maker potions"},
        };
        for (size_t t = 0; t < sizeof(neededTags) / sizeof(neededTags[0]);
             ++t) {
          std::string catFile = auxCatPath + neededTags[t].file;
          struct stat st;
          if (stat(catFile.c_str(), &st) == 0 && S_ISREG(st.st_mode)) {
            if (theCatalogue.AddTagFromFile(catFile, langid,
                                            neededTags[t].tag)) {
              theFlightRecorder.Log(
                  4, "InitLocalisation: imported tag \"%s\" from %s",
                  neededTags[t].tag, neededTags[t].file);
            }
          }
        }
      }
...
bool App::GetWorldDirectoryVersion(int dir, std::string &path,
  ...
  path = buf;
  // get the words between the last two slashes (works with / or \)
...
void App::SetScrolling(int andMask, int eorMask,
                       const std::vector<byte> *speedUp,
                       const std::vector<byte> *speedDown) {
  myScrollingMask = (myScrollingMask & andMask) ^ eorMask;
  if (speedUp) {
    myScrollingSpeedRangeUp = *speedUp;
  }
  if (speedDown) {
    myScrollingSpeedRangeDown = *speedDown;
  }
}
...
bool App::InitConfigFiles(const std::string &userfile,
  ...
  // The game directory is set via chdir() in main() (--game-dir flag), so
  // we can use relative paths here.  The config files sit directly in the
  // game directory root (e.g. "machine.cfg", "user.cfg").
  myMachineSettings.BindToFile(machinefile);
  myUserSettings.BindToFile(userfile);

  return true;
}

#ifndef _WIN32
AgentHandle GetSingleStepAgent() { return NULLHANDLE; }
