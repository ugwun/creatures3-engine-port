...
// Helper: resolve journal base path for the given directory mode.
// directory 0 = current world's journal
// directory 1 = main journal
// directory 2 = journal of the world named in game var "engine_other_world"
static std::string ResolveJournalPath(int directory, bool createDir) {
  if (directory == 0) {
    std::string path;
    theApp.GetWorldDirectoryVersion(JOURNAL_DIR, path, createDir);
    return path;
  } else if (directory == 2) {
    // DS extension: use the journal directory of another world,
    // named by the game variable "engine_other_world".
    // The world switcher sets this before doing file ops on
    // other worlds (e.g. reading their wtype files for icons).
    CAOSVar &var = theApp.GetWorld().GetGameVar("engine_other_world");
    if (var.GetType() == CAOSVar::typeString) {
      std::string otherWorld;
      var.GetString(otherWorld);
      if (!otherWorld.empty()) {
        // Build path: <worlds_dir>/<otherWorld>/Journal/
        char buf[MAX_PATH];
        theApp.GetDirectory(WORLDS_DIR, buf);
        std::string path = std::string(buf) + otherWorld + "/Journal/";
        if (createDir && access(path.c_str(), F_OK) != 0)
          mkdir(path.c_str(), 0755);
        return path;
      }
    }
    // Fallback: if engine_other_world is not set, use current world journal
    std::string path;
    theApp.GetWorldDirectoryVersion(JOURNAL_DIR, path, createDir);
    return path;
  } else {
    // directory == 1 (or any other value): main journal directory
    return theApp.GetDirectory(JOURNAL_DIR);
  }
}
...
void GeneralHandlers::SubCommand_FILE_IOPE(CAOSMachine &vm) {
  ...
  std::string basepath = ResolveJournalPath(directory, false);
