...
void CreatureHandlers::Command_SPNL(CAOSMachine &vm) {
  ...
  vm.GetCreatureTarg().GetBrain()->SetInput(lobeToken.c_str(), neuronNo,
                                            vm.FetchFloatRV());
}
...
void CreatureHandlers::Command_APPR(CAOSMachine &vm) {
  ...
  int creatureTargCategory =
      SensoryFaculty::GetCategoryIdOfAgent(AgentHandle(c));
  int caIndex = theAgentManager.GetSmellIdFromCategoryId(attnId);
  int creatureRealRoomId = -1;
  // set it so you can't smell yourself if you're navigating to other creatures
  // of your category:
  ...
  map.WhichDirectionToFollowCA(creatureFootRoomId, caIndex, mapDirection,
                               true); // true means approach
...
void CreatureHandlers::Command_FLEE(CAOSMachine &vm) {
  ...
  if (c.GetItAgent().IsValid()) {
    ...
    //		c.ResetAnimationString();			// got as close
    //as we can get so stop walking! 		if (vm.IsBlocking()) 			vm.UnBlock(); 		return;
  }
  ...
  Map &map = theApp.GetWorld().GetMap();
  int creatureFootRoomId;
  if (!theApp.GetWorld().GetMap().GetRoomIDForPoint(c.GetDownFootPosition(),
                                                    creatureFootRoomId)) {
  ...
  int caIndex = theAgentManager.GetSmellIdFromCategoryId(
      c.Motor()->GetCurrentAttentionId());

  int mapDirection = GO_NOWHERE;
  map.WhichDirectionToFollowCA(creatureFootRoomId, caIndex, mapDirection,
                               false); // false means retreat

  NavigateDirection(c, mapDirection, caIndex);
}

void CreatureHandlers::NavigateDirection(Creature &c, int mapDirection,
                                         int caIndex) {
...
void CreatureHandlers::Command_LTCY(CAOSMachine &vm) {
  ...
  vm.GetCreatureTarg().Motor()->SetLatencyOfInvoluntaryAction(
      x, y < l ? Rnd(y, l) : Rnd(l, y));
}
...
void CreatureHandlers::Command_STIM(CAOSMachine &vm) {
  ...
  OutputFormattedDebugString(
      "STIM classifier %d %d %d %d\n", scriptClassifier.Family(),
      scriptClassifier.Genus(), scriptClassifier.Species(),
      scriptClassifier.Event());
#endif
...
void CreatureHandlers::Command_LOCI(CAOSMachine &vm) {
  ...
  float *locus = vm.GetCreatureTarg().GetLocusAddress(lociType, lociOrgan,
                                                      lociTissue, lociId);
...
void CreatureHandlers::Command_BRN(CAOSMachine &vm) {
  ...
  static CommandHandler HandlerTable[subcount] = {
      SubCommand_BRN_SETN, SubCommand_BRN_SETD, SubCommand_BRN_SETL,
      SubCommand_BRN_SETT, SubCommand_BRN_DMPB, SubCommand_BRN_DMPL,
      SubCommand_BRN_DMPT, SubCommand_BRN_DMPN, SubCommand_BRN_DMPD};
  int subcmd;
...
void CreatureHandlers::SubCommand_BRN_SETN(CAOSMachine &vm) {
  ...
  float value = vm.FetchFloatRV();
  if (!vm.GetCreatureTarg().GetBrain()->SetNeuronState(lobe, neuron, state,
                                                       value))
...
void CreatureHandlers::SubCommand_BRN_SETD(CAOSMachine &vm) {
  ...
  float value = vm.FetchFloatRV();
  if (!vm.GetCreatureTarg().GetBrain()->SetDendriteWeight(tract, dendrite,
                                                          weight, value))
...
void CreatureHandlers::SubCommand_BRN_DMPL(CAOSMachine &vm) {
  if (!vm.GetCreatureTarg().GetBrain()->DumpLobe(vm.FetchIntegerRV(),
                                                 *vm.GetOutStream()))
    vm.ThrowRunError(CAOSMachine::sidCouldNotDumpLobe);
}

void CreatureHandlers::SubCommand_BRN_DMPT(CAOSMachine &vm) {
  if (!vm.GetCreatureTarg().GetBrain()->DumpTract(vm.FetchIntegerRV(),
                                                  *vm.GetOutStream()))
...
void CreatureHandlers::SubCommand_BRN_DMPN(CAOSMachine &vm) {
  ...
  if (!vm.GetCreatureTarg().GetBrain()->DumpNeuron(lobe, neuron,
                                                   *vm.GetOutStream()))
...
void CreatureHandlers::SubCommand_BRN_DMPD(CAOSMachine &vm) {
  int tract = vm.FetchIntegerRV();
  int dendrite = vm.FetchIntegerRV();
  if (!vm.GetCreatureTarg().GetBrain()->DumpDendrite(tract, dendrite,
                                                     *vm.GetOutStream()))
...
void CreatureHandlers::Command_GENE(CAOSMachine &vm) {
  ...
  static CommandHandler HandlerTable[subcount] = {
      SubCommand_GENE_CROS, SubCommand_GENE_MOVE, SubCommand_GENE_KILL,
      SubCommand_GENE_LOAD, SubCommand_GENE_CLON,
  };
  int subcmd;
...
void CreatureHandlers::SubCommand_GENE_CROS(CAOSMachine &vm) {
  ...
  if (mumChance < 0 || mumChance > 255 || mumDegree < 0 || mumDegree > 255 ||
    ...
    if (!child.GetAgentReference().GetGenomeStore().CrossoverFrom(
            child_index, mum.GetAgentReference().GetGenomeStore(), mum_index,
            dad.GetAgentReference().GetGenomeStore(), dad_index, mumChance,
            mumDegree, dadChance, dadDegree, false)) {
      vm.ThrowRunError(CAOSMachine::sidInvalidGeneVariable);
    }
  }

  // note: requires exception handling if concieve fails cos cant create mum or
  // dad
  //		genomes
}
...
void CreatureHandlers::SubCommand_GENE_MOVE(CAOSMachine &vm) {
  ...
  if (!dest.GetAgentReference().GetGenomeStore().MoveSlotFrom(
          dest_index, source.GetAgentReference().GetGenomeStore(),
          source_index)) {
    vm.ThrowRunError(CAOSMachine::sidInvalidGeneVariable);
  }
}
...
void CreatureHandlers::SubCommand_GENE_LOAD(CAOSMachine &vm) {
  ...
  if (!load.GetAgentReference().GetGenomeStore().LoadEngineeredFile(load_index,
                                                                    gene_file))
...
void CreatureHandlers::SubCommand_GENE_CLON(CAOSMachine &vm) {
  ...
  if (!dest.GetAgentReference().GetGenomeStore().CloneSlot(
          dest_index,
          source.GetAgentReference().GetGenomeStore().MonikerAsString(
              source_index))) {
    vm.ThrowRunError(CAOSMachine::sidInvalidGeneVariable);
  }
}
...
void CreatureHandlers::Command_NOHH(CAOSMachine &vm) {
  ...
  vm.ValidateTarg();
  vm.GetCreatureTarg().StopHoldingHandsWithThePointer(vm.GetOwner(),
