...
void CompoundHandlers::SubCommand_PAT_DULL(CAOSMachine &vm) {
  ...
  // the main part
  int numImages = 0;
  part = new CompoundPart(FilePath(gallery + ".s16", IMAGES_DIR), baseimage,
                          position, relplane, numImages);
...
void CompoundHandlers::SubCommand_PAT_BUTT(CAOSMachine &vm) {
  ...
  part =
      new UIButton(FilePath(gallery + ".s16", IMAGES_DIR), baseImage, numImages,
                   position, relplane, hoverAnim, messageID, option);
...
void CompoundHandlers::SubCommand_PAT_TEXT(CAOSMachine &vm) {
  ...
  // EntityImage Constructor doesn't use numimages (should tidy up param lists).
  part = new UIText(FilePath(gallery + ".s16", IMAGES_DIR), baseimage, 0,
                    position, relplane, messageID, fontSprite);
...
void CompoundHandlers::SubCommand_PAT_FIXD(CAOSMachine &vm) {
  ...
  // EntityImage Constructor doesn't use numimages (should tidy up param lists).
  part = new UIFixedText(FilePath(gallery + ".s16", IMAGES_DIR), baseimage, 0,
                         position, relplane, fontSprite);
...
void CompoundHandlers::SubCommand_PAT_GRPH(CAOSMachine &vm) {
  ...
  // EntityImage Constructor doesn't use numimages (should tidy up param lists).
  part = new UIGraph(FilePath(gallery + ".s16", IMAGES_DIR), baseimage, 0,
                     position, relplane, numValues);
...
void CompoundHandlers::SubCommand_PAT_CMRA(CAOSMachine &vm) {
  ...
  // EntityImage Constructor doesn't use numimages (should tidy up param lists).
  part = new CameraPart(FilePath(gallery + ".s16", IMAGES_DIR), baseImage,
                        numImages, position, relplane, viewWidth, viewHeight,
...
void CompoundHandlers::SubCommand_PAT_MOVE(CAOSMachine &vm) {
  ...
  int partid = vm.FetchIntegerRV();
  int relx = vm.FetchIntegerRV();
  int rely = vm.FetchIntegerRV();

  CompoundAgent &agent = vm.GetTarg().GetCompoundAgentReference();
  if (!agent.ValidatePart(partid))
    vm.ThrowRunError(CAOSMachine::sidInvalidPart, partid);

  CompoundPart *part = agent.GetPart(partid);
  part->SetRelX((float)relx);
  part->SetRelY((float)rely);
  Vector2D pos = agent.GetPosition();
  agent.MoveTo(pos.x, pos.y);
  (void)pos; // pos.x and pos.y already used
}
