...
public:
  ...
  static void Command_ALPH(CAOSMachine &vm); // DS: set agent alpha (stub)
  ...
  static void Command_CALL(CAOSMachine &vm); // DS: send message event to targ
  static void Command_DROP(CAOSMachine &vm); // DS: drop held object (stub)
  ...
  static void StringRV_GALL(CAOSMachine &vm,
                            std::string &str); // get current gallery name
  static void StringRV_LIMB(
      CAOSMachine &vm,
      std::string &str); // DS: creature body-part gallery lookup (stub)
  ...
  // -----------------------------------------------------------------------
  // Testable logic layer — accept IWorldServices& so tests can inject
  // a FakeWorld without a live App.
  // -----------------------------------------------------------------------

  // MESG WRIT / WRT+ / CALL: push a message onto the world queue
  static void SendMessage(IWorldServices &world, AgentHandle const &from,
                          AgentHandle const &to, int msgid, CAOSVar const &p1,
                          CAOSVar const &p2, unsigned delay);

  // NORN: return the currently selected creature
  static AgentHandle GetSelectedCreature(IWorldServices &world);
