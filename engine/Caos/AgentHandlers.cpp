...
void AgentHandlers::Command_PERM(CAOSMachine &vm) {
  ...
  if (!ok) {
    theFlightRecorder.Log(1, "PERM: invalid map position (%f, %f) for agent %d",
...
void AgentHandlers::SubCommand_MESG_WRIT(CAOSMachine &vm) {
  ...
  SendMessage(theApp.GetWorld(), vm.GetOwner(), to, msgid, INTEGERZERO,
              INTEGERZERO, 0);
}
...
void AgentHandlers::SubCommand_MESG_WRT(CAOSMachine &vm) {
  ...
  SendMessage(theApp.GetWorld(), vm.GetOwner(), to, msgid, p1, p2, delay);
}
...
void AgentHandlers::Command_ATTR(CAOSMachine &vm) {
  ...
  if (!ok) {
    theFlightRecorder.Log(1, "ATTR: invalid map position (%f, %f) for agent %d",
                          vm.GetTarg().GetAgentReference().GetPosition().x,
                          vm.GetTarg().GetAgentReference().GetPosition().y,
                          vm.GetTarg().GetAgentReference().GetUniqueID());
  }
...
// DS-only: ALPH alpha_value enable_flag
// Sets per-agent alpha level.  Alpha blending is not implemented on this
// SDL port, so we consume the arguments and do nothing.
void AgentHandlers::Command_ALPH(CAOSMachine &vm) {
  vm.FetchIntegerRV(); // alpha value (0-256)
  vm.FetchIntegerRV(); // enable flag (0 or 1)
  // no-op: alpha blending unsupported on SDL1 16-bit surface
}
...
// DS: GALL as string RV - returns the current gallery/sprite filename stem
// (without the .s16/.c16 extension) for the target agent/part.
// Previously stubbed with ""; now delegates to Agent::GetGalleryName so
// scripts that use gall to create indicator agents get a valid filename.
void AgentHandlers::StringRV_GALL(CAOSMachine &vm, std::string &str) {
  vm.ValidateTarg();
  str = vm.GetTarg().GetAgentReference().GetGalleryName(vm.GetPart());
}

// DS: LIMB body_part genus gender age variant -> gallery filename
// Returns the sprite file name for a creature body part.
// Offline stub returns "" (no network creature gallery resolution).
void AgentHandlers::StringRV_LIMB(CAOSMachine &vm, std::string &str) {
  vm.FetchIntegerRV(); // body_part
  vm.FetchIntegerRV(); // genus
  vm.FetchIntegerRV(); // gender
  vm.FetchIntegerRV(); // age
  vm.FetchIntegerRV(); // variant
  str = "";
}
...
AgentHandle AgentHandlers::AgentRV_NORN(CAOSMachine &vm) {
  return GetSelectedCreature(theApp.GetWorld());
}
...
// CALL event_id p1 p2 -- call script event on targ with parameters.
// Equivalent to MESG WRT+ targ event_id p1 p2 0 (no delay).
...
// Pointer carry not yet implemented; stub that does nothing.
void AgentHandlers::Command_DROP(CAOSMachine &vm) {
  // DS: drop held object -- stub (pointer carry not yet implemented)
}
