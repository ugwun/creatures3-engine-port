...
bool World::Write(CreaturesArchive &ar) const {
  ...
  ar << myLoadedAuxBootstrapFolders;
  ar << myGameEndTime;
  ar << myLastPlayLength;
...
bool World::Load(std::string const &worldName, bool loadBackup) {
  ...
  if (FileExists(fullPath.c_str()) == false)
  {
    ...
    if (worldName == "Startup") {
      ...
      myLoadedBootstrapFolders.clear();
...
void World::InitialiseFromGameVariables() {
  ...
  if (seasonLength.GetType() != CAOSVar::typeInteger)
    return;
  ...
  if (numseasons.GetType() != CAOSVar::typeInteger)
    return;
  ...
  if (volume.GetType() != CAOSVar::typeInteger)
    return;
  ...
  if (dropAll.GetType() != CAOSVar::typeInteger)
    return;
...
void World::CheckForNewNewlyInstalledProductsOrAddOns() {
  theFlightRecorder.Log(4,
                        "CheckForNewNewlyInstalledProductsOrAddOns: starting");
  // ── Primary (DS) bootstrap dir ───────────────────────────────
  FilePath primaryPath("", BOOTSTRAP_DIR);
  std::string primaryBase = primaryPath.GetFullPath();

  std::vector<std::string> unloadedPrimary;
  GetMissingDirsInDirectory(primaryBase, myLoadedBootstrapFolders,
                            unloadedPrimary);
  std::sort(unloadedPrimary.begin(), unloadedPrimary.end());

  // ── Auxiliary (C3) bootstrap dir ─────────────────────────────
  // Only scan auxiliary (C3) bootstrap if the world is "docked".
  // The DS world switcher sets eame "engine_no_auxiliary_bootstrap_1" to 1
  // for undocked worlds — meaning "skip C3 bootstrap".  When the flag
  // is 0 (docked) or unset (legacy/first boot), we load C3 content.
  std::vector<std::string> unloadedAux;
  const char *auxBase = theApp.GetAuxiliaryDirectory(BOOTSTRAP_DIR);
  std::string auxStr;
  bool skipAuxBootstrap = false;
  {
    CAOSVar &eameFlag = theApp.GetEameVar("engine_no_auxiliary_bootstrap_1");
    if (eameFlag.GetType() == CAOSVar::typeInteger &&
        eameFlag.GetInteger() == 1) {
      skipAuxBootstrap = true;
      theFlightRecorder.Log(4,
          "CheckForNewNewlyInstalledProductsOrAddOns: "
          "skipping auxiliary bootstrap (engine_no_auxiliary_bootstrap_1 = 1)");
    }
  }
  if (auxBase && !skipAuxBootstrap) {
    auxStr = auxBase;
    GetMissingDirsInDirectory(auxStr, myLoadedAuxBootstrapFolders, unloadedAux);
    std::sort(unloadedAux.begin(), unloadedAux.end());
  } else if (auxBase && skipAuxBootstrap) {
    // Undocked world: don't load C3 bootstrap scripts, but still register
    // the folder names as "already processed".  Without this, the empty
    // myLoadedAuxBootstrapFolders would cause World::Read() to re-detect
    // the C3 dirs as "new" on every reload and re-run !map.cos, crashing
    // because the C3 metarooms don't exist in the saved world.
    auxStr = auxBase;
    std::vector<std::string> auxDirsToSuppress;
    GetMissingDirsInDirectory(auxStr, myLoadedAuxBootstrapFolders,
                              auxDirsToSuppress);
    for (const auto &folder : auxDirsToSuppress) {
      myLoadedAuxBootstrapFolders.push_back(folder);
    }
    std::sort(myLoadedAuxBootstrapFolders.begin(),
              myLoadedAuxBootstrapFolders.end());
    theFlightRecorder.Log(4,
        "CheckForNewNewlyInstalledProductsOrAddOns: "
        "registered %d auxiliary dirs as suppressed (undocked)",
        (int)auxDirsToSuppress.size());
  }

  // ── Build merged sorted list of ABSOLUTE paths ───────────────
  // Skip "000 Switcher" dirs — DS's switcher already ran.
  struct BootDir {
    std::string absPath;
    std::string folderName;
  };
  std::vector<BootDir> merged;

  for (const auto &folder : unloadedAux) {
    if (folder == "000 Switcher")
      continue;
    merged.push_back({auxStr + folder + "/", folder});
  }
  for (const auto &folder : unloadedPrimary) {
    if (folder == "000 Switcher")
      continue;
    merged.push_back({primaryBase + folder + "/", folder});
  }

  // Sort by folder name only to get correct cross-game order:
  // "001 World" (C3) before "010 Docking Station" (DS)
  std::sort(merged.begin(), merged.end(),
            [](const BootDir &a, const BootDir &b) {
              return a.folderName < b.folderName;
            });

  std::vector<std::string> absDirsToLoad;
  for (const auto &bd : merged) {
    absDirsToLoad.push_back(bd.absPath);
  }

  if (!absDirsToLoad.empty()) {
    for (size_t i = 0; i < absDirsToLoad.size(); ++i) {
      theFlightRecorder.Log(4, "Bootstrap dir [%d]: %s", (int)i,
                            absDirsToLoad[i].c_str());
    }
    CosInstaller cos(absDirsToLoad, true);
  } else {
    theFlightRecorder.Log(4, "No bootstrap dirs to load!");
  }

  // ── Register newly loaded folders ────────────────────────────
  for (const auto &folder : unloadedPrimary) {
    myLoadedBootstrapFolders.push_back(folder);
  }
  ...
  // ── magic.cos (unchanged) ───────────────────────────────────
  FilePath worldPath(myName, WORLDS_DIR);
  std::string fullPath = worldPath.GetFullPath() + "/magic.cos";
