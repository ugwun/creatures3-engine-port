...
AgentHandle
Agent::IsTouching(uint32 AttrFields, // which Attr bit(s) I care about
                  uint32 AttrState)  // what state I want them to be
{
  _ASSERT(!myGarbaged);
  Vector2D ptThis;
...
void Agent::SoundEffect(uint32 fsp, int delay) {
  _ASSERT(!myGarbaged);
...
void Agent::ControlledSound(uint32 fsp, bool loop) {
  _ASSERT(!myGarbaged);
...
bool Agent::Write(CreaturesArchive &ar) const {
  ...
  //	uint32 mySoundName;			// Name of active sound (0=none)
