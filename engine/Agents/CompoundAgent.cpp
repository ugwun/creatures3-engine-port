...
bool CompoundAgent::AddPart(int id, CompoundPart *part) {
  ...
  if (id != 0) {
    ...
    part->ChangeCameraShyStatus((GetAttributes() & attrCameraShy) ==
                                attrCameraShy);
  } else {
...
void CompoundAgent::Trash() {
  ...
  // Guard: if construction failed before any parts were added,
  // myParts will be empty — skip part deletion entirely.
  if (myParts.empty()) {
    base::Trash();
    return;
  }
...
void CompoundAgent::Init() { myNormalBasePlaneForPartZero = 0; }
...
CompoundAgent::CompoundAgent(int family, int genus, int species, uint32 id,
  ...
  try {
    ...
    CompoundPart *part0 =
        new CompoundPart(gallery, baseimage, zero, plane, numimages);
    AddPart(0, part0);
    ...
    myFailedConstructionException =
        "NLE0010: Unknown exception caught in compound agent constructor";
  }
}
...
bool CompoundAgent::HitTest(const Vector2D &point) {
  _ASSERT(!myGarbaged);
...
int CompoundAgent::GetPlane(int part) {
  ...
  if (part == 0)
    ...
    return myParts[part]->GetEntity()->GetPlane() -
           myParts[0]->GetEntity()->GetPlane();
}
...
int CompoundAgent::ClickAction(int x, int y) // xy in world coords
{
  ...
  if (myDefaultClickAction != -1) {
    // Think about this... If we are over an important UIPart, then we are also
    // -1
    for (int i = 0; i < myParts.size(); ++i) {
      if (!ValidatePart(i))
        continue;
      ...
      if (part->GetType() &
          (CompoundPart::partEdit | CompoundPart::partButton)) {
        ...
        if ((clickX > partX) || (clickY > partY))
          continue;
        return -1; // This is inside the entity of a UI button or text edit
                   // field.
      }
    }
  }
...
// virtual
void CompoundAgent::DrawLine(int32 x1, int32 y1, int32 x2, int32 y2,
  ...
  */
  myParts[bestpart]->DrawLine(x1, y1, x2, y2, lineColourRed, lineColourGreen,
                              lineColourBlue, stippleon, stippleoff,
                              stippleStart);
  myResetLines = true;
}
...
void CompoundAgent::SetGallery(FilePath const &galleryName, int baseimage,
                               int part) {
  _ASSERT(!myGarbaged);
