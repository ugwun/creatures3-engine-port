...
// -------------------------------------------------------------------------
// Class: Agent
// The core entity object in the game. *Probably* everything that exists
// and interacts in the world (creatures, items, vehicles) derives from this.
// It tracks positions, velocities, unique IDs, CAOS variables (state),
// and ports for input/output events.
// -------------------------------------------------------------------------
...
public:
  ...
  // Per-agent string-keyed NAME variables (e.g. name "status").
  // Default type is integer 0 (CAOSVar default). Scripts may store either
  // numeric (setv name "waiting" 1) or string (sets name "wtype" "undocked")
  // values — the type is set by whatever SETV/SETS/SETA writes first.
  CAOSVar &GetNameVar(const std::string &key) {
    _ASSERT(!myGarbaged);
    return myNamedVariables[key];
  }
  ...
  AgentHandle IsTouching(uint32 AttrFields, uint32 AttrState);
  ...
  // agents 				carrying me.  (Default value is to be
  ...
  // Returns the gallery name (file stem, no extension) for the given part.
  // Returns "" if no entity image is present or gallery was never set.
  virtual std::string GetGalleryName(int part = 0);
...
protected:
  ...
  uint32 mySoundName;         // Name of active sound (NULL=none)
  bool mySoundLooping;       // Controlled sound is looping

  AgentHandle mySelf;

  CAOSVar myGlobalVariables[GLOBAL_VARIABLE_COUNT]; // OV00..OV99
  std::map<std::string, CAOSVar> myNamedVariables;  // string-keyed NAME vars
