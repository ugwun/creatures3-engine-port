...
enum { CREATURE = 0x01, POINTER = 0x02 };
...
public:
  ...
  CompoundAgent(int family, int genus, int species, uint32 id,
                FilePath const &gallery, int numimages, int baseimage,
                int plane);
  ...
  virtual bool SetBaseImage(int image,
                            int partid = 0); // default helper fn for BASE macro
