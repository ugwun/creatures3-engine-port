...
void PointerAgent::Tint(const uint16 *tintTable, int part) {
  ...
  if (myEntityImage) {
    ...
    ClonedEntityImage *clone = new ClonedEntityImage(
        myEntityImage->GetGallery()->GetName(), myEntityImage->GetPlane(),
...
PointerAgent::PointerAgent(int family, int genus, int species, uint32 id,
                           FilePath const &gallery, int numimages,
                           int baseimage)
  ...
  try {
    ...
    myAgentType = AgentHandle::agentNormal | AgentHandle::agentSimple |
                  AgentHandle::agentPointer;

    theMainView.AddFloatingThing(mySelf);

    myName = SensoryFaculty::GetCategoryName(
        SensoryFaculty::GetCategoryIdOfClassifier(&myClassifier));
  } catch (BasicException &e) {
    myFailedConstructionException = e.what();
  } catch (...) {
    myFailedConstructionException =
        "NLE0009: Unknown exception caught in pointer constructor";
  }
}
...
void PointerAgent::Update() {
  ...
  if (myConnecting) {
    if (myConnectingAgent.IsInvalid()) {
      ...
      myOutputPort = NULL;
      myResetLines = true;
      ExecuteScriptForEvent(SCRIPTPOINTERPORTCANCEL, mySelf, INTEGERZERO,
                            INTEGERZERO);
      return;
    }
  }
  ...
  if (myConnecting) {
    ...
    DrawLine(Map::FastFloatToInteger(mousePosition.x),
             ...
             Map::FastFloatToInteger(portPosition.y), r, g, b, 4, 4,
             myMarchingAntCount);
  ...
  // Uncomment this to see what a waste of time it would be to re-implement fast
  // entities
  ...
  // OutputFormattedDebugString("The pointer is over tiles: (%d,%d) to
  ...
  for (i = 0; i < im.GetEventCount(); ++i) {
    ...
    if (ev->EventCode == InputEvent::eventMouseDown &&
        ...
        (ev->EventCode == InputEvent::eventMouseDown &&
         ev->MouseButtonData.button == InputEvent::mbLeft &&
         TreatBothButtonsAsRightClick())) {
      // Start RMB
      if (myIsHoldingHandsWithCreature) {
        if (myHandHeldCreature.IsValid()) {
          // Write the normal message to the clicked-on agent
          p1.SetFloat(myPositionVector.x -
                      myHandHeldCreature.GetAgentReference().GetPosition().x);
          p2.SetFloat(myPositionVector.y -
                      myHandHeldCreature.GetAgentReference().GetPosition().y);
          theApp.GetWorld().WriteMessage(mySelf, myHandHeldCreature,
                                         STOP_HOLD_HANDS, p1, p2, 0);
          return;
        }
      }
      ...
      // (note: a mouseable agent may be behind some scenery which would have
      // been
      //  found by Find() above)
      a = IsTouching(attrMouseable, attrMouseable);
      if (a.IsValid()) {
        theApp.GetWorld().WriteMessage(mySelf, a, PICKUP, INTEGERZERO,
                                       INTEGERZERO, 0);
      }
      // End RMB
    }
    ...
    if (ev->EventCode == InputEvent::eventMouseDown &&
        ...
        if (port) {
          ...
          if (inputnotoutput) {
            ...
            if (myInputPort != NULL) {
              ...
              ExecuteScriptForEvent(SCRIPTPOINTERPORTERROR, mySelf, INTEGERZERO,
                                    INTEGERZERO);
              return;
            }
            if (myOutputPort) {
            ...
            if (myInputPort) {
              ...
              bool dummy;
              bool valid =
                  myInputPort->GetOwner().GetAgentReference().IsConnectionValid(
      ...
      if (a.IsValid()) {
        ...
        if (msgid != -1) {
          ...
          // Write the normal message to the clicked-on agent
          p1.SetFloat(myPositionVector.x -
                      a.GetAgentReference().GetPosition().x);
          p2.SetFloat(myPositionVector.y -
                      a.GetAgentReference().GetPosition().y);
          // If this is a creature, then don't send the message. (pointer
          // scripts send them)
          if (!a.IsCreature())
...
bool PointerAgent::Write(CreaturesArchive &archive) const {
  ...
  base::Write(archive);
  archive << myHotSpot << myConnecting << myInputPort << myOutputPort << myPort
          << myConnectingAgent << myIsHoldingHandsWithCreature
          << myHandHeldCreature << myScanInputEvents << myMouseButtonAction
          << myLastPointerActionDispatched << myName;
...
bool PointerAgent::Read(CreaturesArchive &archive) {
  ...
  if (version >= 3) {
    ...
    archive >> myHotSpot >> myConnecting >> myInputPort >> myOutputPort >>
        myPort >> myConnectingAgent >> myIsHoldingHandsWithCreature >>
        myHandHeldCreature >> myScanInputEvents >> myMouseButtonAction >>
        myLastPointerActionDispatched >> myName;
  } else {
    _ASSERT(false);
...
void PointerAgent::SetName(std::string name) {
  ...
  for (int c = 0; c != noCreatures; c++) {
    LinguisticFaculty &l =
        *creatureCollection[c].GetCreatureReference().Linguistic();
