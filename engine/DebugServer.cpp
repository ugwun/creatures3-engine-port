	...
	myImpl->svr.Post("/api/genetics/inject", [this, writeGenome](const httplib::Request& req, httplib::Response& res) {
		...
		item->work = [body, writeGenome]() -> std::string {
			try {
                ...
                int gender = j.value("gender", 0);  // 0=random, 1=male, 2=female
                std::string sexStr = std::to_string(gender);
                std::string cmd;
                if (injectMode == "egg") {
                    // For eggs, ov01 holds the sex: 1=male, 2=female
                    std::string ov01Line;
                    if (gender == 0) {
                        ov01Line = "setv ov01 rand 1 2 ";  // random gender
                    } else {
                        ov01Line = "setv ov01 " + sexStr + " ";  // explicit gender
                    }
                    cmd =
                    ...
                    cmd =
                        ...
                        "new: crea 4 targ 1 " + sexStr + " 0 "  // hatch with selected sex
