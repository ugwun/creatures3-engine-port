...
class World : public IWorldServices {
public:
  void Init();

  Map &GetMap() { return myMap; };
  // IWorldServices interface
  virtual Scriptorium &GetScriptorium() override { return myScriptorium; }
  ...
  // (GetScriptorium declared above as override)
  ...
  bool SetMetaRoom(uint32 metaRoomID, uint32 transitionType = 0,
                   ...
                   bool bCentre = false);
  virtual uint32 GetWorldTick() const override { return myWorldTick; }
  ...
  // globally
  //				available, string-indexed variables are stored
  // in the 				World object. If GetGameVar is called
  // for a non-existant 				var, a new var is
  // created (contents undefined).
  // Arguments:	name - name of variable.
  // Returns:		a reference to a CAOSVar (which can hold int, float,
  //				string or agent).
  // ------------------------------------------------------------------------
  virtual CAOSVar &GetGameVar(const std::string &name) override;
  ...
  // Returns:
  // ------------------------------------------------------------------------
  virtual void DeleteGameVar(const std::string &name) override;
  virtual std::string GetNextGameVar(const std::string &name) override;

  virtual AgentHandle GetSelectedCreature() override;
  virtual void SetSelectedCreature(AgentHandle &h) override;
  ...
  virtual int GetTimeOfDay(bool useCurrentTime = true,
                           uint32 worldTick = 0) override;
  virtual int GetSeason(bool useCurrentTime = true,
                        uint32 worldTick = 0) override;
  ...
  //				immediate dispatch( on the next tick) or delayed
  ...
  // (Unfortunately we can't make it a 				parameter to the
  // constructor, because it could call 				CAOS
  // commands from the boostrap, and some of those require
  ...
  virtual int WorldCount() override;
  virtual bool WorldName(int index, std::string &name) override;

  virtual std::string GetWorldName() const override { return myName; }
  virtual std::string GetUniqueIdentifier() const override {
    return myUniqueIdentifier;
  }

  bool IsStartUpWorld();

  virtual bool GetPausedWorldTick() const override { return myPausedWorldTick; }
  virtual void SetPausedWorldTick(bool paused) override {
    myPausedWorldTick = paused;
  }
  ...
  virtual HistoryStore &GetHistoryStore() override { return myHistoryStore; }
...
private:
  ...
  std::vector<std::string> myLoadedAuxBootstrapFolders;
