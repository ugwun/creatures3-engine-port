...
class Map : public PersistentObject, public IMap {
...
public:
  ...
  bool GetMetaRoomDefaultCameraLocation(const int metaRoomID, int32 &positionX,
                                        int32 &positionY);
  ...
  bool GetRoomIDWithHighestCA(int roomID, int caIndex, bool includeUpDown,
                              int &roomIDHighest);

  bool GetRoomIDWithLowestCA(int roomID, int caIndex, bool includeUpDown,
                             int &roomIDLowest);
...
template <typename D>
inline void Map::UpdateDoorCABetweenRooms(const D &doorOrLink) {
...
template <typename D> inline void Map::CalculateDoorDoorage(D &doorOrLink) {
