...
// provides for line of sight 				testing; agent/creature
// collision testing; agent/creature 				location
// testing; agent/creature physics; serialization.
//
...
// Constructor
//
Map::Map(void) { Initialise(); }

//
// Destructor
//
Map::~Map(void) { Cleanup(); }
...
bool Map::DoesPathIntersectLine(
  ...
  float th =
      acos(deltaPath.DotProduct(deltaLine) / (pathLength * deltaLine.Length()));
  if (th > HALFPI)
    th = PI - th;
...
// Returns:		true if there is an intersection; false otherwise
// ---------------------------------------------------------------------------
bool Map::DoesPathCrossOrTouchLine(const Vector2D &startPath,
...
//				positionMin - minimum point of the bounding box
//(out) 				positionMax - maximum point of the
// bounding box (out)
// Returns:		None
// ---------------------------------------------------------------------------
void Map::GetLineBoundingBox(const Vector2D &start, const Vector2D &end,
...
// ---------------------------------------------------------------------------
bool Map::IsPointOnLine(const Vector2D &position, const Vector2D &start,
  ...
  if (delta.x != 0.0f) {
    ...
    if ((position.x < positionMin.x) || (position.x > positionMax.x))
      return false;
...
// ---------------------------------------------------------------------------
bool Map::IsPointBelowCeilingDoors(const Vector2D &position, Room *room) {
  ...
  // Point x is not covered by any ceiling door segment — this can happen
  // at exact room boundaries due to floating-point rounding.  The original
  // Windows release build compiled this assertion out via NDEBUG.
  return false;
}
...
// ---------------------------------------------------------------------------
bool Map::IsPointAboveFloorDoors(const Vector2D &position, Room *room) {
  ...
  // Point x is not covered by any floor door segment — see ceiling comment.
  return false;
}
...
// Returns:		None
// ---------------------------------------------------------------------------
void Map::GetGridIndexesForPoint(const Vector2D &position, int &squareX,
                                 int &squareY) {
...
//				roomCount - how many rooms contain the point
//(out) 				roomID - the "primary" room ID of those
// found (out)
// Returns:		true if good input, false otherwise
// Notes:		A point can be in a maximum of four rooms when it lies
// on a
//				vertex. The primary ID is the leftmost/topmost.
// ---------------------------------------------------------------------------
bool Map::GetRoomInformationForPoint(const Vector2D &position,
                                     int roomIDList[4], Room *roomList[4],
                                     int &roomCount, int &roomID) {
  int xSquare, ySquare;
...
bool Map::IsPointInsideRoom(const Vector2D &position, Room *room) {
  ...
  if ((position.x > xMin) && (position.x < xMax)) {
    // Check against the ceiling
    below = IsPointBelowLine(position, room->startCeiling, room->deltaCeiling,
                             yBelow);
    if (below && (yBelow > 0.5f)) {
      // Check against the floor
      above = IsPointAboveLine(position, room->startFloor, room->deltaFloor,
                               yAbove);
...
// ---------------------------------------------------------------------------
bool Map::IsRegionInsideSameRoom(const Vector2D &positionMin,
...
//				xSquareMax, ySquareMax - max grid square of
// region (in)
// Returns:		true if inside same room
// Notes:		myTempRoomCollection stores the rooms
//				myTempRoomCount stores the room count
// ---------------------------------------------------------------------------
void Map::GetRoomsOverlappingRegion(const int xSquareMin, const int ySquareMin,
...
//				roomID - the adjacent room ID or -1 if none
//(out)
// Returns:		true if position is valid, false if not
// ---------------------------------------------------------------------------
bool Map::SearchForAdjacentRoom(const Vector2D &position, const int direction,
                                int &roomID) {
  int roomIDList[4];
  Room *roomList[4];
  int roomCount;
...
void Map::CalculateSlideAccelerationAndSlideVelocity(
    const Vector2D &velocity, const Vector2D &slope, const Vector2D &gravity,
    Vector2D &slideAcceleration, Vector2D &slideVelocity) {
...
//				velocityCollision - velocity at collision point
//(out) 				timeElapsed - how much time used up by
// the move as a number 					in the range
// [0,1] (out)
// Returns:		None
// ---------------------------------------------------------------------------
void Map::CalculateCollisionVelocityAndElapsedTime(
    const Vector2D &velocity, const Vector2D &acceleration,
    const Vector2D &path, const Vector2D &deltaTravelled,
    Vector2D &velocityCollision, float &timeElapsed) {
  //
  ...
  if (acceleration.x == 0.0f) {
    ...
    float temp =
        velocity.x * velocity.x + 2.0f * acceleration.x * deltaTravelled.x;
    _ASSERT(temp >= 0.0f);
    float vx = sqrtf(temp);
    if (path.x < 0.0f)
      vx = -vx;
  ...
  if (acceleration.y == 0.0f) {
    ...
    float temp =
        velocity.y * velocity.y + 2.0f * acceleration.y * deltaTravelled.y;
    _ASSERT(temp >= 0.0f);
    float vy = sqrtf(temp);
    if (path.y < 0.0f)
      vy = -vy;
...
//				useEdgeDelta - true to use "edgeDelta" parameter
//(in) 				edgeDelta - agents striking a spike along an
// edge of 					their diamond are best reflected
// about that edge. This 					gives the delta
// of the edge vector. 				minDoorPermiability - minimum
// door permiability (in)
...
// Returns:		None
// ---------------------------------------------------------------------------
void Map::GetReflectionFromVertex(const Vector2D &positionVertex,
  ...
  ok = GetRoomInformationForPoint(positionVertex, roomIDList, roomList,
                                  roomCount, dummy1);
  if (!ok) {
    // Vertex is outside the room system (e.g. newly-created world with no
    // map data). Gracefully return a trivial reflection rather than crashing.
    vReflection = v;
    spike = false;
    return;
  }
  ...
  // Guard: if no wall edges were found at this vertex, bail out safely.
  // Degenerate map geometry (e.g. an isolated vertex) can produce count==0;
  // returning here leaves vReflection unchanged from its caller-initialised
  // value and prevents the abort() / SIGABRT crash.
  if (count == 0)
    return;
  ...
  // Guard: degenerate geometry can produce a zero-length mirror in some edge
  // cases (e.g. two parallel unit vectors with equal opposite angles produce
  // right - left == 0). Return a safe vertical-bounce reflection rather than
  // SIGABRT.
  if (mirror.x == 0.0f && mirror.y == 0.0f) {
    vReflection = Vector2D(v.x, -v.y); // bounce off floor normal
    spike = false;
    return;
  }
...
//				minDoorPermiability - minimum door permiability
//(in) 				reached - true if path reaches the boundary
//(out) 				positionReached - where the path touches
// the boundary (out) 				doorReached - which door the
// path reached, or NULL if path 					passes
// through a vertex (out) 				blocked - true if the
// path is blocked (out) 				throughVertex - true if
// the path goes through a vertex (out)
// Returns:		None
// ---------------------------------------------------------------------------
void Map::DoesPathReachRoomBoundary(Room *room, const Vector2D &start,
                                    const Vector2D &end, const Vector2D &delta,
                                    const int minDoorPermiability,
                                    bool &reached, Vector2D &positionReached,
                                    Door *&doorReached, bool &blocked,
                                    bool &throughVertex) {
  Door *door;
  ...
  while (count-- > 0) {
    ...
    if ((position == door->start) || (position == door->end))
...
//				delta - delta of path (in)
//				minDoorPermiability - minimum door permiability
//(in) 				reached - true if path reaches a boundary (out)
...
// crossed boundary (out) 				blocked - true if the
// path is blocked (out) 				throughVertex - true if
// the path goes through a vertex (out)
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::DoesPathReachSurroundingRoomBoundary(
  ...
  if (!GetRoomInformationForPoint(start, roomIDList, roomList, roomCount,
                                  dummy1))
  ...
  for (i = 0; i < roomCount; ++i) {
    ...
    DoesPathReachRoomBoundary(roomList[i], start, end, delta,
                              minDoorPermiability, reachedRoom, position, door,
...
//				end - second point (in)
//				minDoorPermiability - minimum door permiability
//(in) 				canSee - true if point can see the other point
//(out)
// Returns:		true if good input, false otherwise
// --------------------------------------------------------------------------
bool Map::CanPointSeePoint(const Vector2D &start, const Vector2D &end,
  ...
  if (!DoesPathReachSurroundingRoomBoundary(
          start, end, delta, minDoorPermiability, reached, temp, door,
          previousRoomID, blocked, throughVertex))
  ...
  while (tempDelta.SquareLength() >= 0.01f) {
    if (throughVertex) {
      (void)DoesPathReachSurroundingRoomBoundary(
          temp, end, tempDelta, minDoorPermiability, reached, position, door,
      ...
      DoesPathReachRoomBoundary(myRoomCollection[roomID], temp, end, tempDelta,
...
//				minDoorPermiability - minimum door permiability
//(in) 				positionSafe - a safe place to locate the agent
//(out)
// Returns:		If a safe place could be found
// ---------------------------------------------------------------------------
bool Map::FindSafeAgentLocation(const Vector2D &positionPreferred,
...
//				currentDownFoot - which foot is currently down;
// 0 is 					left, 1 is right (in)
// minDoorPermiability - minimum door permiability (in)
// downFootSafe - a safe place to locate the creature (out)
// Returns:		If a safe place could be found
// ---------------------------------------------------------------------------
bool Map::FindSafeCreatureLocation(const Vector2D &footLeft,
...
// --------------------------------------------------------------------------
bool Map::ArePointsInRectangle(Vector2D points[4], const int pointCount,
...
//				positionMin - the minimum x and y of all the
// lines (in) 				positionMax - the maximum x and y of all
// the lines (in) 				minDoorPermiability - minimum
// door permiability (in)
// lineCount - how many lines (in)
// Returns:		true if all lines are valid
// --------------------------------------------------------------------------
bool Map::AreLinesValidInRoomSystem(Vector2D starts[4], Vector2D ends[4],
  ...
  if (!GetRoomInformationForPoint(starts[1], dummy1, dummy2, dummy3, dummy4))
    return false;
  ...
  for (i = 0; i < myTempRoomCount; ++i) {
    ...
    while (count-- > 0) {
      ...
      for (j = 0; j < lineCount; ++j) {
        if (DoesPathCrossOrTouchLine(starts[j], ends[j], deltas[j], door->start,
                                     door->end, door->delta, dummy))
          return false;
      }
    }
  }
...
// ---------------------------------------------------------------------------
bool Map::IsAgentLocationValidInRoomSystem(const Vector2D &position,
                                           const float width,
                                           const float height,
                                           const int minDoorPermiability) {
...
// Returns:		true if agent location valid; false otherwise
// ---------------------------------------------------------------------------
bool Map::IsAgentLocationValidInRectangle(const Vector2D &position,
                                          const float width, const float height,
                                          const float left, const float right,
                                          const float top, const float bottom) {
  Vector2D points[4];
...
//				points - the four points representing the
// Creature (out) 				positionMin - the minimum x and
// y of the set (out) 				positionMax
//- the maximum x and y of the set (out)
// Returns:		None
// ---------------------------------------------------------------------------
void Map::GetCreaturePoints(const Vector2D &footLeft, const Vector2D &footRight,
                            const float yMin, Vector2D starts[3],
                            Vector2D ends[3], Vector2D points[4],
                            Vector2D &positionMin, Vector2D &positionMax) {
...
// ---------------------------------------------------------------------------
bool Map::IsCreatureLocationValidInRoomSystem(const Vector2D &footLeft,
...
// Returns:		true if location valid; false otherwise
// ---------------------------------------------------------------------------
bool Map::IsCreatureLocationValidInRectangle(const Vector2D &footLeft,
                                             const Vector2D &footRight,
  ...
  GetCreaturePoints(footLeft, footRight, yMin, starts, ends, points, dummy1,
                    dummy2);
  return ArePointsInRectangle(points, 4, left, right, top, bottom);
}
...
//				top - y of top side (in)
//				bottom - y of bottom side (in)
//				positionCollision - the position of the
// collision (out) 				deltaCollision - delta from the
// agent to the collision (out) 				deltaWall - the
// delta of the blocking wall (out)
//				wall - which of the four walls we collided with
//(out) 				minSquaredDistance - distance to
// collision (out)
...
bool Map::IsAgentPathBlockedByRectangle(
    ...
    const Vector2D &path, const float left, const float right, const float top,
    const float bottom, Vector2D &positionCollision, Vector2D &deltaCollision,
    Vector2D &deltaWall, int &wall, float &minSquaredDistance) {
  // Test for no path
  ...
  for (i = 0; i < count; ++i) {
    ...
    intersect =
        DoesPathIntersectLine(agentPoint, agentPointProjected, path, edgeStart,
                              edgeEnd, edgeDelta, positionPath, positionLine);
    if (intersect) {
...
//				bottom - y of bottom side (in)
//				positionCollision - the position of the
// collision (out) 				deltaCollision - delta to the
// collision (out) 				deltaWall - the delta of the
// blocking wall (out) 				wall - which of the four walls
// we collided with (out) 				floorCollision - true if
// one of the feet hit the floor (out) 				foot - which
// foot hit the floor; left 0, right 1 (out)
...
bool Map::IsCreaturePathBlockedByRectangle(
    ...
    const Vector2D &path, const float left, const float right, const float top,
    const float bottom, Vector2D &positionCollision, Vector2D &deltaCollision,
    Vector2D &deltaWall, int &wall, bool &floorCollision, int &foot,
...
//				pathBoxMin - minimum point of path bounding box
//(in) 				pathBoxMax - maximum point of path bounding box
//(in) 				startN, endN - three points describing the three
// exposed 					vertices and two exposed edges
// of the agent diamond (in) 				edge1Delta - the delta
// of the first edge (in) 				edge2Delta - the delta
// of the second edge (in) 				path - the path of the
// agent (in) 				pathReversed - the negative of the path
// vector (in)
// minDoorPermiability - minimum door permiability (in)
...
// hitVertex - true
// if the collision point is a door vertex (out)
// positionVertex - the position of the blocking vertex (out)
// edge - true if an
// agent edge strikes the collision point, or
// false if an agent vertex strikes (out) edgeDelta - the delta of the edge
// which struck, this is 					always returned
// even if edge is false (out) 				minSquaredDistance - min
// distance to collision (in/out)
...
bool Map::IsAgentPathBlockedByRoom(
  ...
  while (count-- > 0) {
    ...
    if (DoesPathIntersectLine(start1, end1, path, startDoor, endDoor, deltaDoor,
                              positionPath, positionLine)) {
    ...
    if (DoesPathIntersectLine(start2, end2, path, startDoor, endDoor, deltaDoor,
                              positionPath, positionLine)) {
    ...
    if (DoesPathIntersectLine(start3, end3, path, startDoor, endDoor, deltaDoor,
                              positionPath, positionLine)) {
    ...
    intersect = DoesPathIntersectLine(startDoor, startDoorProjected,
                                      pathReversed, start1, start2, edge1Delta,
                                      positionPath, positionLine);
    if (intersect) {
    ...
    if (!intersect) {
      if (DoesPathIntersectLine(startDoor, startDoorProjected, pathReversed,
                                start2, start3, edge2Delta, positionPath,
                                positionLine)) {
        distance = startDoor.SquareDistanceTo(positionPath);
        // Hit vertex has priority, hence <= not <
        if (distance <= minSquaredDistance) {
    ...
    intersect =
        DoesPathIntersectLine(endDoor, endDoorProjected, pathReversed, start1,
                              start2, edge1Delta, positionPath, positionLine);
    if (intersect) {
    ...
    if (!intersect) {
      if (DoesPathIntersectLine(endDoor, endDoorProjected, pathReversed, start2,
                                start3, edge2Delta, positionPath,
                                positionLine)) {
        distance = endDoor.SquareDistanceTo(positionPath);
        // Hit vertex has priority, hence <= not <
        if (distance <= minSquaredDistance) {
...
//				pathBoxMin - minimum point of path bounding box
//(in) 				pathBoxMax - maximum point of path bounding box
//(in) 				footLeft - position of left foot (in)
// footRight - position of right foot (in) 				points -
// points defining the creature (in) 				starts -
// creature edge starting points (in) 				ends - creature
// edge ending points (in) 				deltas - creature edge
// deltas (in) 				path - the path of the creature (in)
// pathReversed - the negative of the path vector (in)
// minDoorPermiability -
// minimum door permiability (in)
// positionCollision - the position of the collision (out)
// deltaCollision - delta to the collision (out)
// doorCollision - the door that the path collided with (out)
// hitVertex - true if the collision point is a door vertex (out)
// positionVertex
//- the position of the blocking vertex (out)
// floorCollision - true if one of the feet hit the floor (out)
// foot
//- which foot hit the floor; left 0, right 1 (out)
// minSquaredDistance - distance to collision (out)
// Returns:		true if creature path is blocked, false otherwise
// ---------------------------------------------------------------------------
bool Map::IsCreaturePathBlockedByRoom(
  ...
  while (count-- > 0) {
    ...
    for (i = 0; i < 4; ++i) {
      start = points[i];
      intersect =
          DoesPathIntersectLine(start, start + path, path, startDoor, endDoor,
    ...
    for (i = 0; i < 3; ++i) {
      ...
      intersect =
          DoesPathIntersectLine(startDoor, startDoorProjected, pathReversed,
                                start, end, delta, positionPath, positionLine);
      if (intersect) {
        distance = startDoor.SquareDistanceTo(positionPath);
        int value;
        if (positionLine == footLeft)
          ...
          value = -1;
        if ((wall == DIRECTION_FLOOR) && ((value == 0) || (value == 1))) {
          // Feet test has priority, hence <= not <
          if (distance <= minSquaredDistance) {
            blocked = true;
            minSquaredDistance = distance;
      ...
      intersect =
          DoesPathIntersectLine(endDoor, endDoorProjected, pathReversed, start,
                                end, delta, positionPath, positionLine);
      if (intersect) {
        distance = endDoor.SquareDistanceTo(positionPath);
        int value;
        if (positionLine == footLeft)
...
//				path - the path of the agent (in)
//				minDoorPermiability - minimum door permiability
//(in) 				positionCollision - the position of the
// collision (out) 				deltaCollision - delta from the
// agent to the collision (out) 				doorCollision -
// the door that the path collided with (out)
// hitVertex - true if the collision point is a door vertex (out)
// positionVertex - the position of the blocking vertex (out)
...
bool Map::IsAgentPathBlockedByRoomSystem(
    ...
    Vector2D &positionCollision, Vector2D &deltaCollision, Door *&doorCollision,
    bool &hitVertex, Vector2D &positionVertex, bool &edge, Vector2D &edgeDelta,
    float &minSquaredDistance) {
  //
  ...
  for (i = 0; i < myTempRoomCount; ++i) {
    ...
    if ((room->positionMin.x < (pathBoxMax.x + 0.5f)) &&
      ...
      blocked = IsAgentPathBlockedByRoom(
          room, pathBoxMin, pathBoxMax, start1, end1, start2, end2, start3,
          end3, edge1delta, edge2delta, path, pathReversed, minDoorPermiability,
...
//(in) 				positionCollision - the position of the
// collision (out) 				deltaCollision - delta to the
// collision (out) 				doorCollision - the door that
// the path collided with (out) 				hitVertex - true
// if the collision point is a door vertex (out)
// positionVertex - the position
// of the blocking vertex (out) 				floorCollision -
// true if
// one of the feet hit the floor (out) 				foot
//- which foot hit the floor; left 0, right 1 (out)
// minSquaredDistance - distance to collision (out)
// Returns:		true if creature path is blocked, false otherwise
// ---------------------------------------------------------------------------
bool Map::IsCreaturePathBlockedByRoomSystem(
    ...
    const Vector2D &path, const int minDoorPermiability,
    Vector2D &positionCollision, Vector2D &deltaCollision, Door *&doorCollision,
    bool &hitVertex, Vector2D &positionVertex, bool &floorCollision, int &foot,
    float &minSquaredDistance) {
  ...
  Vector2D deltas[3];
  Vector2D points[4];
  GetCreaturePoints(footLeft, footRight, yMin, starts, ends, points, pMin,
                    pMax);
  int i;
  for (i = 0; i < 3; ++i)
    deltas[i] = ends[i] - starts[i];
  ...
  for (i = 0; i < myTempRoomCount; ++i) {
    ...
    if ((room->positionMin.x < (pathBoxMax.x + 0.5f)) &&
        ...
        (room->positionMax.y > (pathBoxMin.y - 0.5f)) &&
        (room->permiability < minDoorPermiability)) {
      blocked = IsCreaturePathBlockedByRoom(
          room, pathBoxMin, pathBoxMax, footLeft, footRight, points, starts,
          ends, deltas, path, pathReversed, minDoorPermiability,
          positionCollision, deltaCollision, doorCollision, hitVertex,
          positionVertex, floorCollision, foot, minSquaredDistance);
      if (blocked)
        collision = true;
    }
  }
...
//				width - agent width (in)
//				height - agent height (in)
//				direction - which direction to test, one of
// DIRECTION_LEFT, 					DIRECTION_RIGHT,
// DIRECTION_UP or DIRECTION_DOWN (in) 				distanceTest -
// how far to look ahead (in)
//				left - x of left side (in)
//				right - x of right side (in)
//				top - y of top side (in)
//				bottom - y of bottom side (in)
...
bool Map::TestForAgentCollisionInRectangle(
  ...
  switch (direction) {
  case DIRECTION_LEFT:
    path = Vector2D(-distanceTest, 0.0f);
    break;
  case DIRECTION_RIGHT:
    path = Vector2D(distanceTest, 0.0f);
    break;
  case DIRECTION_UP:
    path = Vector2D(0.0f, -distanceTest);
    break;
  case DIRECTION_DOWN:
    path = Vector2D(0.0f, distanceTest);
    break;
  ...
  collision = IsAgentPathBlockedByRectangle(
      position, width, height, path, left, right, top, bottom,
      positionCollision, deltaCollision, wallDelta, wall, minSquaredDistance);
  if (collision)
...
bool Map::TestForCreatureCollisionInRectangle(
  ...
  collision = IsCreaturePathBlockedByRectangle(
      footLeft, footRight, yMin, path, left, right, top, bottom,
      positionCollision, deltaCollision, wallDelta, wall, floorCollision, foot,
      minSquaredDistance);
  if (collision)
    distanceCollision = sqrtf(minSquaredDistance);
  return true;
...
// DIRECTION_LEFT, 					DIRECTION_RIGHT,
// DIRECTION_UP or DIRECTION_DOWN (in) 				distanceTest -
// how far from agent to test
//(in) 				minDoorPermiability - minimum door permiability
//(in) collision - is there a collision? (out) distanceCollision - how far to
// the collision (out)
...
bool Map::TestForAgentCollisionInRoomSystem(
  ...
  if ((minDoorPermiability < IMPERMIABLE) || (minDoorPermiability > PERMIABLE))
    return false;

  collision = IsAgentPathBlockedByRoomSystem(
      position, width, height, path, minDoorPermiability, positionCollision,
      deltaCollision, doorCollision, vertex, positionVertex, edge, edgeDelta,
      minSquaredDistance);
  if (collision)
    distanceCollision = sqrtf(minSquaredDistance);
  return true;
...
// DIRECTION_LEFT, 					DIRECTION_RIGHT,
// DIRECTION_UP or DIRECTION_DOWN (in) 				distanceTest -
// how far from creature to
// test (in) 				minDoorPermiability - minimum door
// permiability (in) 				collision - is there a
// collision? (out) 				distanceCollision - how far to
// the collision (out)
// Returns:		true if good input data, false otherwise
// --------------------------------------------------------------------------
bool Map::TestForCreatureCollisionInRoomSystem(
  ...
  collision = IsCreaturePathBlockedByRoomSystem(
      footLeft, footRight, yMin, path, minDoorPermiability, positionCollision,
      deltaCollision, doorCollision, hitVertex, positionVertex, floorCollision,
      foot, minSquaredDistance);
  if (collision)
    distanceCollision = sqrtf(minSquaredDistance);
  return true;
}
...
//				height - height of agent (in)
//				applyPhysics - true to apply elasticity,
// friction, gravity 					and aerodynamics, false
// otherwise (in) 				collisionFactor - percentage
// decrease in velocity due to 					collision (in)
// aeroDynamicFactor - percentage decrease in velocity due to
// media resistance (in) 				frictionFactor -
// percentage decrease in velocity due to
// friction (in) 				gravity - how much y velocity
// increases per time slice (in) 				left - x of left
// side (in) 				right - x of right side (in)
// top - y of top side (in) 				bottom - y of bottom
// side (in) 				position - top-left corner of agent
// (in/out) 				velocity - velocity of
// agent (in/out) 				collision - was there a
// collision? (out) 				wall - the wall the agent hit
// (out) 				stopped - is agent now at rest? (out)
...
void Map::MoveAgentInsideRectangle(
    ...
    const float right, const float top, const float bottom, Vector2D &position,
    Vector2D &velocity, bool &collision, int &wall, bool &stopped,
  ...
  while (times-- > 0) {
    ...
    blocked = IsAgentPathBlockedByRectangle(
        position, width, height, path, left, right, top, bottom,
    ...
    if (!blocked) {
      ...
      // Check integrity
      _ASSERT(IsAgentLocationValidInRectangle(position, width, height, left,
                                              right, top, bottom));
      return;
    }
    ...
    if (moved) {
      CalculateCollisionVelocityAndElapsedTime(velocity, acceleration, path,
                                               deltaCollision, vc, timeElapsed);
      // Clamp instead of asserting: on the first frame after world load,
      // velocity/path can be zero, producing NaN/Inf from the kinematic calc.
      if (timeElapsed < 0.0f)
        timeElapsed = 0.0f;
    ...
    if (applyPhysics) {
      ...
      if (!moved && (wallCollision == DIRECTION_FLOOR)) {
        sliding = true;
        Vector2D dummy;
        CalculateSlideAccelerationAndSlideVelocity(
            vc, wallDelta, gravityAcceleration, dummy, velocity);
        ...
        Vector2D positionMiddle =
            positionStart + (positionEnd - positionStart) / 2.0f;
...
//				footRight - position of right foot (in)
//				yMin - y of top of bounding box (in)
//				applyPhysics - true to apply elasticity, gravity
// and 					aerodynamics; false otherwise (in)
// collisionFactor - percentage decrease in velocity due to
// collision (in) 				aeroDynamicFactor - percentage
// decrease in velocity due to 					media resistance
// (in) 				gravity - how much y velocity increases
// per tick (in) 				left - x of left side (in)
// right - x of right side (in) 				top - y of top
// side (in) 				bottom - y of bottom side (in)
// downFootPosition - position of the down foot (in/out)
// velocity - velocity of creature (in/out) collision - was there a collision?
// (out) 				wall - the wall the creature hit (out)
...
void Map::MoveCreatureInsideRectangle(
    ...
    Vector2D &downFootPosition, Vector2D &velocity, bool &collision, int &wall,
    bool &stopped, int &downFoot, Vector2D &velocityCollision) {
  Vector2D velocityEnd;
  Vector2D path;
  Vector2D positionCollision;
  Vector2D deltaCollision;
  ...
  while (times-- > 0) {
    ...
    // See if any of the four walls block the trajectory
    blocked = IsCreaturePathBlockedByRectangle(
        footLeftTemp, footRightTemp, yMinTemp, path, left, right, top, bottom,
        positionCollision, deltaCollision, wallDelta, wallCollision,
    ...
    if (!blocked) {
      ...
      footRightTemp += path;
      yMinTemp += path.y;
      _ASSERT(IsCreatureLocationValidInRectangle(
          footLeftTemp, footRightTemp, yMinTemp, left, right, top, bottom));
#endif
      return;
    }
...
//				applyPhysics - true to apply elasticity,
// friction, gravity 					and aerodynamics; false
// otherwise (in) 				minDoorPermiability - minimum
// door permiability [0 if 					unblockable]
// (in) 				collisionFactor - percentage decrease in
// velocity due to 					collision (in)
// aeroDynamicFactor - percentage decrease in velocity due to
// media resistance (in) 				frictionFactor -
// percentage decrease in velocity due 					to
// friction (in) 				gravity - how much y velocity
// increases per tick (in) 				position - top-left
// corner of agent (in/out) 				velocity
//- velocity of agent (in/out) 				collision - was there a
// collision? (out) 				wall - the wall the agent hit
// (out) 				stopped - is agent now at rest? (out)
//				velocityCollision - velocity at point of
// collision (out)
// Returns:		None
// --------------------------------------------------------------------------
void Map::MoveAgentInsideRoomSystem(
  ...
  while (times-- > 0) {
    ...
    blocked = IsAgentPathBlockedByRoomSystem(
        position, width, height, path, minDoorPermiability, positionCollision,
        deltaCollision, doorCollision, hitVertex, positionVertex, edge,
        edgeDelta, minSquaredDistance);

    //		OutputFormattedDebugString("[%d] Position %f %f Velocity %f %f
    // Path %f %f Blocked %d\n", 			times, position.x,
    // position.y, velocity.x, velocity.y, path.x, path.y,
    // blocked);
    ...
    if (!blocked) {
      ...
      velocity = velocityEnd;
      // Check integrity
      if (!IsAgentLocationValidInRoomSystem(position, width, height,
                                            minDoorPermiability))
        return; // agent outside room system, bail gracefully
      return;
    }
    ...
    if (hitVertex && edge) {
      ...
      GetReflectionFromVertex(positionVertex, temp, true, edgeDelta,
                              minDoorPermiability, velocityReflected,
                              slidingDoor, spike);
      slope = edgeDelta;
      ...
      if (hitVertex) {
        ...
        //
        if (!doorCollision) {
          // doorCollision is null — agent hit something unexpected.
          // Bring it to rest safely rather than crashing.
          velocity = ZERO_VECTOR;
          stopped = true;
          return;
        }
    ...
    if (applyPhysics) {
      ...
      if ((!moved) && ((wallCollision == DIRECTION_FLOOR) || spike)) {
        sliding = true;
        CalculateSlideAccelerationAndSlideVelocity(
            vc, slope, gravityAcceleration, slideAcceleration, velocity);
...
// unblockable] (in) 				collisionFactor - percentage
// decrease in velocity due to 					collision (in)
// aeroDynamicFactor - percentage decrease in velocity due to
// media resistance (in) 				gravity - how much y
// velocity increases per tick (in) 				downFootPosition
// - position of the down foot (in/out) 				velocity
// - velocity of creature (in/out) 				collision - was
// there a collision? (out) 				wall - the wall the
// creature hit (out) 				stopped - has creature came to
// rest on the floor? (out) 				downFoot - which foot is
// the down foot; left 0, right 1 (out)
// velocityCollision - velocity at point of collision (out) Returns:
// None
// --------------------------------------------------------------------------
void Map::MoveCreatureInsideRoomSystem(
  ...
  while (times-- > 0) {
    ...
    // See if any walls get in the way of the path
    blocked = IsCreaturePathBlockedByRoomSystem(
        footLeftTemp, footRightTemp, yMinTemp, path, minDoorPermiability,
        positionCollision, deltaCollision, doorCollision, hitVertex,
        positionVertex, floorCollision, footCollision, minSquaredDistance);
...
// Returns:		None
// ----------------------------------------------------------------------
void Map::GetNearbyFloorInformation(const Vector2D &position, Room **floorRooms,
                                    int &floorRoomCount) {
  int id;
  int nid, id1, id2;
  int dummy1;
  ...
  bool ok =
      GetRoomInformationForPoint(position, idList, roomList, idCount, dummy1);
  _ASSERT(ok);
...
//					where the region represents the movement
// of a 					walking creature
// Arguments:	floorRooms - the floor rooms (in)
//				floorRoomCount - the number of rooms (in)
//				minDoorPermiability - minimum door permiability
//(in) 				xMinTest... - the region to test (in)
...
bool Map::IsRegionBlockedByVerticalWall(
    Room **floorRooms, int floorRoomCount, const int minDoorPermiability,
    const float xMinTest, const float xMaxTest, const float yMinTest,
    const float yMaxTest, const bool right, float &xDisplacement) {
  int i;
  Room *room;
  DoorIterator doorIterator;
  DoorIterator doorIteratorEnd;
...
//				floorRoomCount - the number of rooms (in)
//				minDoorPermiability - minimum door permiability
//(in) 				right - true if moving right (in)
// snapped1/2 - whether the points could be snapped (out)
// positionSnap1/2 - the snapped points (out) 				door1/2
// - the doors the points snapped to (out)
// distance1/2 - the distances to the snap points (out)
// Returns:		None
// ----------------------------------------------------------------------
void Map::SnapPointsToFloor(const Vector2D &position1,
                            const Vector2D &position2, Room **floorRooms,
  ...
  for (i = 0; i < floorRoomCount; ++i) {
    ...
    while (doorIterator != doorIteratorEnd) {
      ...
      if ((position1.x >= door->start.x) && (position1.x <= door->end.x)) {
      ...
      if ((position2.x >= door->start.x) && (position2.x <= door->end.x)) {
...
// Returns:		None
// ----------------------------------------------------------------------
void Map::SnapFeetToFloor(const Vector2D &downFoot, const Vector2D &upFoot,
  ...
  SnapPointsToFloor(down, up, floorRooms, floorRoomCount, minDoorPermiability,
                    right, downSnapped, upSnapped, downSnap, upSnap, downDoor,
                    upDoor, downDistance, upDistance);
...
// Returns:		None
// ----------------------------------------------------------------------
void Map::CalculateGradients(const bool fall, const bool right, Door *door,
...
//				upFootPrevious - the previous up foot position
//(in) 				upFootCurrent - the current up foot position
//(in) 				xMinCurrent - min x of the current bounding box
// (in) 				xMaxCurrent - max x of the current
// bounding box (in) 				yMinPrevious - min y of the
// previous bounding box (in) 				yMinCurrent - min y of
// the current bounding box (in)
// minDoorPermiability - minimum door permiability (in)
// directionRight - if the creature is walking right (in)
// downFootNew - the new
// down foot position (out) 				gradientDownhill -
// downhill gradient (out) 				gradientUphill - uphill
// gradient (out) 				collision -
// true if there was a wall collision (out) 				wall -
// which wall the creature hit (out) footChange - true if up foot is now the
// down
// foot (out)
...
void Map::TestNewUpFootInRoomSystem(
    ...
    const float xMaxCurrent, const float yMinPrevious, const float yMinCurrent,
    const int minDoorPermiability, const bool directionRight,
    Vector2D &downFootNew, float &gradientDownhill, float &gradientUphill,
    bool &collision, int &wall, bool &footChange, bool &fall) {
  ...
  collision = IsRegionBlockedByVerticalWall(
      floorRooms, floorRoomCount, minDoorPermiability, xMinTest, xMaxTest,
      yMinTest, yMaxTest, right, xDisplacement);
  ...
  SnapFeetToFloor(down, up, floorRooms, floorRoomCount, minDoorPermiability,
                  right, downFootNew, door, footChange, fall);
...
//					a rectangle
// Arguments:	downFootCurrent - the current down foot position (in)
//				upFootCurrent - the current up foot position
//(in) 				xMinCurrent - min x of the current bounding box
//(in) 				xMaxCurrent - max x of the current bounding box
// (in) 				left - x of left side (in)
// right - x of right side (in) 				top - y of top
// side (in) 				bottom - y of bottom side (in)
// downFootNew
//- the new down foot position (out) 				collision - true
// if there was a wall collision (out) 				wall - which
// wall the creature hit (out) 				footChange - true if up
// foot is now the down foot (out)
// Returns:		None
// ----------------------------------------------------------------------
void Map::TestNewUpFootInRectangle(
    const Vector2D &downFootCurrent, const Vector2D &upFootCurrent,
    const float xMinCurrent, const float xMaxCurrent, const float left,
    const float right, const float top, const float bottom,
    Vector2D &downFootNew, bool &collision, int &wall, bool &footChange) {
  float gapWest;
  float gapEast;
...
//				minDoorPermiability - minimum door permiability
//(in) 				xShift - the amount to shift the creature by
//(in) 				downFootNew - the new down foot position (out)
// collision - true if there was a wall collision (out)
// wall - which wall the
// creature hit (out) 				footChange - true if up foot is
// now the down foot (out)
// fall - true if creature should start to fall (out)
// Returns:		None
// ----------------------------------------------------------------------
void Map::ShiftCreatureAlongFloor(const Vector2D &downFoot,
                                  const Vector2D &upFoot, const float xMin,
...
// Returns:	true if good input, false otherwise
// ----------------------------------------------------------------------
bool Map::GetRoomCentre(const int roomID, Vector2D &centre) {
  Room *room;
...
// Returns:	true if good input, false otherwise
// ----------------------------------------------------------------------
bool Map::GetMetaRoomTrack(const int metaRoomID, std::string &track) {
  MetaRoom *metaRoom;
  bool ok;
...
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------
bool Map::SetMetaRoomTrack(const int metaRoomID, std::string &track) {
  MetaRoom *metaRoom;
  bool ok;
...
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------
bool Map::SetRoomTrack(const int roomID, std::string &track) {
  Room *room;
  bool ok;
...
// Returns:		true if good input, false otherwise
// ----------------------------------------------------------------------
bool Map::GetRoomTrack(const int roomID, std::string &track) {
  Room *room;
  bool ok;
...
//				positionY - top-left y coordinate of meta-room
//(in) 				width - width of meta-room (in)
// height - height of meta-room (in) 				background -
// initial background (in) 				metaRoomID - ID of the
// new meta-room (out) Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::AddMetaRoom(const int positionX, const int positionY, const int width,
                      const int height, std::string background,
                      int &metaRoomID) {
  int i;
  bool slotFound;
  MetaRoom *metaRoom;
...
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::RemoveMetaRoom(const int metaRoomID) {
  MetaRoom *metaRoom;
  MetaRoom *m;
  int roomCount;
  int i;
  ...
  myMetaRoomIDCollection.erase(std::find(myMetaRoomIDCollection.begin(),
...
// ------------------------------------------------------------------------
int Map::GetMetaRoomCount(void) { return myMetaRoomCount; }
...
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::SetCurrentMetaRoom(const int metaRoomID) {
  bool ok;
  MetaRoom *metaRoom;
...
// ---------------------------------------------------------------------------
int Map::GetCurrentMetaRoom(void) { return myMetaRoomID; }
...
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::GetMetaRoomDefaultCameraLocation(const int metaRoomID,
...
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::SetMetaRoomDefaultCameraLocation(const int metaRoomID,
                                           const Position &position) {
  MetaRoom *metaRoom;
  bool ok = GetValidMetaRoomPointer(metaRoomID, metaRoom);
  if (!ok)
    return false;
  metaRoom->positionDefault =
      Vector2D((int)position.GetX(), (int)position.GetY());
  return true;
}
...
// ---------------------------------------------------------------------------
DoorCollection &Map::GetDoorCollection(void) { return myDoorCollection; }
...
void Map::CalculateFloorsForRoom(Room *room) {
  ...
  for (doorIterator = room->doorCollection.begin();
       doorIterator != room->doorCollection.end(); ++doorIterator) {
    door = *doorIterator;
    if ((door->doorType == DOOR_CEILING_FLOOR) && (door->parent1 == roomID))
...
void Map::CalculateCeilingsForRoom(Room *room) {
  ...
  for (doorIterator = room->doorCollection.begin();
       doorIterator != room->doorCollection.end(); ++doorIterator) {
    door = *doorIterator;
    if ((door->doorType == DOOR_CEILING_FLOOR) && (door->parent2 == roomID))
...
void Map::CalculateLeftWallsForRoom(Room *room) {
  ...
  for (doorIterator = room->doorCollection.begin();
       doorIterator != room->doorCollection.end(); ++doorIterator) {
    door = *doorIterator;
    if ((door->doorType == DOOR_LEFT_RIGHT) && (door->parent2 == roomID))
...
void Map::CalculateRightWallsForRoom(Room *room) {
  ...
  for (doorIterator = room->doorCollection.begin();
       doorIterator != room->doorCollection.end(); ++doorIterator) {
    door = *doorIterator;
    if ((door->doorType == DOOR_LEFT_RIGHT) && (door->parent1 == roomID))
...
//					and right navigable doors for a given
// room
// Arguments:	room - room to perform calculation on (in)
// Returns:		None
// ---------------------------------------------------------------------------
void Map::CalculateNeighbourInformationForRoom(Room *room) {
  ...
  for (doorIterator = room->doorCollection.begin();
       doorIterator != room->doorCollection.end(); ++doorIterator) {
    door = *doorIterator;
    if ((door->parentCount != 2) || (door->doorType != DOOR_LEFT_RIGHT))
      continue;
    ...
    if ((door->parent1 == roomID) &&
        room->endFloor == myRoomCollection[door->parent2]->startFloor)
      room->rightNavigableDoor = door;
    else if ((door->parent2 == roomID) &&
             room->startFloor == myRoomCollection[door->parent1]->endFloor)
...
//				yLeftCeiling - y coordinate of left hand ceiling
//(in) 				yRightCeiling - y coordinate of right hand
// ceiling (in) 				yLeftFloor - y coordinate of
// left hand
// floor (in) 				yRightFloor - y coordinate of right hand
// floor (in) 				roomID - ID of the new room (out)
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::AddRoom(const int metaRoomID, const int xLeft, const int xRight,
  ...
  // Is there an unused ID slot?
  slotFound = false;
  if (myMaxRoomID < myRoomIndexBase - 1)
    myMaxRoomID = myRoomIndexBase - 1;
  ...
  room->caTotalDoorage = 0;
  room->caInput = 0;
  room->perimeterLength =
      room->deltaFloor.Length() + room->deltaCeiling.Length() +
      (yLeftFloor - yLeftCeiling) + (yRightFloor - yRightCeiling);
  ...
  IntegerCollection &downNeigbours =
      room->neighbourIDCollections[DIRECTION_DOWN];
  IntegerCollection &leftNeigbours =
      room->neighbourIDCollections[DIRECTION_LEFT];
  IntegerCollection &rightNeigbours =
      room->neighbourIDCollections[DIRECTION_RIGHT];
...
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::RemoveRoom(const int roomID) {
  int i;
  DoorIterator doorIterator, doorIteratorEnd;
  ...
  while (doorIterator != doorIteratorEnd) {
    ...
    if (door->parentCount == 1) {
      // Destroy the door
      doorToDelete =
          std::find(myDoorCollection.begin(), myDoorCollection.end(), door);
      ...
      neighbourToDelete =
          std::find(otherRoom->neighbourIDCollection.begin(),
                    otherRoom->neighbourIDCollection.end(), roomID);
      otherRoom->neighbourIDCollection.erase(neighbourToDelete);
      for (i = 0; i < 4; ++i) {
        neighbourToDelete =
            std::find(otherRoom->neighbourIDCollections[i].begin(),
                      otherRoom->neighbourIDCollections[i].end(), roomID);
  ...
  myRoomIDCollection.erase(
      std::find(myRoomIDCollection.begin(), myRoomIDCollection.end(), roomID));
  // Erase the room ID from the meta-room's set of room IDs
  metaRoom->roomIDCollection.erase(std::find(metaRoom->roomIDCollection.begin(),
                                             metaRoom->roomIDCollection.end(),
                                             roomID));
  if (myRoomCount == 0) {
    myMaxRoomID = -1;
...
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::GetRoomIDCollectionForMetaRoom(const int metaRoomID,
...
// ---------------------------------------------------------------------------
bool Map::IsMetaRoomIDValid(const int metaRoomID) {
  MetaRoom *dummy;
...
// Returns:		true if valid; false otherwise
// ---------------------------------------------------------------------------
bool Map::IsRoomIDValid(const int roomID) {
  Room *dummy;
...
//				metaRoomID - ID of the meta-room, -1 if none
//(out)
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::GetMetaRoomIDForPoint(const Vector2D &position, int &metaRoomID) {
  int id;
  MetaRoom *metaRoom;
  IntegerIterator integerIterator;
  IntegerIterator integerIteratorEnd;
...
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::GetRoomIDForPoint(const Vector2D &position, int &roomID) {
  int dummy1[4];
  Room *dummy2[4];
  int dummy3;
  bool ok;
  ...
  if ((position.y < 0.0f) || (position.y >= myMapHeight))
    return false;
  ok = GetRoomInformationForPoint(position, dummy1, dummy2, dummy3, roomID);
  if (!ok) {
    return false;
  } else {
    return true;
...
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::SetMapDimensions(const int width, const int height) {
  float w = width;
  float h = height;
  ...
  if ((w <= myMetaRoomMaxCoordinates.x) || (h <= myMetaRoomMaxCoordinates.y))
    return false;
  myMapWidth = w;
  myMapHeight = h;
  BuildCache();
...
// Returns:		None
// ---------------------------------------------------------------------------
void Map::GetMapDimensions(int &width, int &height) {
...
// ---------------------------------------------------------------------------
int Map::GetGridSize(void) { return FastFloatToInteger(myGridSquareSize); }
...
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::GetMetaRoomLocation(const int metaRoomID, int &positionX,
...
//				yLeftCeiling - y coordinate of left hand ceiling
//(out) 				yRightCeiling - y coordinate of right
// hand ceiling (out) 				yLeftFloor - y coordinate of
// left hand floor (out) 				yRightFloor - y
// coordinate of right hand floor (out)
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::GetRoomLocation(const int roomID, int &metaRoomID, int &xLeft,
...
// Arguments:	metaRoomID - meta-room ID (in)
//				backgroundCollection - the background collection
//(in)
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::SetBackgroundCollection(const int metaRoomID,
...
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::GetBackgroundCollection(const int metaRoomID,
                                  StringCollection &backgroundCollection) {
  MetaRoom *metaRoom;
  bool ok;
...
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::AddBackground(const int metaRoomID, std::string background) {
  MetaRoom *metaRoom;
  bool ok;
...
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::RemoveBackground(const int metaRoomID, std::string background) {
  MetaRoom *metaRoom;
...
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::SetCurrentBackground(const int metaRoomID, std::string background) {
  MetaRoom *metaRoom;
  bool ok;
...
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::GetCurrentBackground(const int metaRoomID, std::string &background) {
  MetaRoom *metaRoom;
  bool ok;
...
//				direction - which wall(s) to get neighbours for,
// chosen 					from DIRECTION_LEFT,
// DIRECTION_RIGHT, DIRECTION_CEILING,
// DIRECTION_FLOOR or DIRECTION_ALL (in)
//				neighbourIDCollection - neighbour IDs (out)
// Returns:		true if good input, false otherwise
// --------------------------------------------------------------------------
bool Map::GetNeighbours(const int roomID, const int direction,
...
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::GetValidMetaRoomPointer(const int metaRoomID, MetaRoom *&metaRoom) {
    ...
    for (y = squareY1; y <= squareY2; ++y) {
      ...
      myIntegerCollectionGrid[index].erase(
          std::find(myIntegerCollectionGrid[index].begin(),
                    myIntegerCollectionGrid[index].end(), roomID));
    }
  }
}
...
void Map::BuildCache(void) {
  ...
  myIntegerCollectionGrid =
      new IntegerCollection[mySquareCountX * mySquareCountY];
...
//				overlapInformation - info on overlapping
// sections (out) 				sharedIndex - which of the
// overlapping sections represents 					a shared
// section (out)
// Returns:		Number of overlapping sections
// ---------------------------------------------------------------------------
int Map::GetOverlapInformation(Door *door, const Vector2D &startEdge,
                               const Vector2D &endEdge,
                               const Vector2D &deltaEdge,
  ...
  edgeStartOnDoor =
      IsPointOnLine(startEdge, startDoor, endDoor, deltaDoor, 1.0f);
  edgeEndOnDoor = IsPointOnLine(endEdge, startDoor, endDoor, deltaDoor, 1.0f);
  doorStartOnEdge =
      IsPointOnLine(startDoor, startEdge, endEdge, deltaEdge, 1.0f);
  doorEndOnEdge = IsPointOnLine(endDoor, startEdge, endEdge, deltaEdge, 1.0f);
...
//				oldSharedDoor - the door that used to be shared
// between the 					room and its deleted neighbour
// (in/out)
// Returns:		None
// ---------------------------------------------------------------------------
...
// Returns:		None
// ---------------------------------------------------------------------------
void Map::AddRoomEdge(IntegerCollection &siblingIDs, Room *room,
  ...
  while (roomIDIterator != roomIDIteratorEnd) {
    ...
    while (doorIterator != doorIteratorEnd) {
      ...
      while (overlapIterator != overlapIteratorEnd) {
        if ((doorType == DOOR_CEILING_FLOOR) &&
          ...
          break;
                   ...
                   (overlapIterator->start.y >
                    overlapDoorInformation.start.y)) {
          found = true;
          break;
        }
        ++overlapIterator;
      ...
      if (found) {
        overlapCollection.insert(overlapIterator, overlapDoorInformation);
      } else {
        overlapCollection.insert(overlapIteratorEnd, overlapDoorInformation);
      }
      ...
      for (i = 0; i < sections; ++i) {
        ...
        if (overlapInformation[i].shared) {
          ...
          if (create) {
            newDoor = new Door(doorType, PERMIABLE, 2, parent1, parent2,
            ...
            exposedDoor->SetDoorProperties(doorType, PERMIABLE, 2, parent1,
          ...
          exposedRoom->neighbourIDCollections[edgeTypeOpposite].push_back(
              roomID);
        } else {
          ...
          if (create) {
            ...
            exposedDoor->SetDoorProperties(doorType, IMPERMIABLE, 1, parent1,
                                           parent2, overlapInformation[i].start,
                                           overlapInformation[i].end);
          }
        }
  ...
  if (!overlap) {
    ...
    if (((doorType == DOOR_LEFT_RIGHT) && (info.start.y > start.y)) ||
        ((doorType == DOOR_CEILING_FLOOR) && (info.start.x > start.x))) {
      // Create a new door
    ...
    while (overlapIterator != lastOverlap) {
      ...
      if (((doorType == DOOR_LEFT_RIGHT) && (info.end.y != infoNext.start.y)) ||
          ((doorType == DOOR_CEILING_FLOOR) &&
           (info.end.x != infoNext.start.x))) {
        // Create a new door
        newDoor = new Door(doorType, IMPERMIABLE, 1, parent1, parent2, info.end,
                           infoNext.start);
        // Add this door to the new room's set of doors
        room->doorCollection.push_back(newDoor);
        // Add this door to the temporary door collection
    ...
    if (((doorType == DOOR_LEFT_RIGHT) && (info.end.y < end.y)) ||
        ((doorType == DOOR_CEILING_FLOOR) && (info.end.x < end.x))) {
      // Create a new door
      newDoor =
          new Door(doorType, IMPERMIABLE, 1, parent1, parent2, info.end, end);
      // Add this door to the new room's set of doors
      room->doorCollection.push_back(newDoor);
      // Add this door to the temporary door collection
      myConstructedDoorCollection.push_back(newDoor);
...
// Returns:		true if good input, false otherwise
// ---------------------------------------------------------------------------
bool Map::GetDoorBetweenRooms(const int id1, const int id2, Door *&door) {
  Room *room;
  DoorIterator doorIterator, doorIteratorEnd;
  Door *d;
  int p1, p2;
  ...
  while (doorIterator != doorIteratorEnd) {
    ...
    if (d->parentCount == 2) {
      p1 = d->parent1;
      p2 = d->parent2;
      if (((p1 == id1) && (p2 == id2)) || ((p1 == id2) && (p2 == id1))) {
        door = d;
        return true;
      }
    }
...
static void WriteIntegerCollection(CreaturesArchive &ar,
  ...
  for (integerIterator = integerCollection.begin();
       integerIterator != integerCollection.end(); ++integerIterator) {
    i = *integerIterator;
    ar << i;
  }
}
...
static void WriteDoorCollection(CreaturesArchive &ar,
  ...
  for (doorIterator = doorCollection.begin();
       doorIterator != doorCollection.end(); ++doorIterator) {
    door = *doorIterator;
    ar << door;
  }
}
...
static void WriteStringCollection(CreaturesArchive &ar,
  ...
  for (stringIterator = stringCollection.begin();
       stringIterator != stringCollection.end(); ++stringIterator) {
    s = *stringIterator;
    ar << s;
  }
}
...
bool Link::Write(CreaturesArchive &ar) const {
  ar << parent1 << parent2 << permiability << length << doorage1 << doorage2;
  return true;
}
...
bool Room::Read(CreaturesArchive &ar) {
  ...
  if (version >= 3) {
    ...
    for (i = 0; i < CA_PROPERTY_COUNT; ++i) {
      ar >> caValues[i];
      if (theApp.GetWorld().GetMap().IsCANavigable(i))
        caValues[i] = 0.0f;
      ar >> caOldValues[i];
      ar >> caOlderValues[i];
    }
