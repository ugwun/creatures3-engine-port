...
//				directory but Note that the new world switcher
//style requires 				one folder per product or update to appear in the bootstrap
//				directory.  The products must be named
//alphabetically 				corresponding to their release dates.  Eg 0100 C3, 0200 C3 Life
//Kit
//
//
// History:
...
// History:
// -------  Alima		created
// 26Jun99	Alima		Altered to work with the world switcher
// directory structure.
//						  The cos installer now needs to
//load all scripts 						for each new product detected.
// --------------------------------------------------------------------------
...
public:
  ...
  // Load from pre-resolved absolute directory paths (for multi-base loading).
  CosInstaller(std::vector<std::string> &absBootstrapDirs, bool absolutePaths);
  ...
  void SetUpProgressBarAbs(std::vector<std::string> &absDirs);

  bool AddScript(Classifier classifier);
  bool ReadScriptFile(std::string const &filename,
                      bool updateScriptoriumOnly = false);
