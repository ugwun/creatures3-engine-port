...
// 19Aug98	Peter Chilvers	Added CRect, CPoint unsigned int and uint16
//							to supported types
...
public:
  ...
  // Removed Write(int) to avoid collision with Write(int32) when int32 is int.
  ...
  */
  ...
  friend CreaturesArchive &operator<<(CreaturesArchive &ar, const char* string) {
    ar.Write(string);
    return ar;
  }
