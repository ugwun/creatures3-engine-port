...
RemoteCamera::RemoteCamera() : myCameraWidth(0), myCameraHeight(0) {
  myScreenBound.top = 0;
...
RemoteCamera::RemoteCamera(
  ...
  if (!SetUpDrawingSurface()) {
    std::string string = ErrorMessageHandler::Format(
        theDisplayErrorTag, (int)DisplayEngine::sidNoSecondaryCameraCreated,
...
void RemoteCamera::ZoomBy(int32 pixels, int32 x, int32 y) {
  ...
  if ((testWidth > 0 &&
       testWidth < DisplayEngine::theRenderer().GetSurfaceWidth()) &&
      (testHeight > 0 &&
       testHeight < DisplayEngine::theRenderer().GetSurfaceHeight())) {
    myViewWidth += pixels;
    myViewHeight += pixels;
  }
...
bool RemoteCamera::SetUpDrawingSurface() {
  ...
  //		drawFrom.bottom
  //=DisplayEngine::theRenderer().GetSurfaceHeight();
  ...
  // for me
  mySurface = DisplayEngine::theRenderer().CreateSurface(drawFrom.right,
                                                         drawFrom.bottom);

  return (mySurface == NULL ? false : true);
#endif         // WORK_IN_PROGRESS
  return true; // No DD surface needed; drawing code is gated by
               // WORK_IN_PROGRESS
}
...
void RemoteCamera::Draw() {
  ...
  DisplayEngine::theRenderer().BlitToBackBuffer(myScreenBound, mySurface,
                                                drawFrom, transparencyAware);

#endif WORK_IN_PROGESS
}

void RemoteCamera::DrawMirrored() { Update(true, true); }

void RemoteCamera::Refresh() { Update(true, true); }
...
bool RemoteCamera::Read(CreaturesArchive &ar) {
  ...
  // success
  if (!SetUpDrawingSurface()) {
    // Log the error but don't throw — the camera will exist as a
    // non-rendering stub.  All Draw() code is gated by WORK_IN_PROGRESS.
    std::string string = ErrorMessageHandler::Format(
        theDisplayErrorTag, (int)DisplayEngine::sidNoSecondaryCameraCreated,
        std::string("RemoteCamera::RemoteCamera"));
    ErrorMessageHandler::NonLocalisable(string, "RemoteCamera::Read");
  }
  return true;
}
  ...
  if (myTrackObject.IsValid()) {
    ...
    if (!InTrackingRectangle(offbyx, offbyy)) {
      ...
      theApp.GetWorld().GetMap().GetMetaRoomIDForPoint(
          Vector2D((int)myWorldPosition.GetX(), (int)myWorldPosition.GetY()),
          oldmetaRoomID);
