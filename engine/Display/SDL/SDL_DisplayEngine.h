...
public:
  ...
  bool BlitFromPrimaryToMe(RECT &destination, SDL_Surface *dest, RECT &source,
                           bool transparencyAware);

  bool BlitFromBackBufferToMe(RECT &destination, SDL_Surface *dest,
...
inline int32 DisplayEngine::GetSurfaceWidth(void) {
  // Use ourSurfaceArea (set once in Start()) rather than mySurfaceArea
  // (a per-frame temp copy that is zero until the first Update() call).
  return ourSurfaceArea.right;
}

inline int32 DisplayEngine::GetSurfaceHeight(void) {
  return ourSurfaceArea.bottom;
}
