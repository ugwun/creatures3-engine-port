...
// -------------------------------------------------------------------------
// DisplayEngine::Start()
// *Probably* initializes the SDL video surfaces and buffers.
// In SDL mode, rather than using complex DirectX surface flips, it simply
// allocates an SDL_Surface for the front and back buffers.
// -------------------------------------------------------------------------
...
void DisplayEngine::CreateUserInterfaceGalleries() {
  // Load the character sprite sheet for DrawString / DrawStringToBackBuffer.
  // Chars.s16 ships with Creatures 3, not Docking Station. Probe primary image
  // directories first, then any auxiliary game image directory from settings.
  {
    const char *kCharFile = "Chars.s16";
    std::string charsPath;

    // 1. Check the primary registered directories (IMAGES_DIR first).
    for (int d = 0; d < NUM_DIRS && charsPath.empty(); ++d) {
      std::string dir = theApp.GetDirectory(d);
      if (dir.empty())
        continue;
      std::string candidate = dir + kCharFile;
      if (access(candidate.c_str(), R_OK) == 0)
        charsPath = candidate;
    }

#ifndef _WIN32
    // 2. Fall back to auxiliary game image directories from MachineSettings.
    //    Keys are "Auxiliary N Images Directory", values are paths relative to
    //    machineBase.  Derive machineBase = parent of the primary Images dir.
    if (charsPath.empty()) {
      std::string imagesDir = theApp.GetDirectory(IMAGES_DIR);
      // imagesDir ends with "Images/"; go up two components to get machineBase.
      // e.g. ".../Docking Station/Images/" -> ".../Docking Station/"
      std::string base = imagesDir;
      if (!base.empty() && base[base.size() - 1] == '/')
        base.resize(base.size() - 1); // strip trailing /
      size_t slash = base.find_last_of('/');
      if (slash != std::string::npos)
        base = base.substr(0, slash + 1); // ".../Docking Station/"

      for (int n = 1; n <= 9 && charsPath.empty(); ++n) {
        char key[64];
        snprintf(key, sizeof(key), "Auxiliary %d Images Directory", n);
        std::string relPath;
        theApp.MachineSettings().Get(std::string(key), relPath);
        if (relPath.empty())
          break;

        // Normalise: convert Windows backslashes.
        for (size_t i = 0; i < relPath.size(); ++i)
          if (relPath[i] == '\\')
            relPath[i] = '/';

        // Build absolute path: resolve relPath relative to base.
        std::string auxDir;
        if (!relPath.empty() && relPath[0] == '/')
          auxDir = relPath; // already absolute
        else
          auxDir =
              base +
              relPath; // e.g. ".../Docking Station/" + "../Creatures 3/Images"

        // Ensure trailing slash.
        if (auxDir.empty() || auxDir[auxDir.size() - 1] != '/')
          auxDir += '/';

        std::string candidate = auxDir + kCharFile;
        if (access(candidate.c_str(), R_OK) == 0)
          charsPath = candidate;
      }
    }
    ...
    if (!charsPath.empty()) {
      // FilePath(-1) means "use path as-is without directory lookup".
      myTextGallery =
          (NormalGallery *)SharedGallery::theSharedGallery().CreateGallery(
              FilePath(charsPath, -1));
    } else {
      OutputDebugString("WARNING: Chars.s16 not found - DrawString disabled\n");
    }
  }

  // The transition/sprite surface (trans.s16 + DirectDraw DDSURFACEDESC2
  // lockable surface) is not yet ported – leave it disabled.
#ifdef WORK_IN_PROGRESS
...
// data 				screen_step - place to put the amount of
// screen pixels we need to skip when drawing screen_ptr - workout where on
// screen to start drawing 				bitmapWidth -
// bitmapHeigth - workout exactly how much of the bitmap we need
// to draw due to lipping 				right - whether we'll
// need to clip right 				wether - whether we'll need to
// clip bottom Returns:     None
//
...
void DisplayEngine::DrawString(Bitmap *destination, std::string text,
  ...
  uint32 sourceHeight = myTextGallery->GetBitmapHeight(0);
...
// are enumrated at 				start up of the engine
//
// ----------------------------------------------------------------------
...
// of code 				because I have different routines for
...
bool DisplayEngine::GetPixelFormat(uint32 &format) {
  ...
  SDL_PixelFormat *pf = myFrontBuffer->format;

  // Check for 555 format: R=0x7c00, G=0x03e0, B=0x001f
  if (pf->Rmask == 0x7c00 && pf->Gmask == 0x03e0 && pf->Bmask == 0x001f) {
    myPixelFormat = RGB_555;
  }
  // Check for 565 format: R=0xf800, G=0x07e0, B=0x001f
  else if (pf->Rmask == 0xf800 && pf->Gmask == 0x07e0 && pf->Bmask == 0x001f) {
    myPixelFormat = RGB_565;
  } else {
    // Unknown or BGR format from SDL - assume 565, which matches the
    // Docking Station game assets (confirmed in machine.cfg "Display Type" = 2)
    myPixelFormat = RGB_565;
  }
