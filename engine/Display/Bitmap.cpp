...
Bitmap::Bitmap()
    : myWidth(0), myHeight(0), myDataFlag(0), myOffset(0), myData(0),
      myXOffset(0), myYOffset(0), myPosition(0, 0), myClippedWidth(0),
      myClippedHeight(0) {}
...
bool Bitmap::LoadFromBmp(char *name) {
  ...
  handle = (HBITMAP)LoadImage(NULL, name, IMAGE_BITMAP, 0, 0,
  ...
  if (handle) {
    ...
    if (bitmap.bmPlanes == 1 && bitmap.bmBitsPixel == 24) {
      ...
      // get the bits
      uint8 *src =
          (uint8 *)bitmap.bmBits + (3 * bitmap.bmWidth * (bitmap.bmHeight - 1));
...
void Bitmap::CreateTest(int32 width,int32 height,int32 index)
{
        ...
        uint16
colour1=gColourTable[(index/COLOUR_TABLE_SIZE)&(COLOUR_TABLE_SIZE-1)];
        myWidth=width;
        myHeight=height;
...
//
// ----------------------------------------------------------------------
void Bitmap::Save(File &file) { file.Write(myData, myWidth * myHeight); }
...
void Bitmap::OffsetDraw(Position &pos, RECT &clip) {
  DisplayEngine::theRenderer().OffsetDrawTile(pos, *this, clip);
}
...
//				false otherwise. Note no checks for out of
// bounds
//
// ----------------------------------------------------------------------
...
void Bitmap::SetData(MemoryMappedFile &file) {
  ...
  // Point myData directly into the memory-mapped file at the current position.
  uint8 *data;
  file.ReadUINT8Ptr(data);
  myData = (uint16 *)data;

  // Advance past this tile's pixel data.
  // ReadUINT8Ptr already consumed 1 byte (the first pixel byte), so we seek
  // forward the remaining (w * h * 2 - 1) bytes.
  // FIXED: was -(1) which left the file cursor 1 byte short per tile, causing
  // each subsequent tile to start 1 byte too early → checkerboard corruption.
