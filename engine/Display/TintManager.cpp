...
bool TintManager::Write(CreaturesArchive &ar) const {
  ...
  unsigned long csize = 128 * 1024 * sizeof(uint16);
  uint32 temp;
  if (compress((uint8 *)myCompressedSpace, &csize, (uint8 *)myTintTable,
               65536 * sizeof(uint16)) != Z_OK) {
    temp = 1;
    ar << temp;
...
bool TintManager::Read(CreaturesArchive &ar) {
  ...
  if (version >= 3) {
    ...
    if (size == 2) {
      ...
      ar >> myRedTint >> myGreenTint >> myBlueTint >> myRotation >> mySwap;
      unsigned long destLen = 65536 * sizeof(uint16);
      if (uncompress((uint8 *)myTintTable, &destLen, (uint8 *)myCompressedData,
                     csize) != Z_OK) {
...
void TintManager::BuildTintTable(int16 redTint, int16 greenTint, int16 blueTint,
                                 int16 rotation, int16 swap) {
  ...
  if (redTint == 128 && blueTint == 128 && greenTint == 128 &&
  ...
  // OutputFormattedDebugString("Building of Tint table of (%d,%d,%d) : %d : %d
  // ...",redTint,greenTint,blueTint,rotation,swap);
...
void TintManager::TintBitmap(Bitmap *thisBitmap) const {
  ...
  for (int i = 0; i < (w * h); i++) {
    uint16 val = *bitmapData;
    *bitmapData++ = myTintTable[val];
  }
}
