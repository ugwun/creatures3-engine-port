...
static bool InitInstance(HINSTANCE instance)
{
	...
	theRegistry.CreateValue("Software\\MyEngine\\", "Default Game", theApp.GetGameName());
...
uint32 WINAPI Ticker( LPVOID ob )
{
	// Catch C style exceptions as C++ style ones
	// for more informative messages
	ConvertCStyleExceptionsToCppStyle();
...
bool SetMultimediaTimer(bool bActive)
{
	...
	if (bActive)
	{
		if (ourTimerID == 0)
		{
			ourTimerID = timeSetEvent( theApp.GetWorldTickInterval(), 0, TickProc, 0, TIME_PERIODIC );
			if( ourTimerID == NULL )
			{
