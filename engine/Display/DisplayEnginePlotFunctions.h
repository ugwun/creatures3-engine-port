...
inline int Get555Red(int colour) { return (colour >> 7) & 0xf8; }

inline int Get555Green(int colour) { return (colour >> 2) & 0xf8; }

inline int Get555Blue(int colour) { return (colour << 3) & 0xf8; }

inline int Get565Red(int colour) { return (colour >> 8) & 0xf8; }

inline int Get565Green(int colour) { return (colour >> 3) & 0xfc; }

inline int Get565Blue(int colour) { return (colour << 3) & 0xf8; }

inline void DisplayEngine::OffsetDrawTile(Position &topLeft, Bitmap &bitmap,
                                          RECT &clip) {
  ...
  if (screen_ptr == NULL)
    return;
  screen_ptr +=
      (bitmap.GetPosition().GetY() * screen_step) + bitmap.GetPosition().GetX();

  RECT rect = {0, 0, (int)bitmap.GetWidth(), (int)bitmap.GetHeight()};
  ...
  // Almost lifted from DrawBitmap - so any problems - remember to address there
  // too :)
  ...
  if (rectWidth & 1)
    _asm
    {
		...
		;mov ebp,dword ptr [dwordWidth]		; ebp is the width of the bitmap - already shifted to uint32 size
...
;		sub eax,1							; decrement bitmap height properly
      ...
      eax is now the number of lines to plot.mov esi, dword ptr[data_ptr];
      esi points to sprite data now mov edi, dword ptr[screen_ptr];
      edi points to screen data now;
      mov ebp, dword ptr[dwordWidth];
      ebp is the width of the bitmap - already shifted to uint32 size mov ebx,
          dword ptr[data_step];
      ebx is now the data step mov edx, dword ptr[screen_step];
      edx is now the screen step lineloop2 :;
      mov ecx, ebp;
      ecx gets the number of dwords to copy per loop mov ecx,
          dword ptr[dwordWidth] rep movs dword ptr[edi], dword ptr[esi];
      copy the dwords from data to screen lea esi, [esi + ebx * 2];
      esi += ebx(data_ptr += data_step) lea edi, [edi + edx * 2];
      edi += edx(screen_ptr += screen_step);
      sub eax, 1;
      decrement bitmap height properly dec eax jne lineloop2;
      If not zero, do another line
    }
#endif
}
...
// Returns:     None
//
// Description: The interface to the display object this method creates
//				surface.
//
// ----------------------------------------------------------------------
inline void DisplayEngine::DrawBitmap(Position &position, Bitmap &bitmap) {
  ...
  // For the memcpy path, advance by the FULL source row width and the full
  // screen row pitch. The asm path below uses (step - bitmapWidth) because
  // REP MOVS already advances the pointer; memcpy does not touch our pointers.
  {
    int widthinbytes = bitmapWidth << 1;
    int srcRowStride = bitmap.GetWidth(); // full source row in uint16 units
    for (; bitmapHeight--;) {
      memcpy(screen_ptr, data_ptr, widthinbytes);
      data_ptr += srcRowStride; // advance to next source row
      screen_ptr += myPitch;    // advance to next dest row
    }
  }
#else
  ...
  if (bitmapWidth == 0 || bitmapHeight == 0)
    return;
  // In the asm path REP MOVS advances ESI/EDI by bitmapWidth already,
  // so we only step by the remaining difference.
...
;		sub eax,1							; decrement bitmap height properly
      ...
      mov eax, dword ptr[bitmapHeight];
      eax is now the number of lines to plot.mov esi, dword ptr[data_ptr];
      esi points to sprite data now mov edi, dword ptr[screen_ptr];
      edi points to screen data now;
      mov ebp, dword ptr[dwordWidth];
      ...
      sub eax, 1;
      decrement bitmap height properly dec eax jne lineloop2;
      If not zero, process another line
    }
#endif
}
...
//
// ----------------------------------------------------------------------
inline void DisplayEngine::DrawWholeBitmapRegardless(Position &position,
                                                     Bitmap &bitmap) {
  ...
  // Apply the tile's screen position (this is done in the asm branch below
  // but was missing from the C path, causing all tiles to write to (0,0)).
