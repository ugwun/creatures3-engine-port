...
// Purpose:		This class handles the images of all the creatures in
// the
//				system.
//				All Creature data is stored in one large
// database.
//
//
//
//
// Description:
//				A 16MB memory mapped file is allocated for all
// creatures. 				  The file has
// a header and slots for 18 creatures precedeed by a key
// (the creature's unique
// id) and an offset to the next creature 				 data.
// The CreatureFile is a linked list of creature data. When a new creature file
// is requested the file is created 				and then slotted
// into the next free slot (where the key is null).
// When a creature must be deleted all we need do is set the key
// to null.
//
//				There should only be one instance of the
// Creature file. 				The slots are for the size of
// the largest possible creature.
//
//
//
//
...
CreatureGallery::CreatureGallery(int32 sizeOfSpriteSet, std::string name)
    : mySizeOfASetOfNornSprites(sizeOfSpriteSet), myMaxCreatures(1),
      myCurrentPartNumber(0), myRewindToHeaders(0), myGallery(NULL), myPart(0),
  ...
  myTemplateSize =
      mySizeOfASetOfNornSprites * 1048576; // all creature files should fit in
  myCreatureGallerySize =
      myMaxCreatures * myTemplateSize; // room for 4 creatures
...
//				creatures.  The key (moniker) for eah slot at
// this point 				is zero.
//
// ----------------------------------------------------------------------
void CreatureGallery::CreateTemplateFile() {
  ...
  while ((done * myTemplateSize) < myCreatureGallerySize) {
    ...
    uint32 nextSlotOffset = done * myTemplateSize;
    OutputFormattedDebugString(
        "CreatureGallery::CreateTemplateFile: slot=%u nextOffset=%u "
        "templateSize=%u gallerySize=%u mmapLen=%u\n",
        done, nextSlotOffset, myTemplateSize, myCreatureGallerySize,
        myMemoryMappedFile.GetLength());
    myMemoryMappedFile.WriteUINT32(nextSlotOffset);

    bytesWritten += 8;

    myMemoryMappedFile.Seek(nextSlotOffset, File::Start);
  }
}
...
//							its genome.  Also used
// as a key for the 						composite
// creature gallery file. 				numSpritesInFile - yes
// Returns:		a pointer to the
// Description:	Creates a creature gallery getting the correct body parts
//				as defined in the genome.  Then slots the data
// in the 				first free slot.
//
// ----------------------------------------------------------------------
Gallery *CreatureGallery::AddCompressedCreature(
    std::string moniker, uint32 uniqueID, int32 PVariantGenus[NUMPARTS],
    int32 PVariant[NUMPARTS], uint32 validParts, uint8 Sex, uint8 Age,
    int32 BodySprites[NUMPARTS], CompressedGallery creatureParts[NUMPARTS],
    int32 numSpritesInFile) {

  switch (myCreatureBuildingStage) {

  case STAGE_ONE:
    if (!Part1CreatureBuilder(BodySprites, creatureParts, numSpritesInFile,
                              uniqueID, validParts))
      myCreatureBuildingStage = STAGE_COMPLETE;
    break;
  case STAGE_TWO:
    Part2CreatureBuilder(validParts, BodySprites, creatureParts);
    break;
  case STAGE_THREE:
    Part3CreatureBuilder(creatureParts, validParts);
    break;
  case STAGE_FOUR:
    Part4CreatureBuilder(creatureParts, validParts);
    break;
  case STAGE_FIVE: {
...
#ifdef _WIN32
    myGallery =
        new CompressedGallery(galleryName, myMemoryMappedFile.GetFileMapping(),
                              0, 8, myNumberOfBytesWritten);
#else
    // POSIX: the creature data was written to myName (the creature gallery
    // file) via mmap during stages 1-4. Flush the mapped pages so the
    // file on disk is up to date, then create a CompressedGallery by
    // opening the same file and seeking past the 8-byte creature gallery
    // header (4-byte uniqueID + 4-byte offset).
    {
      CompressedGallery *cg = new CompressedGallery();
      cg->SetName(galleryName);
      // Use myCreatureGallerySize so the mapping matches the full
      // ftruncated file, not just the bytes written so far.
      cg->myMemoryMappedFile.Open(myName, GENERIC_READ,
                                  FILE_SHARE_READ | FILE_SHARE_WRITE,
                                  myCreatureGallerySize);
      cg->myMemoryMappedFile.Seek(8, File::Start);
      if (!cg->InitBitmaps()) {
        delete cg;
        cg = NULL;
      }
...
bool CreatureGallery::Part1CreatureBuilder(
    int32 BodySprites[NUMPARTS], CompressedGallery creatureParts[NUMPARTS],
    int32 numSpritesInFile, uint32 uniqueID, uint32 ValidParts) {
  ...
  if (lastOffset < myCreatureGallerySize) {
    ...
    myNumImagesHolder = (uint16 *)(myMemoryMappedFile.GetFileStart() +
                                   myMemoryMappedFile.GetPosition());
    myMemoryMappedFile.Seek(sizeof(uint16), File::Current);
...
void CreatureGallery::Part2CreatureBuilder(
    uint32 ValidParts, int32 BodySprites[NUMPARTS],
  ...
  if (myPart < ValidParts) {
    ...
    CompressedBitmap *bitmap =
        (CompressedBitmap *)creatureParts[myPart].GetBitmap(0);
...
void CreatureGallery::Part3CreatureBuilder(
    CompressedGallery creatureParts[NUMPARTS], uint32 ValidParts) {
  // For each gallery...
  ...
  for (int Part = 0; Part < ValidParts; Part++) {
    ...
    CompressedBitmap *bitmap =
        (CompressedBitmap *)creatureParts[Part].GetBitmap(0);
    // and for each image in gallery...
    count = creatureParts[Part].GetCount();
    for (uint32 i = 0; i < count; i++) {
      // On Windows, STAGE_FIVE creates a MapViewOfFile starting at byte 8
      // (past the creature gallery header), so offsets must be relative to
      // byte 8.  On POSIX, mmap maps the whole file from byte 0, so offsets
      // must be file-absolute.
#ifdef _WIN32
      startOfPixelData =
          myMemoryMappedFile.GetPosition() - 8; // relative to byte 8
#else
      startOfPixelData =
          myMemoryMappedFile.GetPosition(); // file-absolute for POSIX
#endif
      ASSERT(bitmap);
...
void CreatureGallery::Part4CreatureBuilder(
    CompressedGallery creatureParts[NUMPARTS], uint32 ValidParts) {
  if (myPart == 0) {
...
uint32 CreatureGallery::FindSlot(uint32 key) {
  ...
  OutputFormattedDebugString(
      "CreatureGallery::FindSlot key=%u name=%s templateSize=%u "
      "gallerySize=%u mmapLength=%u\n",
      key, myName.c_str(), myTemplateSize, myCreatureGallerySize,
      myMemoryMappedFile.GetLength());
  ...
  while (myMemoryMappedFile.ReadUINT32() != key) // GetUINT32At(data) != key)
  {
    ...
    // Also guard against the offset exceeding the actual mmap length
    // to prevent Seek() from throwing on a size mismatch.
    if (lastOffset < myCreatureGallerySize &&
        lastOffset <= myMemoryMappedFile.GetLength()) {
