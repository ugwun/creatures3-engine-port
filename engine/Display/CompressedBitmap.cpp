...
//				Compressed bitmaps have pointers to the first
// pixel 				in every line.
//
//				They are not writable they always have
//				their data in a memory mapped file and should
// not 				be used with a cloned gallery.
//
// History:
// -------
//		   Alima			Created
...
//
// ----------------------------------------------------------------------
void CompressedBitmap::Save(File &file) { ASSERT(FALSE); }
...
//				scanLineStartPtr - pointer to the place in the
// gallery 				file where the scan line offset values
// are stored.
//
// Returns:     the number of bytes we have written
//
// Description: Stores a pointer to each line in the compressed bitmap.
//				The first line's offset value is stored in
// myOffset 				which is the original S16 header member.
// The other 				scanline offset values are stored in a
// block directly after 				the formal header.
//
//
// ----------------------------------------------------------------------
...
//				store the actual lengths of lines of data so
// that we 				can store scanlines in heap memory.
//
//
// ----------------------------------------------------------------------
