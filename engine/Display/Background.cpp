...
// have the 				file on and update the given path.  The
// path is only
...
// to draw. 				Get the real world coordinates and then
...
void Background::Draw(bool completeRedraw, std::vector<RECT> &dirtyRects,
  ...
  div_t x_div = div((int)myWorldPosition.GetX(),
                    (int)DEFAULT_ENVIRONMENT_RESOLUTION); // bitmap width
  div_t y_div = div((int)myWorldPosition.GetY(),
                    (int)DEFAULT_ENVIRONMENT_RESOLUTION); // bitmap height
