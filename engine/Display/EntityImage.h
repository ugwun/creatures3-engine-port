...
//evolve as required. 				Some member may appear now to allow integration with the
//current 				version of the C2e engine but be removed later.
//
//
//
// Description:
...
//CompoundObjects. 	|					Also serves as base
//class for articulated entities, 	| 					for
//encapsulation within Creatures. 	| 					Whereas
//a Sprite uniquely represents a given bitmap, 	|
...
//connected to 	| 					one or no daughter Limbs
//(eg. a thigh Limb connects 	| 					to a shin Limb);
//A Body is similar, but connects 	Limb                to several Limb chains.
//The BodyPart class contains 						methods & data that are common to both types 						Body
//& Limb classes contain pointers to BodyData 						or LimbData classes respectively.
//
//						The BodyPart Entities contain
//pointers to LimbData 						or BodyData structs, which contain the XY 						information
//that is common to all BodyParts with 						the same appearance.
//
//
//
// History:
...
//names from Object to Entity. 							Introducing Methods from the C2 Entity class.
//
// 27Nov98	Alima			Made constructor and destructor public
// so that the
//							C2e engine can create
//EntityImages. 							Adding galleries to EntityImages to replace current 							single bit
//map method.  Eventually I will remove 							old method.
...
public:
  ...
  EntityImage(FilePath const &galleryName, int32 numimages,
              int32 plane,          // plane
  ...
  Vector2D CentrePoint() const {
    return Vector2D(GetWorldX() + (GetWidth() / 2.0f),
                    GetWorldY() + (GetHeight() / 2.0f));
  }
  ...
  //
  // ----------------------------------------------------------------------
  Position &GetPosition(void) { return myWorldPosition; }
  ...
  //
  // ----------------------------------------------------------------------
  int32 GetWorldX() const { return myWorldPosition.GetX(); }

  int32 GetWorldY() const { return myWorldPosition.GetY(); }
  ...
  Sprite &GetSprite() const { return *mySprite; }

  Sprite *GetSpritePtr() const { return mySprite; }
  ...
  //
  // ----------------------------------------------------------------------
  uint32 GetCurrentIndex() const { return myCurrentImageIndex; }
  ...
  void CreateGalleryUponSerialisation(bool flag) {
    myReadArchivedGalleryFlag = flag;
  }
  ...
  //
  // ----------------------------------------------------------------------
  int32 GetPose() { return (myCurrentImageIndex - myCurrentBaseImageIndex); }
  ...
  int32 GetAbsolutePoseMax() { return myNumberOfImages; }

  int32 GetAbsoluteBaseImage() { return myAbsoluteBaseImage; }
  ...
  //				baseimage - the first image in the gallery
  ...
  //
  // ----------------------------------------------------------------------
  virtual bool SetGallery(FilePath const &galleryName, uint32 baseimage,
                          uint32 numImages = 0, bool reset = true);

  virtual void ReloadGallery(FilePath const &galleryName, uint32 baseimage,
                             uint32 numImages = 0);
  ...
  const FilePath &GetDefaultGalleryName() const { return myDefaultGalleryName; }
  ...
  bool IsPixelTransparent(uint32 x, uint32 y, int32 imageIndex = -1);
