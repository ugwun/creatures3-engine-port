...
//				The gallery is read on demand from a memory
// mapped file
//
//
//
// Description: On creation the gallery works in one of two ways:
//				Sprite galleries create a bitmap shell for each
// sprite 				which contains its width and height
// Background galleries rely on the fact that all tiles have the
// same width and height.  Only one bitmap shell  is created.
//
//				When the gallery is accessed the bitmap is
// updated to the 				start of its data in the memory
// mapped file.
//
//				NOTE ALL SPRITE FILES ARE CONSIDERED
// UNCOMPRESSED FOR NOW
//
//
// History:
// -------  Chris Wylie		created
// 11Nov98	Alima			Now reads galleries from memory mapped
// files.
//							background files now
// contain their tile width and
...
//				right initialisation fucntion depending on
// whether this is 				a background file or a sprite
// file
//
//				Note needs some exception handling
//
// ----------------------------------------------------------------------
CompressedGallery::CompressedGallery() : myCompressedBitmaps(NULL) {}

CompressedGallery::CompressedGallery(FilePath const &galleryName)
    : Gallery(galleryName), myMemoryMappedFile(galleryName.GetFullPath()),
...
bool CompressedGallery::Reload(FilePath const &name) {
  ...
  myPixelFormat = 0;
  myMemoryMappedFile
      .Close(); // Close any previously-opened file before reopening (POSIX)
...
//				highOffset -
//				lowOffset - these make the point in the file to
// start 							mapping from
// numBytesToMap - how many bytes to map.
//
// Description: This memory maps the supplied file.  Then it Initialises
//				the bitmaps.  This can be used for the Creature
// gallery 				which a composite file of galleries
// hanlded elsewhere. 				The gallery only needs to map to
...
CompressedGallery::CompressedGallery(FilePath const &filename,
                                     HANDLE memoryFile, uint32 highOffset,
                                     uint32 lowOffset, uint32 numBytesToMap)
    : Gallery(filename),
      myMemoryMappedFile(memoryFile, highOffset, lowOffset, numBytesToMap),
...
CompressedGallery::~CompressedGallery() { Trash(); }
...
//				and heights.  Each bitmaps data is set to the
// start of the 				the bitmaps data in the file.
//
//
// ----------------------------------------------------------------------
...
bool CompressedGallery::LoadFromC16(FilePath &fileName) {
  ...
  // Strip the 16-bit marker flag if present. Creature body-part C16 files
  // (written by CreatureGallery) always set C16_16BITFLAG alongside
  // C16_FLAG_565. We only need the 565/555 format bit going forward.
  ...
  if (myCount == 0) {
    ...
    ErrorMessageHandler::Show(
        "gallery_error", 1, std::string("CompressedGallery::CompressedGallery"),
...
//								if this is -1
// then use the current image 				x - x coordinate of the
// pixel to test 				y - y coordinate of the pixel to
// test
//
...
//
// ----------------------------------------------------------------------
bool CompressedGallery::IsPixelTransparent(uint32 x, uint32 y,
                                           int32 imageIndex) {
...
bool CompressedGallery::ValidateBitmapSizes(int32 minimumWidth,
                                            int32 minimumHeight) {
...
int32 CompressedGallery::GetBitmapWidth(uint32 index) {
  ...
  return index < myCount ? myCompressedBitmaps[index].GetWidth()
                         : myCompressedBitmaps[0].GetWidth();
}

int32 CompressedGallery::GetBitmapHeight(uint32 index) {
  ASSERT(index < myCount);
  return index < myCount ? myCompressedBitmaps[index].GetHeight()
                         : myCompressedBitmaps[0].GetHeight();
}
