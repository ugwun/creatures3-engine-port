...
Camera::Camera(int32 viewx,  // world x co-ordinate of view
  ...
  if (!Create(viewx,                // world x co-ordinate of view
    ...
    std::string string = ErrorMessageHandler::Format(
        theDisplayErrorTag, (int)DisplayEngine::sidNoBackgroundCreated,
...
Camera::Camera() : myWorldPosition(0, 0) {
  myAmIACameraFlag = true;
  InitialiseCameraVariables();
  myEntityHandler.SetInterestLevel(false);
}
...
Camera::~Camera() {
  // On non-Windows platforms, objects may still be registered when the camera
  // is destroyed during shutdown, so we don't assert here.
...
bool Camera::Create(int32 viewx,  // world x co-ordinate of view
  ...
  if (!myBackground0.Create(
          back1, Position(topLeftXofBackground, topLeftYofBackground), this)) {
...
bool Camera::Create(int32 viewx,  // world x co-ordinate of view
  ...
  if (!myBackground0.Create(back1, bounds, this)) {
    return false;
  }
...
void Camera::DoChangeMetaRoom(std::string &backgroundName, RECT &bounds,
  ...
  if (useTopLeftOnly) {
    ok = myBackgrounds[0]->Create(backgroundPath,
                                  Position(bounds.top, bounds.left), this);
  } else {
    ok = myBackgrounds[0]->Create(backgroundPath, bounds, this);
  }
...
void Camera::Update(bool completeRedraw, bool justBackBuffer) {
  ...
  // Draw the background and all entities to the back buffer, then flip.
  // SDL uses the main back buffer (NULL surface);
  // non-SDL remote cameras use their own mySurface.
#ifdef C2E_SDL
  DisplayEngine::theRenderer().Update(myBackgrounds[0], &myEntityHandler,
                                      completeRedraw, justBackBuffer, NULL);
#else
  DisplayEngine::theRenderer().Update(myBackgrounds[0], &myEntityHandler,
...
void Camera::FadeScreen() { DisplayEngine::theRenderer().FadeScreen(); }
...
//
// ----------------------------------------------------------------------
void Camera::AddDirtyRect(RECT rect) { myEntityHandler.AddDirtyRect(rect); }
