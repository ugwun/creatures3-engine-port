...
//mapped file. 				The bitmap data is compressed
//
//
//
// Description: On creation the gallery works in one of two ways:
//				Sprite galleries create a bitmap shell for each
//sprite 				which contains its width and height 				Background galleries rely on the
//fact that all tiles have the 				same width and height.  Only one bitmap shell  is
//created.
//
//				When the gallery is accessed the bitmap is
//updated to the 				start of its data in the memory mapped file.
//
//
// History:
// -------  Chris Wylie		created
...
public:
  ...
  //start 							mapping from 				numBytesToMap - how many bytes to map.
  //
  // Description: This memory maps the supplied file.  Then it Initialises
  //				the bitmaps.  This can be used for the Creature
  //gallery 				which a composite file of galleries hanlded elsewhere. 				The gallery
  //only needs to map to it's part of the file.
  //
...
#ifdef _WIN32
  CompressedGallery::CompressedGallery(FilePath const &name, HANDLE memoryFile,
                                       uint32 highOffset, uint32 lowOffset,
  ...
  */
  ...
  //				width - the width of the bitmaps that the whole
  //file will 				be split into. 				height - the height of the individual bitmaps that
  //the 				whole file will be split into.
  //
  ...
  virtual bool IsPixelTransparent(uint32 x, uint32 y, int32 imageIndex);
