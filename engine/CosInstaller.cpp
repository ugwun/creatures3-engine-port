...
CosInstaller::CosInstaller() {
  ...
  if (access(switcher.c_str(), F_OK) == 0)
  {
...
// Load an ordered list of absolute bootstrap directories.
// Used when directories come from multiple base paths (e.g. DS + C3).
// The bool parameter distinguishes this from the relative-path constructor.
CosInstaller::CosInstaller(std::vector<std::string> &absBootstrapDirs,
                           bool /*absolutePaths*/) {
  if (absBootstrapDirs.empty())
    return;

  SetUpProgressBarAbs(absBootstrapDirs);

  for (const auto &dir : absBootstrapDirs) {
    LoadCosFiles(dir, true);
  }

  theApp.EndProgressBar();
}

void CosInstaller::SetUpProgressBarAbs(std::vector<std::string> &absDirs) {
  theApp.StartProgressBar(2);
  int n = 0;
  std::vector<std::string> files;
  for (const auto &dir : absDirs) {
...
bool CosInstaller::AddScript(Classifier classifier) {
  ...
  if (m) {
    ...
    if (theApp.GetWorld().GetScriptorium().InstallScript(m)) {
      ...
      out << '\0';
      ErrorMessageHandler::Show("script_error", 4, "CosInstaller::AddScript",
                                out.str().c_str());
    }
#endif
...
bool CosInstaller::ReadScriptFile(std::string const &filename,
                                  bool updateScriptoriumOnly /*=false*/) {
  theFlightRecorder.Log(4, "CosInstaller: loading %s", filename.c_str());
...
bool CosInstaller::ReadScriptStream(std::istream &in,
  ...
  do {
    ...
    if (!strncmp(buf, "scrp", 4)) {
      ...
      std::istringstream get_class(buf);
      char buf_class[1024];
  ...
  for (it = myExecuteScripts.begin(); it != myExecuteScripts.end(); it++) {
    ...
    if (m) {
      try {
        ...
        ErrorMessageHandler::Show("script_error", 4,
                                  "CosInstaller::ExecuteScripts", out.str().c_str());
      ...
      ErrorMessageHandler::Show("script_error", 4,
                                "CosInstaller::ExecuteScripts", out.str().c_str());
      // Don't abort remaining scripts — one bad file shouldn't prevent
      // critical scripts (e.g. login_disabler.cos) from executing.
      continue;
    }
  }
