...
public:
  ...
  // Arguments:   file - path to a catalogue file
  //              langid - preferred language
  //              tagName - name of the specific tag to import
  // Returns:     true if the tag was found and imported
  // Description:
  // Parses a catalogue file but only imports strings belonging to the
  // specified tag.  All other tags in the file are skipped.  This is
  // used to cherry-pick individual tags from auxiliary catalogue files
  // (e.g. C3) that cannot be loaded in full without causing ID conflicts.
  // ------------------------------------------------------------------------
  bool AddTagFromFile(const std::string &file, const std::string &langid,
...
inline const char *Catalogue::Get(const std::string &basetag, int offsetid) {
  ...
  if (offsetid < 0 || offsetid >= noItems) {
    throw Err("CLE0012: Index %d out of range 0 to %d for tag %s", offsetid,
              noItems - 1, basetag.c_str());
  }
