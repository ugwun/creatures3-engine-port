...
#endif
        str << "Exception in Pray System - eid = " << pe.GetCode() << ", msg = "
<< pe.GetMessage(); #ifdef C2E_OLD_CPP_LIB return BasicException(str.str());
#else
...
int PrayHandlers::ConvertCategoryToAppId(int cat) {
  ...
  case 11:
    return -1; // Denied :) return PRAYFILE_DIR;
  default:
    return -1;
  }
  return -1;
...
int PrayHandlers::Dependency_Helper(CAOSMachine &vm, std::string &name,
                                    int doinst) {
  try {
    // 1. Get Chunk from Manager...
    PrayChunkPtr chunk(theApp.GetResourceManager().GetChunk(name));
...
// DS: PRAY FORE resource_type last_known
// Forward-iterate the PRAY resource list of the given type, starting after
// 'last_known'. Returns the name of the next chunk, or the first chunk if
// 'last_known' is not found. Returns "" if there are no chunks of that type.
// This is equivalent to PRAY NEXT and is an alias used in DS bootstrap scripts.
...
int PrayHandlers::IntegerRV_PRAY_IMPO(CAOSMachine &vm) {
  ...
  try {
    ...
    for (int historyIndex = 0; historyIndex < count; ++historyIndex) {
      ...
      if (!photo.empty()) {
        const PrayChunkPtr &gC =
            theApp.GetResourceManager().GetChunk(photo + ".photo");
        File f;
    ...
    if (porching) {
      std::string creatureFile = theApp.GetResourceManager()
                                     .GetChunkParentFile(moniker + ".creature")
                                     .c_str();
...
int PrayHandlers::IntegerRV_PRAY_EXPO(CAOSMachine &vm) {
  ...
  Creature &creature =
      vm.GetCreatureTarg(); // Check we're looking at a valid creature
  ...
  for (int i = 0; i < theStore.GetSlotCount(); i++) {
    ...
    if (!interestingMoniker.empty()) {
      if (theApp.GetResourceManager().CheckChunk(interestingMoniker +
                                                 ".genetics") != 0)
  ...
  try {
    theApp.GetResourceManager().AddChunkToFile(
        moniker + ".glist.creature", "GLST", path.GetFullPath(),
#ifdef C2E_OLD_CPP_LIB
        geneStream.tellp(), (uint8 *)hackbuf,
#else
        geneStream.str().size(), (uint8 *)geneStream.str().data(),
#endif
        true);
  } catch (PrayException &pe) {
    vm.ThrowRunError(helper_Convert(pe));
  ...
  try {
    theApp.GetResourceManager().AddChunkToFile(moniker + ".creature", "CREA",
                                               path.GetFullPath(),
#ifdef C2E_OLD_CPP_LIB
                                               stream.tellp(), (uint8 *)hackbuf,
#else
                                               stream.str().size(),
    ...
    /*		static char x = 'a';
                    std::string boo("c:\\moose.foo");
                            boo+=x;
                                    theApp.GetResourceManager().AddChunkToFile(
       moniker + ".creature", "ZOOB", boo+x, stream.str().size(),
                                                                                                            (uint8*)stream.str().data(),
                                                                                                            false );
                    x++;
    */
  ...
  for (monIter = geneNames.begin(); monIter != geneNames.end(); ++monIter) {
    ...
    try {
      theApp.GetResourceManager().AddChunkToFile(
          *monIter + ".genetics", "GENE", path.GetFullPath(), f.GetSize(),
          geneticData, true);
    } catch (PrayException &pe) {
      delete[] geneticData;
      f.Close();
  ...
  for (int historyIndex = 0; historyIndex < count; ++historyIndex) {
    ...
    if (!photo.empty()) {
      ...
      try {
        theApp.GetResourceManager().AddChunkToFile(photo + ".photo", "PHOT",
                                                   path.GetFullPath(),
                                                   f.GetSize(), data, true);
      } catch (PrayException &pe) {
        delete[] data;
  ...
  try {
    theApp.GetResourceManager().AddChunkToFile(
        moniker, chunkName, path.GetFullPath(),
#ifdef C2E_OLD_CPP_LIB
        sigstream.tellp(), (uint8 *)hackbuf,
#else
        sigstream.str().size(), (uint8 *)sigstream.str().data(),
#endif
        true);
  } catch (PrayException &pe) {
    vm.ThrowRunError(helper_Convert(pe));
  ...
  for (monIter = geneNames.begin(); monIter != geneNames.end(); ++monIter) {
    ...
    theAgentManager.ExecuteScriptOnAllAgentsDeferred(SCRIPT_LIFE_EVENT,
                                                     NULLHANDLE, p1, p2);
  }
