...
// -------------------------------------------------------------------------
// Class: PrayManager
// *Probably* the central asset management class for the PRAY format.
// PRAY (agent chunks) files contain resources like creatures, genomes, and
// scripts. This manager caches and looks them up.
// -------------------------------------------------------------------------
class PrayManager {
public:
  // ----------------------------------------------------------------------------------
  ...
  // Returns:		(None)
  // Description:	Adds the parameter to the localise list. It does not
  // rescan yet. Costs:		Immediate: Light, Later: Light to Medium
  // ----------------------------------------------------------------------------------
  ...
  //				Note that this also causes the manager to
  //execute a forced 				GarbageCollect and therefore it is a rather heavy function
  //if called 				more than once in the normal execution of a program.
  ...
  // Returns:		(None)
  // Description: This constructs a manager, with the given language ID -
  // PrayFiles
  //				loaded by this manager will first be localised
  //with the filelocaliser
  // ----------------------------------------------------------------------------------
  ...
  // Returns:		0 if chunk does not exist, otherwise the weight of
  // finding the chunk. Description:	This method scans the chunk map to find
  // its filereference. If it
  //				exists, then the chunk can be returned.
  //				If the chunk does not exist, 0 is returned.
  //				If the chunk is in the cache, 1 is returned
  //				If the chunk is not cached, but is uncompressed,
  //2 is returned 				If the chunk is on disk and compressed, 3 is returned.
  ...
  // Returns:		a PrayChunkPtr representing the data. Throws NULL on
  // failure. Description: Returns a PrayChunkPtr which represents the Chunk.
  //				The PrayChunkPtr allows ref-counts to be kept on
  //the data chunks.
  ...
  //				thisVec	- std::vector<std::string> - A vector to
  //put the chunks in
  // Returns:		(None)
  // Description:	Clears thisVec & adds the names of all chunks of the
  // type passed in. Costs:		Immediate: Light, Later: (None)
  // ----------------------------------------------------------------------------------
  ...
  //				thisFile - std::string - The file to put the
  //chunk in. 				thisSize - int         - The size for the chunk 				thisData - uint8*
  //- A pointer to the data to copy
  //              doCompress - bool      - Whether or not to compress the data
  // Returns:     (None)
  // Description: This method takes the data, and creates a new chunk on file
  // for it.
  //				If the file does not exist, it is created.
  //				That chunk is not then available until you
  //RescanFolders()
  // Costs:		Immediate: Heavy, Later, Medium
  // ----------------------------------------------------------------------------------
  void AddChunkToFile(std::string thisName, std::string thisType,
                      std::string thisFile, int thisSize, uint8 *thisData,
                      bool doCompress);
  ...
  // Returns:		(None)
  // Description:	If force is set, the manager forgets all chunks it has
  // cached. If
  //				a chunk is currently referenced, it isn't
  //deleted, but once all refs 				go out of scope, the chunks are deleted. All
  //chunks must be reloaded 				from disk later. If force is not set, only those
  //chunks which have 				no references outside of the manager are deleted.
  ...
  // Description: This call breaks the encapsulation of the chunk abstraction
  // layer.
  //				However it allows the engine to clean up after
  //creature exports etc.
  // Costs:		Immediate: Low, Later: None
  // ----------------------------------------------------------------------------------
