...
public:
  ...
  //				maxsize - size of largest allowable data
  //(request and 				          result sizes are both limited by this value)
  // Returns:		Success
  ...
  //				returncode - error code to return to client
  //(0=no error)
  // Description: Respond to the pending request and send the contents of
  //				the buffer back to the client.
  //				Don't forget to allow for the '\0' in the
  //resultsize if 				returning text! 				The pending request must be Respond()ed to
  //before 				any other requests will be accepted.
  // ----------------------------------------------------------------------
...
private:
  ...
  struct TransferHeader {
    ...
    uint32 ServerProcessId;   // id of server process
