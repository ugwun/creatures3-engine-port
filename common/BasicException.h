...
// -------------------------------------------------------------------------
// Class: BasicException
// *Probably* a lightweight base exception class used throughout the engine.
// Avoids using std::exception to keep C2E error handling isolated from
// standard C++ library errors.
// -------------------------------------------------------------------------
class BasicException {
public:
  BasicException(const char *msg) { myMessage = msg; }
  BasicException() { myMessage = ""; }

  virtual const char *what() const { return myMessage.c_str(); }
