...
// -------------------------------------------------------------------------
// Class: Vector2D
// *Probably* the primary 2D math primitive for the game engine.
// Handles position, velocity, sizing, and provides associated math operations.
// -------------------------------------------------------------------------
class Vector2D {
public:
  //
  ...
  inline Vector2D(const int vx, const int vy) : x((float)vx), y((float)vy) {}

  inline Vector2D(const float vx, const float vy) : x(vx), y(vy) {}
  ...
  inline float SquareLength(void) const { return x * x + y * y; }
  ...
  inline float DotProduct(const Vector2D &v) const { return x * v.x + y * v.y; }
