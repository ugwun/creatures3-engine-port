...
UINT WindowState::Load(const char* szSection, const char* szEntry, HWND hWnd, bool bNoSize /* = false */)
{
	ASSERT(hWnd);
...
void WindowState::Save(const char* szSection, const char* szEntry, HWND hWnd)
{
	ASSERT(hWnd);
	WINDOWPLACEMENT wndPlacement;
	wndPlacement.length = sizeof(WINDOWPLACEMENT);
