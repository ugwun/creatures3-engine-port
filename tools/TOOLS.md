...
7. **Inject** — click **Inject to Engine** in the inspector header to hatch the currently loaded genome as a live creature. The API writes a temporary uniquely-identified `.gen` file, executes a CAOS macro to hatch the creature, and immediately auto-cleans up the temporary `.gen` file so your library remains uncluttered.
