...
\begin{tikzpicture}[
  ...
  % Feedback: Response → Drive (routed below Verb to avoid crossing)
  \draw[arrow] (resp.south) -- ++(0,-1.2) -| (drive.south);

  % === LABELS ===
  \node[above=0.2cm of visn, label] {What the Norn perceives};
  \node[below=0.4cm of decn, label] {Highest score wins $\rightarrow$ Norn acts};
  \node[left=0.2cm of drive, label, text width=2.2cm] {Internal body state (hunger, pain, etc.)};
