...
\author{%
  ...
  \small \texttt{cyrilsadovsky@ae01.io}
}
\date{July 2026}
...
Large Language Models (LLMs) like ChatGPT and Claude achieve remarkable performance on benchmarks, yet they operate as ``smart amnesiacs'' in embodied, physical environments~\cite{grand1997creatures, grand1998creatures}. When placed in control of a physical robot and asked to perform a simple household task (finding and delivering a stick of butter), the best LLM achieves a 40\% completion rate while untrained humans score 95\%, failing particularly at spatial reasoning and social understanding~\cite{sharrock2025butterbench}. They lack what we call a \emph{limbic system}\footnote{In biological brains, the \emph{limbic system} is the set of brain structures responsible for emotions, motivation, and drives like hunger, fear, and pleasure. It acts as the brain's ``internal compass,'' guiding behavior based on bodily needs.}, an internal homeostatic drive system that would allow them to:
...
\textit{Creatures~3} (1999) is not a typical video game. It is a \emph{digital terrarium}, an artificial life simulation where virtual creatures called ``Norns'' are born, learn, eat, sleep, socialize, reproduce, and die, all driven by their own internal biology rather than pre-scripted behaviors~\cite{grand1997creatures, grand1998creatures, zucconi2020creatures}.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{creatures_3_docking_station.png}
\caption{\textbf{The Creatures 3 / Docking Station environment.} Norns (the small bipedal creatures) inhabit a virtual spaceship ecosystem with multiple rooms, flora, and interactive objects. Each Norn is an autonomous agent driven entirely by its internal neural network and biochemistry; the player can teach and nurture them but never directly controls their behavior. The interface elements (top-left) display the selected Norn's health and drives.}
\label{fig:creatures3-screenshot}
\end{figure}

Each Norn possesses a remarkably detailed internal simulation:
\begin{itemize}[nosep]
  \item A \textbf{815-neuron brain} organized into 15 specialized regions (called ``lobes''), connected by genetically defined synaptic tracts, forming a small but complete neural network.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{interactions_within_creature.png}
\caption{Summary of interactions within a creature and between the creature and its environment. Reproduced from Grand and Cliff~\cite{grand1998creatures}. The diagram traces a complete perception-action-metabolism cycle: sensory input flows through the brain's lobe hierarchy to produce a behavioral decision, while the biochemical consequences of that action (digestion, metabolic reactions, energy production) feed back into the drive system, closing the loop. This tight coupling between neural and metabolic systems is precisely what our experiment exploits by making brain activity deduct ATP.}
\label{fig:creature-interactions}
\end{figure}
...
This architecture (a metabolically embedded, genetically heritable neural network with homeostatic drives) provides a unique platform for testing whether the FEP's complexity penalty can be grounded in physical metabolic costs. The engine port~\cite{sadovsky2025creatures3} supports headless execution, speed multipliers, and native telemetry output, enabling fully automated batch experimentation with no external dependencies.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{developer_tools_brain.png}
\caption{\textbf{Brain monitor from the Creatures~3 engine port developer tools}~\cite{sadovsky2025creatures3}. This web-based interface, developed specifically for the engine port, provides real-time visualization of a Norn's complete neural architecture. The central panel renders all 15 lobes with per-neuron activation states (dark cells indicate firing neurons) and inter-lobe tract connections (orange lines). The right panel lists every lobe with its neuron count and all 29 tracts with their dendrite counts, alongside a summary of the selected creature's vital statistics (species, age, health, top drive). The left panel shows the full population roster with life-state indicators. This tooling enables the fine-grained neural telemetry required for our experiment.}
\label{fig:brain-monitor}
\end{figure}
...
\begin{description}[style=nextline, font=\bfseries]
  ...
  \item[H1.2: Constrained Creatures Survive Longer]
    ...
    \begin{enumerate}[label=(\roman*), nosep]
      ...
      \item a more stable population trajectory, with fewer wild swings between population booms and crashes.
    \end{enumerate}
...
The experiment uses the Creatures~3/Docking Station engine port~\cite{sadovsky2025creatures3}, a C++ codebase compiled for macOS ARM64. The engine provides several features critical for scientific experimentation:
\begin{itemize}[nosep]
...
\label{tab:lobes}
\small
\begin{tabular}{@{}lp{6.8cm}cl@{}}
\toprule
\textbf{Lobe (Brain Region)} & \textbf{What It Does} & \textbf{Neurons} & \textbf{WTA} \\
\midrule
\multicolumn{4}{@{}l}{\textit{Perception}} \\
\quad Noun (\texttt{noun})            & Identifies what object the Norn is looking at & 40 & Yes \\
\quad Visual (\texttt{visn})           & Registers all currently visible objects        & 40 & --- \\
\quad Verb (\texttt{verb})             & Tracks available actions                       & 13 & Yes \\
...
\quad Attention (\texttt{attn})        & Decides what to focus on                       & 40 & Yes \\
\quad Decision (\texttt{decn})         & Chooses what action to perform                 & 13 & Yes \\
...
\noindent \textbf{How the brain learns and adapts.} The brain's synaptic connections change through three mechanisms~\cite{grand1998creatures}:
\begin{itemize}[nosep]
...
\begin{tikzpicture}[
    ...
    lobe/.style={rectangle, draw=black, thick, fill=blue!8, minimum width=2.2cm, minimum height=0.8cm, align=center, font=\small\bfseries},
    lobekey/.style={rectangle, draw=black, thick, fill=orange!12, minimum width=2.2cm, minimum height=0.8cm, align=center, font=\small\bfseries},
    lobehub/.style={rectangle, draw=black, thick, fill=green!8, minimum width=2.2cm, minimum height=0.8cm, align=center, font=\small\bfseries},
    arrow/.style={-{Stealth[length=2.5mm]}, thick, draw=black!70},
    label/.style={font=\scriptsize\itshape, text=black!60},
  ]

  % === CENTER COLUMN: main vertical flow ===
  % Row 1: Sensory inputs
  \node[lobe] (noun) {Noun\\(40n)};
  \node[lobe, right=0.8cm of noun] (visn) {Visual\\(40n)};
  \node[lobe, right=0.8cm of visn] (smel) {Smell\\(40n)};

  % Row 2: Stimulus Source
  \node[lobehub, below=1.0cm of visn] (stim) {Stimulus Source (40n)};

  % Row 3: Combination
  \node[lobekey, below=1.0cm of stim, minimum width=5.5cm, fill=orange!15] (comb) {Combination (440n)};

  % Row 4: Decision
  \node[lobekey, below=1.0cm of comb, minimum width=3.0cm, fill=orange!15] (decn) {Decision (13n)};

  % === LEFT COLUMN: internal state ===
  \node[lobekey, left=1.8cm of comb] (drive) {Drive\\(20 needs)};
  \node[lobe, below=0.8cm of drive] (verb) {Verb\\(13n)};

  % === RIGHT COLUMN: outputs and feedback ===
  \node[lobe, right=1.8cm of comb] (attn) {Attention\\(40n)};
  \node[lobe, right=1.8cm of decn] (resp) {Response\\(20n)};

  % === ARROWS (all verified engine tracts) ===

  % Sensory → Stimulus Source (straight down)
  \draw[arrow] (noun) -- (stim);
  \draw[arrow] (visn) -- (stim);
  \draw[arrow] (smel) -- (stim);

  % Stimulus Source → Combination (straight down)
  \draw[arrow] (stim) -- (comb);

  % Drive, Verb → Combination (straight right)
  \draw[arrow] (drive) -- (comb);
  \draw[arrow] (verb) -- (decn);

  % Combination → Decision (straight down)
  \draw[arrow] (comb) -- (decn);

  % Combination → Attention (straight right)
  \draw[arrow] (comb) -- (attn);

  % Decision → Response (straight right)
  \draw[arrow] (decn) -- (resp);

  % Feedback: Response → Drive (routed left, below Verb, entering Drive from the left)
  \draw[arrow] (resp.south) -- ++(0,-1.2) -| ([xshift=-1.2cm]drive.west) -- (drive.west);

  % === LABELS ===
  \node[above=0.2cm of visn, label] {What the Norn perceives};
  \node[below=0.25cm of decn, label] {Highest score wins $\rightarrow$ Norn acts};
  \node[above=0.2cm of drive, label, text width=2.2cm, align=center] {Internal body state\\(hunger, pain, etc.)};
  \node[below=0.15cm of resp, label] {Feedback loop};

\end{tikzpicture}
\caption{\textbf{How a Norn's brain makes decisions (simplified).} Information flows top-to-bottom through verified engine tracts. Sensory input (what the Norn sees and smells) converges on the Stimulus Source lobe, which acts as a routing hub, feeding into the Combination lobe (440 neurons, the brain's largest region) alongside internal drives and verb categories. The Combination lobe outputs to both the Decision lobe (where Winner-Takes-All competition selects a single action) and the Attention lobe. A feedback loop (Decision $\rightarrow$ Response $\rightarrow$ Drive) closes the perception-action cycle. Orange-highlighted lobes are the primary sites where our metabolic cost is applied. Not shown for clarity: Motor, Detail, Situation, Friend-or-Foe, and Mood lobes (see Table~\ref{tab:lobes} for the complete 15-lobe architecture).}
...
\begin{tikzpicture}[
  ...
  \draw[arrow] (d1.east) -| node[pos=0.1, above, font=\scriptsize] {No} ([xshift=1cm]s5.east) -- (s5.east);
  \draw[arrow] (s6) -- (s7);
...
\begin{enumerate}
  \item \textbf{World Creation.} A fresh, empty world is created using the engine's API. This ensures no leftover state from previous runs, ensuring each trial is truly independent.
...
\begin{enumerate}[label=(\alph*)]
  ...
  \item \textbf{The game requires proprietary data.} Running the experiment requires game data files from \textit{Creatures Exodus} (available for purchase on Steam or GOG for approximately \$6). All novel code, analysis scripts, and results will be made publicly available.
\end{enumerate}
...
\begin{itemize}[nosep]
  \item \textbf{Engine source code:} \url{https://github.com/ugwun/creatures3-engine-port}
