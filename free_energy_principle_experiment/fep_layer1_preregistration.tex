...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textit{Pre-Registration: Metabolic Active Inference in Creatures 3}}
...
\title{%
  ...
  \large A Pre-Registration for Metabolic Active Inference\\in \textit{Creatures 3}%
}
\author{%
  Cyril Sadovsky\\[2pt]
...
% ─── Abstract ────────────────────────────────────────────────────────────────
\begin{abstract}
Modern large language models and artificial agents lack an underlying physiological homeostatic modulation system---a ``limbic gap'' that prevents continuous real-time learning and leads to energy-inefficient compute scaling. This pre-registration specifies a controlled experiment that grounds the abstract information-theoretic complexity penalty of Karl Friston's Free Energy Principle (FEP) in a \emph{physical metabolic cost}: each neuron firing and each synaptic migration event in a \textit{Creatures~3} Norn brain deducts adenosine triphosphate (ATP) from the agent's biochemistry. Using the open-source Creatures~3 engine port with Model Context Protocol (MCP) telemetry, we test two hypotheses: (1)~that metabolic brain constraints force the emergence of sparse, high-efficiency synaptic networks, and (2)~that metabolically constrained agents exhibit superior survival and homeostatic stability across multiple generations. We pre-register a 2-condition between-subjects design (N=10 per condition), running each trial to population extinction or 3~million ticks, with Mann-Whitney~U tests as primary confirmatory analysis and Bayesian model comparison as supplementary analysis.
\end{abstract}
...
\begin{description}[style=nextline, font=\small\bfseries, leftmargin=1.2cm, nosep, itemsep=3pt]
  ...
  \item[Pre-Registration] A public commitment to the design and analysis plan of an experiment \emph{before} running it, to ensure scientific transparency.
...
\begin{itemize}[nosep]
  \item A \textbf{952-neuron brain} organized into 9 specialized regions (called ``lobes''), connected by approximately 5{,}000 synapses---a small but complete neural network.
  ...
  \item \textbf{Three learning mechanisms}: connections that strengthen when useful (\emph{reinforcement}), weaken when unused (\emph{atrophy}), and physically detach and reattach to new neurons when they fail (\emph{migration}).
  \item A \textbf{homeostatic drive system} with 13 needs: Hunger, Pain, Tiredness, Fear, Boredom, Loneliness, Anger, and more. These drives are the Norn's ``emotions''---they motivate behavior just as your hunger motivates you to eat.
...
This architecture---a metabolically embedded, genetically heritable neural network with homeostatic drives---provides a unique platform for testing whether the FEP's complexity penalty can be grounded in physical metabolic costs. The open-source engine port~\cite{sadovsky2025creatures3} provides full programmatic access via the Model Context Protocol (MCP)\footnote{The Model Context Protocol (MCP) is a standard interface that allows AI systems to communicate with external tools. In this project, it enables an AI agent to directly observe and control the running game---pausing the simulation, reading brain states, injecting chemicals, and querying the 256 biochemicals in real time.}, enabling automated, headless experimentation with real-time telemetry.

% ─── 1.4 This Experiment ────────────────────────────────────────────────────
\subsection{The Present Experiment}

This pre-registration specifies \textbf{Layer~1} of a two-layer experimental program. In Layer~1 (this document), we introduce metabolic costs to \emph{internal brain operations}: each neuron firing and each synaptic migration event deducts ATP from the Norn's biochemistry. The core question is:
...
\begin{itemize}[nosep]
  ...
  \item \textbf{MCP integration}: 25 programmatic tools that allow an external script to inspect creatures, read brain states, and control the simulation in real time.
  \item \textbf{REST API}: a web-based interface for querying any aspect of the engine state, used by the data collection pipeline.
\end{itemize}
...
A Norn's brain is a small but complete neural network. Information flows from sensory input (what the Norn sees and hears) through several processing stages to a final decision (what the Norn does). Figure~\ref{fig:brain-architecture} shows this flow, and Table~\ref{tab:lobes} lists all nine brain regions.

\begin{table}[H]
\centering
\caption{Norn brain lobe architecture. \textbf{WTA} (Winner-Takes-All) means only the single most active neuron in that region ``wins'' and influences downstream processing---like a vote where only the top candidate matters.}
\label{tab:lobes}
\begin{tabular}{@{}llcl@{}}
\toprule
\textbf{Lobe (Brain Region)} & \textbf{What It Does} & \textbf{Neurons} & \textbf{Selection} \\
\midrule
Stimulus Source & Recognizes objects the Norn sees or hears & 40 & WTA \\
Noun            & Activates when an object name is spoken & 40 & WTA \\
Attention       & Decides what to focus on & 40 & WTA \\
Drive           & Tracks internal needs (hunger, pain, etc.) & 16 (13 active) & --- \\
General Sense   & Represents physical events (being touched, etc.) & 32 & --- \\
Verb            & Activates when an action word is spoken & 40 & WTA \\
Perception      & Combines information from other regions & --- & Copy \\
Concept         & Represents ``situations'' (combinations of needs + objects) & 256 & AND-gate \\
Decision        & Chooses what action to perform & 16 (12 active) & WTA \\
\bottomrule
\end{tabular}
\end{table}

The Decision lobe is where behavior is chosen. Each of its 12 action neurons (``push,'' ``eat,'' ``walk left,'' etc.) accumulates evidence from the Concept lobe---some connections encourage the action, others discourage it. The action with the highest score wins:
\begin{equation}
...
\noindent \textbf{How the brain learns and adapts.} The brain has approximately 5{,}000 active connections, and they change through three mechanisms:
\begin{itemize}[nosep]
  ...
  \item \textbf{Atrophy}: connections that are rarely used gradually weaken and decay. \emph{This is how Norns forget what doesn't work.}
  \item \textbf{Migration}: when a connection becomes too weak, it physically detaches from one neuron and reattaches to a different one, searching for more useful associations. \emph{This is how Norns restructure their understanding of the world.}
  \item \textbf{Sleep consolidation}: major brain updates (like strengthening and weakening connections) happen during sleep, not while awake---just as in biological brains.
\end{itemize}
...
\begin{tikzpicture}[
  ...
  % Sensory input layer
  \node[lobe] (stim) {Stimulus\\Source};
  \node[lobe, right=2.5cm of stim] (noun) {Noun};
  \node[lobe, below=0.8cm of stim] (gsense) {General\\Sense};
  \node[lobe, right=2.5cm of gsense] (verb) {Verb};

  % Attention
  \node[lobe, below=1.5cm of $(stim)!0.5!(noun)$] (attn) {Attention};

  % Drive
  \node[lobekey, left=1.8cm of gsense] (drive) {Drive\\(13 needs)};

  % Perception buffer
  \node[lobe, below=1.2cm of attn, minimum width=5cm] (perc) {Perception Buffer};

  % Concept
  \node[lobe, below=1.0cm of perc, minimum width=4cm, fill=green!8] (conc) {Concept (256n)};

  % Decision
  \node[lobekey, below=1.0cm of conc, minimum width=3.5cm, fill=orange!15] (decn) {Decision (12 actions)};

  % Arrows
  \draw[arrow] (stim) -- (attn);
  \draw[arrow] (noun) -- (attn);
  \draw[arrow] (attn) -- (perc);
  \draw[arrow] (drive) |- (perc);
  \draw[arrow] (gsense) -- (perc);
  \draw[arrow] (verb) |- (perc);
  \draw[arrow] (perc) -- (conc);
  \draw[arrow] (conc) -- node[right, label] {128 ``for'' / 128 ``against''} (decn);

  % Labels
  \node[above=0.2cm of stim, label] {What the Norn perceives};
...
\end{tikzpicture}
\caption{\textbf{How a Norn's brain makes decisions.} Information flows top-to-bottom: sensory input (what the Norn sees and hears) enters at the top, combines with internal needs (hunger, pain, etc.) in the Perception Buffer, generates ``situations'' in the Concept lobe, and finally selects an action in the Decision lobe. Orange-highlighted lobes are the primary sites where our metabolic cost is applied. Each connection between the Concept and Decision lobes carries a ``vote'' for or against an action.}
...
\begin{description}[style=nextline, font=\itshape]
  \item[Synaptic Firing Cost --- ``Thinking Costs Energy'']
    On every engine tick, for every neuron in the Decision and Concept lobes whose activation exceeds a threshold $\theta_{\text{fire}} > 0$, a fixed amount $\delta_{\text{fire}}$ of ATP is deducted from the Norn's biochemistry:
    \begin{equation}
...
This metric measures how disordered the Norn's internal state is. It uses a standard information theory formula (Shannon entropy) applied to the 13 homeostatic drive levels:
\begin{equation}
  \Hbio = -\sum_{i=1}^{13} P(d_i) \log_2 P(d_i)
  \label{eq:Hbio}
\end{equation}

\begin{plainbox}
Imagine a dashboard with 13 gauges---Hunger, Pain, Tiredness, Fear, and so on. If all gauges are fluctuating wildly and unpredictably, that's \textbf{high entropy} (disorder). If most gauges stay near comfortable levels with only small, predictable changes, that's \textbf{low entropy} (stability).
...
The rate of synaptic detachment and re-allocation across the $\sim$5{,}000 active connections:
\begin{equation}
...
\begin{enumerate}
  ...
  \item \textbf{Telemetry Snapshots.} Each snapshot records, for every living Norn:
    \begin{itemize}[nosep]
      ...
      \item All 13 drive levels (hunger, pain, tiredness, etc.).
...
The following analyses are declared as exploratory (not bound by the pre-registration) and will be reported separately:
\begin{itemize}[nosep]
...
\begin{tikzpicture}[
  ...
  % Engine
  \node[blockkey] (engine) {\textbf{Game Engine}\\(simulates creatures)};
  \node[below=0.2cm of engine, font=\tiny\itshape, text=black!50] {Brain, Biochemistry, Physics};

  % MCP
  \node[block, right=2.5cm of engine] (mcp) {\textbf{MCP Server}\\(AI interface)};

  % Telemetry
  \node[data, below=1.5cm of engine] (json) {\textbf{Data Snapshots}\\(every 1{,}000 ticks)};

  % Analysis
  \node[block, below=1.5cm of json] (python) {\textbf{Analysis Scripts}\\(Python)};

  % Results
  \node[data, right=2.5cm of python] (results) {\textbf{Results}\\(graphs + statistics)};

  % Runner
  \node[block, left=2.0cm of engine] (runner) {\textbf{Experiment}\\Runner};

  % Arrows
  \draw[arrow] (engine) -- node[above, font=\scriptsize] {real-time data} (mcp);
  \draw[arrow] (engine) -- node[right, font=\scriptsize] {write} (json);
  \draw[arrow] (json) -- (python);
  \draw[arrow] (python) -- (results);
  \draw[arrow] (runner) -- node[above, font=\scriptsize] {starts} (engine);
  \draw[darrow] (mcp) -- node[right, font=\scriptsize] {query} ++(0,-2.5) -| (json);

\end{tikzpicture}
\caption{\textbf{How the experiment runs automatically.} An experiment runner script launches the game engine in headless mode. The engine simulates the Norn colony and periodically writes data snapshots to disk. After all 20 runs are complete, Python scripts process the data into statistical results and graphs. The MCP Server provides an optional real-time interface for monitoring.}
...
\begin{enumerate}[label=(\alph*)]
  \item \textbf{Social communication is not tested here.} This pre-registration covers only internal brain metabolism (Layer~1). A follow-up experiment (Layer~2) will explore what happens when \emph{talking} also costs energy---testing whether metabolically expensive communication leads to more efficient language.
  ...
  \item \textbf{The brain is small.} The Creatures~3 brain has 952 neurons and $\sim$5{,}000 connections---vastly smaller than a biological brain (86 billion neurons) or a large AI model (billions of parameters). Our results demonstrate the \emph{principle} of metabolic grounding; whether the effect scales to larger architectures remains an open question.

  \item \textbf{The environment is fixed.} All trials run in the standard game environment. We do not vary food scarcity, introduce predators, or change seasonal conditions. Environmental challenges are deferred to future experiments.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Video --- AI reads AI (how MCP connects an AI agent to the game):}\\
