...
- [x] ~~Within the initial menu, when delete a world is selected, the world is not deleted.~~ Fixed: `DeleteDirectory()` and `CopyDirectory()` in `General.cpp` were no-op stubs on non-Windows. Implemented real POSIX versions using `opendir`/`readdir`/`unlink`/`rmdir` for recursive directory operations.
...
- [x] ~~Fullscreen doesn't work.~~ Fixed — SDL2 `SDL_WINDOW_FULLSCREEN_DESKTOP` toggle.
...
- [x] ~~Game crashes when an injected creature steps out of an elevator, and injected creatures 'float' down edges.~~ Fixed: Added a soft recovery for post-collision invalid positions in `Map::MoveCreatureInsideRoomSystem` instead of a hard assert. Fixed the Genetics Kit injection CAOS script to assign the correct physics values (`c3_creature_accg`, `perm`, `bhvr`) matching standard hatched eggs.
